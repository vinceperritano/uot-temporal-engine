...
def reset_calibration(keep: int = 7):
    """
    Delete all runs except the most recent `keep` completed simulations.
    Used to reset Calibration Lab baseline after architectural changes (e.g. Phase 5).
    Temporary endpoint — remove after baseline is re-established.
    """
    with get_db() as conn:
        all_runs = conn.execute(
            "SELECT id FROM runs WHERE status='simulated' ORDER BY created_at DESC"
        ).fetchall()
        runs_to_keep = {row["id"] for row in all_runs[:keep]}
        all_to_delete = conn.execute(
            "SELECT id FROM runs WHERE status='simulated'"
        ).fetchall()
        runs_to_delete = [r["id"] for r in all_to_delete if r["id"] not in runs_to_keep]
