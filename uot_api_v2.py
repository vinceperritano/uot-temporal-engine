...
# ══════════════════════════════════════════════════════════════════════════════
MAX_RUNS_PER_IP_PER_HOUR = int(os.environ.get("MAX_RUNS_PER_IP_PER_HOUR", "3"))
MAX_RUNS_PER_IP_PER_DAY  = int(os.environ.get("MAX_RUNS_PER_IP_PER_DAY",  "10"))
...
def _run_seed_pipeline(run_id: str) -> None:
    ...
    try:
        ...
        if demo_mode:
            ...
            # ── Diagnostic logging (Phase 4 edge debug) ──────────────────────
            node_ids_sample = [n.get("id","?") for n in estimated_nodes[:3]]
            edge_ids_sample = [(e.get("source_id","?"), e.get("target_id","?")) for e in final_edges[:3]]
            print(f"[EDGE_DEBUG] stage_c_edges={len(stage_c_edges)} final_edges={len(final_edges)}")
            print(f"[EDGE_DEBUG] node sample={node_ids_sample}")
            print(f"[EDGE_DEBUG] edge sample={edge_ids_sample}")

            graph = build_graph_from_estimates(estimated_nodes, final_edges)
            print(f"[EDGE_DEBUG] graph.edges after build={len(graph.edges)}")
