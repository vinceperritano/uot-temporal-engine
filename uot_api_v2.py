            ...
            try:
                ...
                for _sa_p in (_slot_assignments or []):
                    if hasattr(_sa_p, 'slot_id') and hasattr(_sa_p, 'effective_probability'):
                        ...
                        # Detect synthetic-backed slots: EVT_SYNTH_ prefix
                        _sa_eid = getattr(_sa_p, 'event_id', '')
                        if _sa_eid and str(_sa_eid).startswith('EVT_SYNTH_'):
...
def _compute_field_source_coherence(slot_source_coherence: dict,
    ...
    Phase 9.10 (GPT guidance): add probability-weighted coherence and
    probability_coverage as a companion metric.

    GPT's conceptual framing:
      Equal-weighted coherence = "Did we observe every branch well?"
                                 (observational breadth diagnostic)
      Probability-weighted coherence = "Is the field we actually believe likely
                                        coherently supported?" (trust gate)

    slot_probabilities: {slot_id: probability} — if provided, enables weighted
      coherence. Should be normalized; if not, is normalized internally.
    canonical_slot_ids: authoritative list of slot_ids — used to exclude label-
      keyed duplicate entries from the slot_source_coherence dict.

    Returns: {source_coherence, source_coherence_flag, source_coherence_equal,
              source_coherence_weighted, probability_coverage}
    """
    ...
    if slot_probabilities:
        ...
        # Probability coverage: fraction of total probability mass that is
        # evidence-bearing. GPT edge case: if a high-prob slot has no evidence,
        # coverage drops and the weighted coherence can look misleadingly good.
        _all_canon_ids = canonical_slot_ids or list(slot_source_coherence.keys())
        _bearing_slot_ids = {k for k, _ in _coh_items}
        _bearing_prob = sum(_norm_probs.get(k, 0.0) for k in _bearing_slot_ids)
        _prob_coverage = min(1.0, _bearing_prob)
