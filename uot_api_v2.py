...
def diagnostics_summary():
    ...
    with get_db() as conn:
        ...
        artifacts = conn.execute(
            "SELECT convergence_summary_json FROM run_artifacts WHERE convergence_summary_json IS NOT NULL"
        ).fetchall()
        iter_logs = conn.execute(
            "SELECT discrepancies_json, near_discrepancies_json, scores_after_json, fallback_used FROM iteration_logs"
        ).fetchall()
        interventions = conn.execute(
            "SELECT intervention_type, COUNT(*) as cnt FROM observer_interventions GROUP BY intervention_type"
        ).fetchall()
    ...
    return {
        ...
        "most_corrected_nodes":     sorted(corrected_nodes.items(), key=lambda x: -x[1])[:10],
