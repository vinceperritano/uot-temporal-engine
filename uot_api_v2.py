...
def create_run(req: CreateRunRequest):
    ...
    settings = json.dumps({"demo_mode": req.demo_mode})
    with get_db() as conn:
        conn.execute(
...
def seed_run(run_id: str):
    """
    Run Stages 0 → E and save the provisional graph for Observer Review.

    Fix 6: during seeding, store source_causal_evidence in node.metadata
    so that later edge-edit recomputation uses original source evidence,
    not the previously graph-derived value (which would create a feedback loop).
    """
    with get_db() as conn:
    ...
    if demo_mode:
        ...
        result = collect_and_extract_seeded_events(run["topic"], observer_basis)
        ...
        # Store original source_causal_evidence before graph derivation overwrites it
        for nid, node in graph.nodes.items():
    ...
    # For demo mode: backfill source_causal_evidence from known test values.
    # auto_estimated stores "causal_support_graph" (graph-derived), not the
    # original per-node source evidence. Using it as a fallback would feed
    # computed values back as independent evidence — exactly the feedback loop
    # we need to prevent. Use explicit known values instead.
    #
    # For live mode: source_causal_evidence is now stored by
    # build_graph_from_estimates() via estimate_uot_fields(), so the
    # metadata.setdefault() calls below are no-ops for live runs.
    DEMO_SOURCE_CAUSAL_EVIDENCE = {
        "e1":  0.90,   # serial cabinet firings — high causal support
        "e2":  0.85,   # approval collapse — strong upstream causation
        "e3a": 0.70,   # D house only — moderate causal chain
        "e3b": 0.55,   # D sweep — weaker chain (requires more alignment)
        "e3c": 0.30,   # R hold — low causal support given headwinds
        "e4":  0.75,   # NATO destabilization — well-supported
        "e5":  0.60,   # constitutional confrontation — conditional
        "e6a": 0.55,   # MAGA succession crisis — moderate
        "e6b": 0.45,   # MAGA stabilization — weaker
        "e6c": 0.40,   # MAGA fragmentation — speculative
        "e7":  0.60,   # Democratic fracture — moderate
    }
    for nid, node in world.event_graph.nodes.items():
        node.metadata.setdefault(
            "source_causal_evidence",
            str(DEMO_SOURCE_CAUSAL_EVIDENCE.get(nid, 0.5))
        )
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (state_id, run_id, "seeded",
             world_to_json(world),
             json.dumps(scores),
             json.dumps(stage_c_edges),
             now)
        )
        ...
        record_audit(conn, run_id, "pipeline_seeded", {
            ...
            "stage_c_edges": len(stage_c_edges),
        })

    return {
        "run_id":  run_id,
        "status":  "seeded",
        "message": "Provisional graph ready for Observer Review.",
        "graph":   _graph_to_response(world, scores),
    }


# ══════════════════════════════════════════════════════════════════════════════
# Observer Review Chamber
# ══════════════════════════════════════════════════════════════════════════════
...
def run_simulation(run_id: str, req: SimulateRequest = SimulateRequest()):
    ...
    for _ in range(req.steps):
        ...
        step_history.append({
            "step":               world.step,
            "global_instability": scores["global_instability"],
            "branch_potential":   scores["branch_details"]["global_branch_potential"],
            "lambda_temporal":    scores["lambda_temporal"],
            "field_stability":    scores["field_stability"],
        })

    final_scores = scores
    # Fix 9: persist step_history inside scores_json
    final_scores_with_history = {**final_scores, "step_history": step_history}
    ...
    with get_db() as conn:
        conn.execute(
            ...
            (str(uuid.uuid4()), run_id, "simulated",
             world_to_json(world),
             json.dumps(final_scores_with_history),
             row["edges_json"],
             now)
        )
        touch_run(conn, run_id, "simulated")
        record_audit(conn, run_id, "simulation_run", {
            "steps":             req.steps,
            "dt":                req.dt,
            "restart_from_reviewed": req.restart_from_reviewed,
            "final_instability": final_scores["global_instability"],
            "final_branch_potential":
                final_scores["branch_details"]["global_branch_potential"],
            "field_stability":   final_scores["field_stability"],
        })
    ...
    return {
        ...
        "field_stability":    final_scores["field_stability"],
        "global_instability": final_scores["global_instability"],
        "branch_details":     final_scores["branch_details"],
        "graph":              _graph_to_response(world, final_scores),
    }
...
def get_results(run_id: str):
    with get_db() as conn:
        row = _get_state_for_stage(conn, run_id, "simulated")
        if not row:
            raise HTTPException(404, "No simulation results yet.")
    world  = world_from_json(row["world_json"])
...
def get_audit(run_id: str):
    """
    Return the full immutable audit trail for a run.

    Every observation event, edit, evidence injection, and simulation is
    recorded here. This is a core UOT principle: the observer should be able
    to see the complete chain of decisions that produced any result.
    """
    with get_db() as conn:
        run = conn.execute("SELECT * FROM runs WHERE id=?", (run_id,)).fetchone()
        if not run:
...
def preview_sources(topic: str, observer_basis_json: str = "{}"):
    try:
        observer_basis = json.loads(observer_basis_json)
    except json.JSONDecodeError:
        observer_basis = {}

    queries = generate_search_queries(topic, observer_basis)
    packets = web_search_sources(topic, observer_basis)
        ...
        "sources": [
            {
                ...
                "recency":          p.recency,
                "summary":          p.summary,
                "causal_phrases":   p.causal_phrases[:3],
                "branch_phrases":   p.branch_phrases[:3],
                "evidence_snippets":p.evidence_snippets[:2],
            }
            for p in packets
        ],
    }
...
def serve_frontend():
    ...
    This avoids CORS issues and is the recommended single-container deployment pattern.
    The frontend calls /api/* paths which resolve same-origin.
    """
    frontend_path = Path(__file__).parent / "uot_frontend_v2.html"
    if not frontend_path.exists():
        return HTMLResponse("<h1>Frontend not found</h1><p>Place uot_frontend_v2.html beside uot_api_v2.py</p>", status_code=404)
