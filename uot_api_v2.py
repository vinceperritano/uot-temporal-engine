...
def _run_seed_pipeline(run_id: str) -> None:
    ...
    try:
        ...
        if demo_mode:
            ...
            try:
                # Phase 6: load locked canonical slots for temporal re-observations
                _locked_pq = None
                _locked_slots = None
                run_series_id = dict(run).get("series_id")
                if run_series_id:
                    _series_row = conn.execute(
                        "SELECT primary_question_json, canonical_slots_json FROM scenario_series WHERE series_id=?",
                        (run_series_id,)
                    ).fetchone()
                    if _series_row and _series_row["canonical_slots_json"]:
                        try:
                            _locked_pq    = json.loads(_series_row["primary_question_json"] or "{}")
                            _locked_slots = json.loads(_series_row["canonical_slots_json"] or "[]")
                            print(f"[API] Re-observation: {len(_locked_slots)} locked slots from series {run_series_id}")
                        except Exception:
                            pass
...
def seed_run(run_id: str, background_tasks: BackgroundTasks):
    ...
    with get_db() as conn:
        ...
        if not run:
            raise HTTPException(404, "Run not found")
        # Prevent double-tracking the same run
        existing_series_id = dict(run).get("series_id")
        if existing_series_id:
            existing = conn.execute(
                "SELECT series_id, title FROM scenario_series WHERE series_id=?",
                (existing_series_id,)
            ).fetchone()
            if existing:
                return {"series_id": existing_series_id, "run_id": run_id,
                        "status": "already_tracked",
                        "message": "This run is already part of a tracked scenario."}
        now = datetime.now(timezone.utc).isoformat()
...
def _extract_canonical_slots_json(art) -> str:
    """Extract canonical slots from run_artifacts as a JSON array of slot objects."""
    if not art or not art["primary_question_json"]:
        return "[]"
    try:
        pq = json.loads(art["primary_question_json"])
        slots = pq.get("canonical_outcome_slots", [])
        # If slots are just IDs (strings), wrap them in minimal objects
        # If they're already objects, pass through
        if slots and isinstance(slots[0], str):
            # Reconstruct from slot IDs — limited info but better than nothing
            return json.dumps([{"slot_id": s, "label": s, "slot_polarity": "yes", "slot_kind": "outcome"}
                               for s in slots])
...
def capture_slot_snapshots(conn, run_id: str, series_id: str, world=None, scores: dict = None):
    ...
    for ev in bg_events:
        ...
        slot_label= str(ev.get("label") or ev.get("branch_label") or slot_id)[:120]
        prob      = float(ev.get("probability", 0.5) or 0.5)
