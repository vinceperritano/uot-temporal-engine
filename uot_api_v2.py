...
def _check_rate_limit(conn, ip: str) -> tuple:
    ...
    if hourly >= MAX_RUNS_PER_IP_PER_HOUR:
        return False, (f"Rate limit: max {MAX_RUNS_PER_IP_PER_HOUR} live runs per hour. "
                       f"Please wait, or switch to Demo Mode.")
    if daily >= MAX_RUNS_PER_IP_PER_DAY:
        return False, (f"Rate limit: max {MAX_RUNS_PER_IP_PER_DAY} live runs per day. "
                       f"Please try again tomorrow, or switch to Demo Mode.")
    return True, ""
...
def create_run(req: CreateRunRequest, request: Request = None):
    ...
    if not req.demo_mode and LIVE_MODE:
        ...
        with get_db() as conn:
            allowed, msg = _check_rate_limit(conn, client_ip)
            if not allowed:
...
def run_simulation(run_id: str, req: SimulateRequest = SimulateRequest()):
    ...
    with get_db() as conn:
        ...
        # Retrieve PrimaryQuestion from engine module-level store
