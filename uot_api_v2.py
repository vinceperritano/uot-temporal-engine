...
def init_db() -> None:
    with get_db() as conn:
        ...
        CREATE TABLE IF NOT EXISTS slot_snapshots (
            ...
            resonance_eligible INTEGER DEFAULT 1,  -- 0 = exclude from resonance computation
            event_label       TEXT,
    ...
    if prev:
        ...
        for snap in conn.execute(
            """SELECT slot_id,slot_label,probability,temporal_energy,indeterminacy,
               snapshot_origin FROM slot_snapshots WHERE run_id=?""",
            (run_id,)
        ).fetchall():
            # Phase 7 (Vince's principle): skip carry-forward AND synthetic snapshots
            # when writing field_deltas. carry-forward delta=0.0 is not real movement.
            # synthetic delta reflects mapping failure, not real field dynamics.
            # Neither belongs in the historical delta record or resonance computation.
            _snap_dict = dict(snap)
            _snap_origin = _snap_dict.get("snapshot_origin") or ""
            _snap_synthetic = bool(_snap_dict.get("synthetic") or 0)
            if _snap_origin == "carry_forward" or _snap_synthetic:
                continue
            sid = snap["slot_id"]
        ...
        try:
            # Phase 7 (fixed): eligibility via synthetic and snapshot_origin columns,
            # not resonance_eligible (which has a DEFAULT 1 bug via CREATE TABLE).
            # Eligible = organic evidence (not synthetic, not carry-forward).
            _eligible_sids = {r["slot_id"] for r in conn.execute(
                """SELECT slot_id FROM slot_snapshots
                   WHERE run_id=?
            ...
            _coverage = round(len(_eligible_sids) / max(_total_slots, 1), 2)
            if _res_rows:
                ...
                try:
                    ...
                    conn.execute(
                        ...
                        (json.dumps({"score": round(_series_r, 3), "flag": _res_label,
                                     ...
                                     "total_slots": _total_slots}), run_id)
                    )
