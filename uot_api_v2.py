...
def run_simulation(run_id: str, req: SimulateRequest = SimulateRequest()):
    ...
    if req.use_iterative_loop:
        ...
        try:
            ...
            # Fall back to single-step simulation on loop failure
            import traceback
            print(f"[iterative loop error] {type(loop_err).__name__}: {loop_err}")
            traceback.print_exc()
    ...
    with get_db() as conn:
        save_world_state(conn, run_id, world, stage="simulated",
                         scores_json=scores_json,
                         step_history_json=step_history_json)
...
def get_audit_trail(run_id: str):
    ...
    with get_db() as conn:
        rows = conn.execute(
            "SELECT * FROM audit_log WHERE run_id = ? ORDER BY ts ASC",
            (run_id,)
        ).fetchall()
