...
def run_simulation(run_id: str, req: SimulateRequest = SimulateRequest()):
    ...
    with get_db() as conn:
        ...
        try:
            ...
            # Fallback: new columns may not exist in old DB (migration pending).
            # Preserve the essential fields; Phase 7.1 data will appear after
            # the DB migration runs on next restart.
            print(f"[Phase 7.1] upsert fallback (column missing?): {_upsert_err}")
            _upsert_run_artifact(conn, run_id,
                final_field_json=world_to_json(world),
                convergence_summary_json=json.dumps(convergence_summary),
                primary_question_json=json.dumps(pq) if pq else None)
