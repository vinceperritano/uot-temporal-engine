...
def _generate_navigation_interpretation(
    ...
    # Phase 10 threshold fix: raw pressure values regularly exceed 0.005 even when
    # branch comparison shifts are 0.0001 (essentially zero). The 0.005 threshold was
    # producing spurious "pressure rising" messages inconsistent with the nav panel
    # header showing "pressure flat". Raised to 0.02 — above typical shadow readings
    # (0.001-0.004 range) — so the message only fires when pressure is genuinely
    # meaningful, preparing for when SlotInfluence activates as a probability modifier.
    if slot_influence_pressure and abs(slot_influence_pressure) > 0.02:
