...
def _compute_slot_source_coherence(slot_id: str, representative, contributions: list,
    ...
    # Penalties (GPT guidance). Note: actor-scope-warning and threshold-bucket-
    # alternative-warning penalties are not yet implemented — that data isn't
    # currently persisted in a form this function can read without further
    # engine changes. Documented here rather than silently treated as zero.
    if family_weights:
...
def _compute_field_source_coherence(slot_source_coherence: dict) -> dict:
    ...
    _flag = ("high" if _mean >= 0.72 else
             "moderate" if _mean >= 0.55 else "low")
