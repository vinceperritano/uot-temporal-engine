...
def capture_slot_snapshots(conn, run_id: str, series_id: str, world, scores: dict):
    """
    Capture per-slot probabilities from the primary branch group into slot_snapshots.
    Uses branch_label as the stable slot identifier — simpler and more reliable than
    trying to match back to canonical slot IDs.
    """
    import uuid as _uuid
    now = datetime.now(timezone.utc).isoformat()

    # Get the primary branch group ID from run artifacts
    art = conn.execute(
        "SELECT primary_question_json FROM run_artifacts WHERE run_id=?", (run_id,)
    ).fetchone()
    bg_id = ""
    ...
    if hasattr(world, "nodes"):
        ...
        if isinstance(raw, dict):
            ...
            try:
                nodes = {n.id: n for n in raw.values()}
            except Exception:
                pass

    # Collect all nodes in the primary branch group (fallback: any branch group)
    bg_nodes = [n for n in nodes.values()
                if getattr(n, "branch_group", None) == bg_id] if bg_id else []
    if not bg_nodes:
        # Fallback: take any nodes that have a branch_group set
        bg_nodes = [n for n in nodes.values() if getattr(n, "branch_group", None)]

    if not bg_nodes:
        print(f"[Phase 6] No branch group nodes found for run {run_id}, skipping snapshot")
        return

    captured = 0
    for node in bg_nodes:
        nid = node.id
        slot_id    = (getattr(node, "branch_label", None) or getattr(node, "label", nid) or nid)
        slot_label = getattr(node, "label", slot_id)
        ...
        conn.execute(
            ...
            (str(_uuid.uuid4()), series_id, run_id,
             slot_id[:120], slot_label[:120],
             getattr(node, "outcome_role", "primary_outcome"),
             "outcome",
             float(getattr(node, "probability", 0.5) or 0.5),
             float(node_scores.get("temporal_energy",   0) or 0),
             float(node_scores.get("temporal_coherence",0) or 0),
             float(node_scores.get("temporal_entropy",  0) or 0),
             float(node_scores.get("indeterminacy",     0) or 0),
             nid, slot_label[:120],
    ...
    # Field deltas vs previous run in same series
    prev = conn.execute(
        "SELECT id FROM runs WHERE series_id=? AND id!=? ORDER BY run_index DESC LIMIT 1",
        (series_id, run_id)
    ).fetchone()
    if prev:
