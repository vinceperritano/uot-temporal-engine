...
def _run_seed_pipeline(run_id: str) -> None:
    ...
    Called by seed_run() after HTTP response is already returned.
    Prevents Railway 60-second timeout from killing live AI pipelines.
    Status: seeding → seeded (OK) or failed (error).
    """
    try:
        with get_db() as conn:
        ...
        with get_db() as conn:
            ...
            record_audit(conn, run_id, "pipeline_failed", {"error": str(exc)[:500]})
