...
def get_status(run_id: str):
    """Quick status check — returns current status and last error if failed."""
    with get_db() as conn:
        run = conn.execute("SELECT status FROM runs WHERE id=?", (run_id,)).fetchone()
        if not run:
            raise HTTPException(404, "Run not found")
        last_err = conn.execute(
            "SELECT details FROM audit_events WHERE run_id=? AND event_type='pipeline_failed' "
            "ORDER BY timestamp DESC LIMIT 1",
            (run_id,)
        ).fetchone()
    result = {"run_id": run_id, "status": run["status"]}
    if last_err:
        result["last_error"] = json.loads(last_err["details"])
    return result
...
def suggest_basis(req: BasisSuggestRequest):
    """
    Pre-seeding step: AI suggests 4-6 observer measurement basis dimensions
    calibrated to the topic and time horizon.

    Called from the landing page before run creation, so the user can review
    and adjust their measurement basis before the source field is collected.
    In UOT terms: this is the pre-observation calibration step where the
    observer chooses the axes along which the possibility field will become salient.
    """
    from uot_engine_v12_patched import HorizonConfig as HC, call_anthropic_api
    ...
    prompt = f"""Given this scenario topic and time horizon, suggest 4-6 observer measurement basis dimensions.

The measurement basis is the observer's interpretive frame: the dimensions along which the engine notices salience, uncertainty, causality, and temporal pressure. Each dimension is an axis through which the possibility field becomes meaningful.

Topic: {req.topic}
Horizon: {horizon_desc}, ending {target_date}
...
{{
  "basis": [
    {{
      ...
      "description": "One sentence: what this dimension measures about this specific scenario.",
      "weight": 0.75
    }}
  ]
}}

Return 4-6 dimensions. Keys must be unique snake_case. Weights between 0.3 and 0.95."""
    ...
    try:
        ...
        if not basis:
            raise ValueError("No valid basis dimensions returned")

        return BasisSuggestResponse(topic=req.topic, basis=basis)

    except Exception as e:
        # Fallback: return generic dimensions for the topic
        return BasisSuggestResponse(
            topic=req.topic,
            basis=[
                ...
                BasisDimension(key="escalation_risk",    label="Escalation risk",  description="Probability of events intensifying beyond current state.", weight=0.65),
