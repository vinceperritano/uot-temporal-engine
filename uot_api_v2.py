            ...
            try:
                ...
                for _sid_c in _all_slot_ids:
                    ...
                    _slot_source_coherence[_sid_c] = _compute_slot_source_coherence(
                        _sid_c, _rep, _contribs, _node_lookup)
                _field_coherence = _compute_field_source_coherence(_slot_source_coherence)
                ...
                print(f"[Phase 9.3] Source coherence: {_field_coherence['source_coherence']} "
                      ...
                      f"per-slot: {_slot_source_coherence}")
...
def classify_source_coherence(score) -> str:
    """
    Phase 9.8 (GPT guidance): shared helper for source_coherence_flag classification.
    Thresholds live in ONE place so resonance, navigation, and UI cannot drift apart.

    low:      < 0.42
    moderate: 0.42 – 0.62
    high:     >= 0.62  (deliberately strict — should be rare under current formula)
...
def _compute_slot_source_coherence(slot_id: str, representative, contributions: list,
    ...
    """
    if representative is None and not contributions:
        return None
    ...
    # weight changes. Avoids blind reweighting without data.
    if weight_total > 0:
        _n_items = _total_evidence_items
        # Collect component averages from representative + contributions
        _rc_vals, _sa_vals, _rc_vals_all, _sa_vals_all = [], [], [], []
        if representative is not None:
            _nd = node_lookup.get(representative.event_id)
            _rc_vals.append(getattr(_nd, 'record_coherence', None) if _nd else None)
            _sa_vals.append(getattr(_nd, 'source_agreement', None) if _nd else None)
        for _c2 in (contributions or []):
            _nd2 = node_lookup.get(_c2.event_id)
            _rc_vals.append(getattr(_nd2, 'record_coherence', None) if _nd2 else None)
            _sa_vals.append(getattr(_nd2, 'source_agreement', None) if _nd2 else None)
        _rc_mean = (sum(v for v in _rc_vals if v is not None) /
                    max(1, sum(1 for v in _rc_vals if v is not None))) if _rc_vals else None
        _sa_mean = (sum(v for v in _sa_vals if v is not None) /
                    max(1, sum(1 for v in _sa_vals if v is not None))) if _sa_vals else None
        _rel_conf = (getattr(representative, 'relation_confidence', None)
                     if representative else None)
        _eng_score = (getattr(representative, 'engine_score', None)
                      if representative else None)
        _penalties = round(weighted_sum / weight_total - final, 4) if weight_total > 0 else 0
        print(f"[Phase 9.8] Slot coherence components for '{slot_id}': "
              f"record_coherence={_rc_mean:.3f if _rc_mean is not None else 'None'} "
              f"source_agreement={_sa_mean:.3f if _sa_mean is not None else 'None'} "
              f"relation_confidence={_rel_conf:.3f if _rel_conf is not None else 'None'} "
              f"engine_score={_eng_score:.3f if _eng_score is not None else 'None'} "
              f"penalties={_penalties:.3f} "
              f"final={final:.4f}")

    return final


def _compute_field_source_coherence(slot_source_coherence: dict) -> dict:
    """
    Phase 9.3 (GPT guidance): field-level source coherence — the mean of
    per-slot coherence over EVIDENCE-BEARING slots only. A slot with no
    evidence at all is excluded from this mean (that's a coverage problem,
    not a coherence problem) rather than counted as zero.

    Returns {"source_coherence": float|None, "source_coherence_flag": str}
    """
    _bearing = [v for v in slot_source_coherence.values() if v is not None]
    if not _bearing:
        return {"source_coherence": None, "source_coherence_flag": "unknown"}
    _mean = sum(_bearing) / len(_bearing)
    # Bugfix (found via testing — China-Taiwan showed resonance score 0.997,
    # essentially perfect historical stability, organic_rate=1.00, yet still
    # labeled "low" coherence under the original 0.55/0.72 thresholds).
    # Traced the cause: event_evidence_coherence is 80% built from
    # record_coherence + source_agreement, and source_agreement is computed
    # from per-source stance/relevance — but the Phase 8.9 Stage A sources
    # instruction only ever asks for title/publisher, never stance or
    # relevance, so every cited source silently inherits the SourceRef
    # dataclass defaults (stance="neutral", relevance=0.5). This mechanically
    # caps source_agreement at a flat ~0.6 regardless of how strong the
    # evidence actually is. Combined with engine_score/relation_confidence
    # being systematically below their own gate thresholds for
    # weak_semantic_match-rescued events (the MOST COMMON mapping path across
    # this engine, by definition of what that rescue path means), the
    # formula's natural baseline for completely healthy, clean evidence sits
    # close to 0.55-0.60 — meaning "low" was firing on essentially everything,
    # not selectively on genuinely weak evidence.
    #
    # This is a conservative interim recalibration based on observed real
    # data (clean runs scoring 0.40-0.50 under the old formula) rather than a
    # fix to the formula/weights themselves, which would need more data to
    # get right. Flagged for GPT's review since they specified the original
    # 0.55/0.72 cutoffs — the better long-term fix is likely having Stage A
    # also classify stance/relevance per cited source, which would let
    # source_agreement actually differentiate instead of clustering at ~0.6.
    # Phase 9.8: use the shared helper — single source of truth for thresholds.
    _flag = classify_source_coherence(_mean)
    return {"source_coherence": round(_mean, 4), "source_coherence_flag": _flag}
...
def get_series_timeline(series_id: str):
    ...
    try:
        ...
        for _sid_key, _hist in _slot_history.items():
            ...
            _slot_coh_latest = _slot_coherence_by_run.get(_latest_run_id, {}).get(_sid_key)
                ...
                try:
                    ...
                    conn.execute(
                        ...
                        (json.dumps({"score": round(_series_r, 3), "flag": _res_label,
                                     ...
                                     "source_coherence_flag": _src_coherence_flag}), run_id)
                    )
