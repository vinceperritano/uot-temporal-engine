...
def run_simulation(run_id: str, req: SimulateRequest = SimulateRequest()):
    ...
    with get_db() as conn:
        ...
        if run_row and run_row["series_id"]:
            ...
            # Phase 10 bugfix: cross-run representative variance diagnostic was
            # previously wired into create_series (initial observation only —
            # runs once per scenario, before any history exists to compare
            # against). This is the actual simulate endpoint that runs on every
            # re-observation, which is where the diagnostic needs to be to ever
            # fire in practice.
            try:
                _curr_slot_evs = {}
                if world and hasattr(world, 'event_graph'):
                    for _n in world.event_graph.nodes.values():
                        _sid = getattr(_n, 'canonical_slot_id', None)
                        _bg = getattr(_n, 'branch_group', None)
                        if not _sid or not _bg:
                            continue
                        _is_primary = ('primary' in str(_bg).lower()
                                       or str(_bg) == getattr(
                                           world, 'primary_branch_group_id', None))
                        if _is_primary and _sid not in _curr_slot_evs:
                            _curr_slot_evs[_sid] = {
                                "label": getattr(_n, 'label', ''),
                                "probability": float(getattr(_n, 'probability', 0.0) or 0.0),
                                "event_kind": getattr(_n, 'event_kind', ''),
                            }
                if _curr_slot_evs:
                    _sc_map = {}
                    try:
                        _pq_for_coh = dict(_eng._LAST_PRIMARY_QUESTION) if _eng._LAST_PRIMARY_QUESTION else {}
                        _sc_for_diag = (_pq_for_coh.get('seed_quality', {})
                                        .get('slot_source_coherence', {}))
                        if _sc_for_diag:
                            _sc_map = {k: float(v) for k, v in _sc_for_diag.items()
                                       if v is not None}
                    except Exception:
                        pass
                    _compute_cross_run_representative_variance(
                        conn, run_row["series_id"], run_id, _curr_slot_evs, _sc_map)
            except Exception as _cre:
                print(f"[Phase 10] Cross-run diagnostic error: {_cre}")
...
# ══════════════════════════════════════════════════════════════════════════════
# Phase 9.12: Semantic Bridge Rescue diagnostic endpoint
# Returns plain text readable in any browser — no developer tools needed.
# Remove this endpoint once the rescue has been confirmed in a live production run.
@app.get("/api/debug/test-bridge-rescue")
def test_semantic_bridge_rescue():
    """Diagnostic endpoint: tests the Semantic Bridge Rescue path using a known
    historical failure pattern (North Korea incremental_development slot,
    engine_score≈0.27, rel_conf≈0.68). Returns plain text for easy mobile reading."""
    from fastapi.responses import PlainTextResponse
    import sys
    sys.path.insert(0, '/app')
    try:
        import uot_engine_v12_patched as eng
        from types import SimpleNamespace

        mock_slot = SimpleNamespace(
            slot_id='steady_incremental_development',
            label='Steady incremental development at historical pace',
            description='North Korea continues weapons program at established pace with routine tests',
            slot_polarity='alternative', slot_kind='outcome',
            aliases=['continued development', 'steady pace', 'incremental testing'],
            evidence_terms=['missile test', 'weapons development', 'testing cadence',
                            'incremental capability', 'routine launch'],
            action_exemplars=[
                'North Korea conducts periodic ballistic missile test at established pace',
                'Kim Jong Un inspects weapons facility, signals continued program activity',
            ],
            negative_examples=['North Korea announces testing moratorium'],
            directionality='continuation', actor_scope='North Korea',
            canonical_slot_hint_score=0.0,
        )
        mock_ev = {
            'id': 'TEST_EVT_001',
            'label': 'North Korea Maintains Active Multi-System Weapons Development Program',
            'description': 'DPRK continues nuclear and missile development at established cadence',
            'probability': 0.10, 'temporal_status': 'unresolved',
            'slot_affinities': {'steady_incremental_development': 0.32},
            'metadata': {'event_kind': 'substantive_development',
                         'directionality': 'continuation', 'horizon_fit': 'inside'},
        }
        rationale = ("North Korea's missile testing program continues at a steady pace, "
                     "consistent with incremental capability development.")

        components = eng.compute_topic_relevance_components(mock_slot, mock_ev, rationale)
        bridge_signals = {
            'rationale_overlap': components.get('rat_sim', 0.0) > 0.0,
            'evidence_term_hit': components.get('evidence_term_hit', 0.0) > 0.0,
            'exemplar_hit': components.get('exemplar_hit', 0.0) > 0.0,
            'affinity_ge_0.30': components.get('affinity', 0.0) >= 0.30,
            'directionality_match': components.get('directionality_match', 0.0) == 1.0,
        }
        bridge_count = sum(1 for v in bridge_signals.values() if v)
        rel_conf, scope_compat = 0.68, 0.80
        engine_score = components['final_score']
        would_rescue = (rel_conf >= 0.65 and scope_compat >= 0.60
                        and engine_score >= 0.22 and bridge_count >= 1)
        lines = [
            "=== Semantic Bridge Rescue Diagnostic ===",
            "",
            "Test case: North Korea incremental_development",
            f"  Event: {mock_ev['label']}",
            f"  Simulated rel_conf: {rel_conf}  (historical: ~0.68)",
            f"  Simulated engine_score: {engine_score:.4f}  (historical: ~0.27)",
            "",
            "Scoring components:",
        ]
        for k, v in components.items():
            lines.append(f"  {k}: {v}")
        lines += ["", "Bridge signals:"]
        for name, active in bridge_signals.items():
            lines.append(f"  {'ACTIVE' if active else 'inactive'}: {name}")
        lines += [
            "",
            f"Bridge signals active: {bridge_count}/5",
            f"Standard rescue would fire: {'YES — PASS' if would_rescue else 'NO — FAIL'}",
            "",
            ("PASS: Semantic Bridge Rescue correctly identifies this mapping. "
             "In a live Gate B run, this would log: "
             f"[Phase 9.11] Semantic Bridge Rescue: TEST_EVT_001 -> 'steady_incremental_development' "
             f"bridges={bridge_count}"
             if would_rescue else
             "FAIL: rescue would not fire. Check bridge signals or engine score."),
        ]

        # Phase 10 (GPT guidance): partial-signal tests to confirm rescue fires
        # at 1-signal and 2-signal thresholds with genuinely sparse engine scores.
        lines += ["", "=" * 40,
                  "PARTIAL-SIGNAL TEST A: 1 bridge signal, engine_score ~0.26",
                  "(sparse vocab, only affinity fires — typical hard rejection case)"]
        mock_ev_sparse = dict(mock_ev)
        mock_ev_sparse['label'] = 'DPRK Weapons Activity Continues'
        mock_ev_sparse['slot_affinities'] = {'steady_incremental_development': 0.35}
        mock_ev_sparse['metadata'] = {'event_kind': 'substantive_development',
                                       'horizon_fit': 'inside'}
        comps_a = eng.compute_topic_relevance_components(mock_slot, mock_ev_sparse, "")
        sigs_a = [comps_a.get('rat_sim',0)>0, comps_a.get('evidence_term_hit',0)>0,
                  comps_a.get('exemplar_hit',0)>0, comps_a.get('affinity',0)>=0.30,
                  comps_a.get('directionality_match',0)==1.0]
        cnt_a = sum(1 for b in sigs_a if b)
        rescues_a = (0.68>=0.65 and 0.80>=0.60 and comps_a['final_score']>=0.22 and cnt_a>=1)
        lines += [f"  engine_score={comps_a['final_score']:.4f}, bridges={cnt_a}/5",
                  f"  Rescue fires: {'YES — PASS' if rescues_a else 'NO — investigate'}"]

        lines += ["", "PARTIAL-SIGNAL TEST B: 2 bridge signals, rel_conf=0.62 (strict variant)",
                  "(borderline rel_conf, needs engine>=0.28 and 2+ signals)"]
        mock_ev_border = dict(mock_ev)
        mock_ev_border['label'] = 'North Korea Incremental Weapons Cadence Steady'
        mock_ev_border['slot_affinities'] = {'steady_incremental_development': 0.31}
        mock_ev_border['metadata'] = {'event_kind': 'substantive_development',
                                       'directionality': 'continuation', 'horizon_fit': 'inside'}
        comps_b = eng.compute_topic_relevance_components(mock_slot, mock_ev_border, "")
        sigs_b = [comps_b.get('rat_sim',0)>0, comps_b.get('evidence_term_hit',0)>0,
                  comps_b.get('exemplar_hit',0)>0, comps_b.get('affinity',0)>=0.30,
                  comps_b.get('directionality_match',0)==1.0]
        cnt_b = sum(1 for b in sigs_b if b)
        rescues_b_strict = (0.60<=0.62<0.65 and 0.80>=0.60 and comps_b['final_score']>=0.28 and cnt_b>=2)
        rescues_b_std = (0.62>=0.65 and comps_b['final_score']>=0.22 and cnt_b>=1)
        lines += [f"  engine_score={comps_b['final_score']:.4f}, bridges={cnt_b}/5",
                  f"  Strict rescue (rel_conf 0.60-0.65, need score>=0.28, 2+ signals): "
                  f"{'YES — PASS' if rescues_b_strict else 'NO (score or bridge threshold not met)'}",
                  f"  Standard rescue (rel_conf>=0.65): not applicable (rel_conf=0.62)"]
        return PlainTextResponse("\n".join(lines))
    except Exception as e:
        return PlainTextResponse(f"Test error: {e}\n\nThis endpoint requires the engine module on the same server.")
    ...
    with get_db() as conn:
        ...
        try:
            ...
            if worlds:
                ...
                # Phase 10 bugfix: cross-run representative variance diagnostic was
                # incorrectly placed here originally. create_series only runs once
                # per scenario at the very first observation, when no prior history
                # exists to compare against — so the diagnostic could never fire from
                # this location. Moved to run_simulation (the /simulate endpoint),
                # which runs on every re-observation and is where cross-run history
...
def _generate_navigation_interpretation(
    probability: float,
    ...
    n_stable: int,
    resonance_flag: str,
    slot_influence_pressure: float = 0.0,
) -> str:
    """
    Phase 10 — Temporal Navigation Interpreter (display-only).
    GPT guidance: explains the temporal field in human-readable terms.
    Rule-based, no model call. Combines probability position, trajectory,
    coherence, confidence, and influence pressure into a brief interpretation.
    """
    prob_pct = round(probability * 100, 1)
    base_pct = round(baseline_probability * 100, 1) if baseline_probability else None
    delta_pct = round((probability - (baseline_probability or probability)) * 100, 1)
    coh = float(slot_coherence) if slot_coherence is not None else None
    traj = (trajectory_label or "").lower()
    conf = (trajectory_confidence or "").lower()

    # Opening: probability position
    if probability >= 0.60:
        position = f"This outcome is high probability ({prob_pct}%)"
    elif probability >= 0.25:
        position = f"This outcome is plausible ({prob_pct}%)"
    elif probability >= 0.10:
        position = f"This outcome is unlikely ({prob_pct}%)"
    else:
        position = f"This outcome is a tail risk ({prob_pct}%)"

    # Trajectory movement
    if traj == "stable":
        if conf == "high":
            movement = "and stable across reliable observations"
        else:
            movement = "and currently stable"
    elif traj == "approaching":
        movement = f"and strengthening (up {abs(delta_pct):.1f}% from baseline)" if delta_pct > 0 else "and approaching"
    elif traj == "retreating":
        movement = f"and retreating (down {abs(delta_pct):.1f}% from baseline)" if delta_pct < 0 else "and retreating"
    elif traj == "accelerating":
        movement = "and accelerating — momentum is building"
    elif traj == "decelerating":
        movement = "and decelerating — momentum is fading"
    elif traj in ("oscillating", "noisy"):
        movement = "with oscillating probability — no stable trend established"
    else:
        movement = ""

    # Coherence quality
    if coherence_flag == "high":
        coherence_str = "Evidence coherence is high — sources consistently support this reading."
    elif coherence_flag == "moderate":
        coherence_str = "Evidence coherence is moderate — sources broadly agree but with some uncertainty."
    elif coherence_flag == "low":
        coherence_str = ("Evidence coherence is limited (source coherence "
                         f"{coh:.2f}) — probability readings may shift as "
                         "different sources are found.")
    else:
        coherence_str = "Evidence coherence is unknown — insufficient data."

    # Confidence and observation depth
    if conf == "high" and n_stable >= 4:
        confidence_str = f"Confidence is high across {n_stable} stable observations."
    elif conf == "moderate":
        confidence_str = f"Confidence is moderate ({n_stable} stable observations)."
    elif conf in ("low", "provisional"):
        # Phase 10 fix: when coherence_flag is also "low", the recommendation already
        # addresses trust — suppress the confidence_str to avoid two redundant warnings.
        if coherence_flag == "low":
            confidence_str = ""
        else:
            confidence_str = ("Limited observational confidence — "
                              "more observations needed before trusting this trajectory.")
    else:
        confidence_str = ""

    # Influence pressure note (when meaningful)
    # Phase 10: suppress SlotInfluence pressure message until SlotInfluence is
    # activated as a probability modifier. The raw pressure value operates in a
    # different scale from branch comparison shifts, causing spurious "rising"
    # messages even when the nav panel header correctly shows "pressure flat."
    # When SlotInfluence activates, the pressure_velocity will be the reliable
    # signal — the interpreter will be updated to use it at that point.
    # For now: no pressure message regardless of raw pressure magnitude.
    if False and slot_influence_pressure and abs(slot_influence_pressure) > 0.02:
        if slot_influence_pressure > 0:
            influence_str = "SlotInfluence pressure is rising."
        else:
            influence_str = "SlotInfluence pressure is falling."
    else:
        influence_str = ""

    # Recommendation
    if conf == "high" and traj == "stable" and coherence_flag in ("high", "moderate"):
        recommendation = "Treat as a settled branch unless major new evidence emerges."
    elif traj in ("approaching", "accelerating") and conf in ("high", "moderate"):
        recommendation = "Watch for continued strengthening — this outcome is gaining ground."
    elif traj in ("retreating", "decelerating") and conf in ("high", "moderate"):
        recommendation = "Reversal factors would be needed to change this trajectory."
    elif traj in ("oscillating", "noisy") or conf == "provisional":
        recommendation = "Monitor for stabilization before treating this as a reliable signal."
    elif coherence_flag == "low":
        recommendation = "Evidence coherence is too low to trust this trajectory reading."
    else:
        recommendation = "Continue observing."

    parts = [f"{position}{', ' + movement if movement else '.'}."]
    parts.append(coherence_str)
    if confidence_str:
        parts.append(confidence_str)
    if influence_str:
        parts.append(influence_str)
...
def _compute_navigation_diagnostics(stable_history: list, pressure_history: list,
    ...
    if (_cov_for_conf >= 0.80 and quality_flag == "green" and _n_stable >= 4
        ...
        trajectory_reason = (f"{_n_stable} stable observations, {_cov_for_conf:.0%} coverage, "
              ...
              and _n_stable >= 2 and _coh_for_conf >= 0.55):
        # Phase 10 (threshold alignment): raised from 0.42 → 0.55 to match the
        # source coherence moderate threshold. Previously a slot with coherence 0.52
        # ("low" by source coherence standards) could still earn "moderate" trajectory
        # confidence, causing the Navigator Interpreter to say "too low to trust" while
        # the header displayed "moderate" — a direct contradiction. Now both systems
        # agree: coherence below 0.55 = low trust in both source coherence and trajectory.
    ...
    return {
        ...
        "navigation_interpretation": _generate_navigation_interpretation(
            probability=float(stable_history[-1].get("probability", 0.5) if stable_history else 0.5),
            # Phase 10 fix (GPT guidance): use resonant_baseline (the carefully computed
            # stable average) rather than stable_history[0] (first observation, which may
            # predate many revisions). Fall back to first observation only if unavailable.
            baseline_probability=float(resonant_baseline) if resonant_baseline is not None
                                 else float(stable_history[0].get("probability", 0.5)
                                            if len(stable_history) > 1 else 0.5),
            trajectory_label=trajectory_label,
            trajectory_confidence=trajectory_confidence,
            slot_coherence=slot_coherence,
            coherence_flag=classify_source_coherence(slot_coherence) if slot_coherence is not None else "unknown",
            velocity=velocity,
            n_stable=len(stable_history),
            resonance_flag=resonance_flag or "",
            # Phase 10 fix (GPT guidance): pass computed pressure so interpreter can
            # describe rising/falling SlotInfluence in its output text.
            slot_influence_pressure=float(pressure) if pressure is not None else 0.0,
        ),
    }
...
def _compute_cross_run_representative_variance(
        conn, series_id: str, run_id: str, current_slot_events: dict,
        slot_source_coherence: dict = None, n_prior: int = 4):
    """
    Phase 10 (GPT guidance): Cross-run Representative Variance Diagnostic.

    For each organic slot in the current run, compare the representative event's
    raw probability against the median representative probability from the last
    n_prior organic runs. Flags when the current representative is an outlier
    relative to the historical distribution — a signal that field movement may
    reflect representative selection rather than a true change in the field.

    current_slot_events: {slot_id: {"label": str, "probability": float, "event_kind": str}}
    """
    if not current_slot_events:
        return

    prior_runs = conn.execute(
        """SELECT DISTINCT run_id FROM slot_snapshots
           WHERE series_id=? AND run_id!=?
             AND COALESCE(synthetic, 0)=0
             AND COALESCE(snapshot_origin, 'organic') NOT IN ('carry_forward', 'synthetic')
           ORDER BY created_at DESC LIMIT ?""",
        (series_id, run_id, n_prior)
    ).fetchall()
    if not prior_runs:
        return
    ...
    for slot_id, curr in current_slot_events.items():
        curr_prob = curr.get("probability")
        if curr_prob is None:
            continue

        # Get prior organic probabilities for this slot
        prior_rows = conn.execute(
            f"""SELECT probability FROM slot_snapshots
                WHERE series_id=? AND slot_id=?
                  AND run_id IN ({placeholders})
                  AND COALESCE(synthetic, 0)=0
                  AND COALESCE(snapshot_origin, 'organic') NOT IN ('carry_forward', 'synthetic')
                ORDER BY created_at DESC""",
            (series_id, slot_id, *prior_run_ids)
        ).fetchall()

        if len(prior_rows) < 2:
            continue

        priors = [float(r["probability"]) for r in prior_rows if r["probability"] is not None]
        if not priors:
            continue

        priors_sorted = sorted(priors)
        mid = len(priors_sorted) // 2
        prior_median = (priors_sorted[mid] if len(priors_sorted) % 2 == 1
                        else (priors_sorted[mid - 1] + priors_sorted[mid]) / 2)
        delta = float(curr_prob) - prior_median

        # Check coherence flag
        coh = slot_source_coherence.get(slot_id) if slot_source_coherence else None
        _coherence_was_assumed = coh is None
        coh_flag = classify_source_coherence(coh) if coh is not None else "moderate"
        # Phase 10 bugfix: slot_coherence is not stored in slot_snapshots, so the
        # original coherence lookup was silently failing and coh_ok was always False,
        # preventing the diagnostic from ever firing. Simplified: use the coherence
        # passed from the in-memory slot_source_coherence dict (if available), or
        # treat as moderate (fires for plausibly evidenced slots, skips unknown).
        coh_ok = coh_flag in ("high", "moderate")

        if abs(delta) >= 0.10 and coh_ok:
            # Phase 10 refinement (GPT guidance): log explicitly when coherence was
            # defaulted to moderate (not grounded in actual data), so a fully-grounded
            # flag can be distinguished from a "diagnostic should not be silent" flag.
            _coh_note = "coherence_assumed=moderate" if _coherence_was_assumed else f"coherence={coh_flag}"
            print(f"[Phase 10] ⚠ Cross-run representative variance risk for '{slot_id}': "
                  f"current={curr_prob:.3f} vs prior median={prior_median:.3f} "
                  f"(delta={delta:+.3f}, {len(priors)} prior run(s), {_coh_note}). "
                  f"Current representative: '{curr.get('label', 'unknown')[:80]}'. "
                  f"Prior representative probabilities: {[round(p, 3) for p in priors]}. "
                  f"Movement may reflect representative selection rather than true field shift.")
...
def capture_slot_snapshots(conn, run_id: str, series_id: str, world=None, scores: dict = None):
    ...
    # Phase 10 bugfix: if the immediately prior run has run_index=NULL (e.g. created
    # via a manual-override-then-resimulate flow that didn't assign one), the query
    # above silently skips it and selects an OLDER run instead — producing a "Recent
    # Field Delta" that actually spans multiple observations, not one. Detected via
    # the AI Regulation divergence slot showing +16.5% when the true single-run
    # delta was -1.7% (the selected "prev" was 2 runs back, not the immediate one).
    # Fallback: if any run exists with created_at more recent than the selected prev
    # (i.e. there's a more recent run we skipped), prefer it via created_at ordering.
    try:
        _created_at_prev = conn.execute(
        ...
        if _created_at_prev and (not prev or dict(_created_at_prev)["id"] != prev["id"]):
            print(f"[Phase 10] ⚠ field_deltas prev-run mismatch: run_index-based selection "
                  f"chose {prev['id'] if prev else None}, but created_at-based selection "
                  f"would choose {dict(_created_at_prev)['id']} (more recent). Using "
                  f"created_at-based selection to avoid comparing against a stale run.")
            prev = _created_at_prev
    except Exception as _pe:
        print(f"[Phase 10] prev-run fallback check error: {_pe}")
