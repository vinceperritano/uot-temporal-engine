    ...
    if prev:
        ...
        try:
            _res_rows = conn.execute(
                """SELECT resonance_score, ABS(probability_delta) as weight
            ...
            if _res_rows:
                ...
                try:
                    conn.execute(
                        ...
                        (json.dumps({"score": round(_series_r, 3), "flag": _flag,
                                     "slot_count": len(_res_rows)}), run_id)
                    )
                    print(f"[Phase 7.4] Series resonance: {_flag} ({_series_r:.3f}) "
                          f"from {len(_res_rows)} slot(s)")
