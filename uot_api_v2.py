...
def _compute_slot_source_coherence(slot_id: str, representative, contributions: list,
    ...
    if weight_total > 0:
        ...
        def _fmt(v): return f"{v:.3f}" if v is not None else "None"
        print(f"[Phase 9.8] Slot coherence components for '{slot_id}': "
              f"record_coherence={_fmt(_rc_mean)} "
              f"source_agreement={_fmt(_sa_mean)} "
              f"relation_confidence={_fmt(_rel_conf)} "
              f"engine_score={_fmt(_eng_score)} "
