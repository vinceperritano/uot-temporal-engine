        ...
        try:
            ...
            if worlds:
                ...
                try:
                    ...
                    if _curr_slot_evs:
                        _sc_lookup = conn.execute(
                            """SELECT slot_id, slot_coherence FROM slot_snapshots
                               WHERE run_id=? AND slot_coherence IS NOT NULL""",
                            (run_id,)
                        ).fetchall()
                        _sc_map = {r["slot_id"]: float(r["slot_coherence"])
                                   for r in _sc_lookup if r["slot_coherence"]}
...
def _compute_cross_run_representative_variance(
    ...
    for slot_id, curr in current_slot_events.items():
        ...
        coh_flag = classify_source_coherence(coh) if coh is not None else "unknown"
