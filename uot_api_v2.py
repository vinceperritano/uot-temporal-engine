...
def _compute_navigation_diagnostics(stable_history: list, pressure_history: list,
    ...
    #
    # Bugfix (per testing feedback): source_continuity and resonance_flag are
    # RUN-LEVEL signals (computed once for the whole observation), but were
    # being applied uniformly to every slot's navigation label — meaning a
    # slot with strong, stable evidence got marked noisy just because OTHER
    # slots' evidence churned that run. A slot's own slot_evidence_coverage
    # (strong/moderate) now partially overrides the blanket run-level signal:
    # a well-evidenced slot is not silenced by a noisy run elsewhere in the field.
    _coverage_override = slot_evidence_coverage in ("strong", "moderate")
    is_noisy = (
        len(stable_history) < 2
        or (not _coverage_override and (
            source_continuity == "low"
    ...
    if is_noisy:
        ...
        elif abs(velocity) < 0.015 and source_continuity in ("high", "moderate") and resonance_flag == "high":
            trajectory_label = "stable"
        elif acceleration is not None and velocity > 0.01 and acceleration > 0.005 and source_continuity != "low":
        ...
        if abs(velocity) < 0.015 and source_continuity in ("high", "moderate") and resonance_flag == "high":
            trajectory_label = "stable"
        elif velocity > 0.01:
            trajectory_label = "approaching"
        elif velocity < -0.01:
...
def get_series_timeline(series_id: str):
    ...
    for _s in snapshots:
        ...
        _is_stable = (
            ...
            and _res.get("source_continuity") != "low"
        ...
        try:
            ...
            if _res_rows:
                ...
                try:
                    ...
                    # Phase 8.4: low source continuity also marks resonance provisional —
                    # the field movement may be source-sample variance, not temporal movement.
                    if _src_continuity == "low" and _res_label not in ("strained", "provisional"):
