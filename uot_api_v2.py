...
def _compute_field_source_coherence(slot_source_coherence: dict) -> dict:
    ...
    _flag = ("high" if _mean >= 0.62 else
             "moderate" if _mean >= 0.42 else "low")
...
def _compute_navigation_diagnostics(stable_history: list, pressure_history: list,
    ...
    # Phase 9.4 (GPT guidance): trajectory_confidence — separate from the
    # trajectory_label itself, telling the observer how much to trust the
    # reading, using coverage + slot_coherence + quality_flag + history depth.
    ...
    if (_cov_for_conf >= 0.80 and _coh_for_conf >= 0.72
            and quality_flag == "green" and _n_stable >= 3):
    ...
    elif (_cov_for_conf >= 0.60 and _coh_for_conf >= 0.55
              and quality_flag != "red" and _n_stable >= 2):
