...
def run_simulation(run_id: str, req: SimulateRequest = SimulateRequest()):
    ...
    with get_db() as conn:
        ...
        # Phase 6: capture slot snapshots if this run belongs to a series
        run_row = conn.execute("SELECT series_id FROM runs WHERE id=?", (run_id,)).fetchone()
        if run_row and run_row["series_id"]:
            try:
                capture_slot_snapshots(conn, run_id, run_row["series_id"], world, scores)
...
def capture_slot_snapshots(conn, run_id: str, series_id: str, world, scores: dict):
    """
    After simulation, extract per-slot probabilities from the world state and
    store them in slot_snapshots for longitudinal tracking.
    Also computes field_deltas vs the previous run in the same series.
    """
    from datetime import datetime, timezone
    import uuid as _uuid
    now = datetime.now(timezone.utc).isoformat()

    # Get primary_question_json to find canonical slots
    art = conn.execute(
        "SELECT primary_question_json FROM run_artifacts WHERE run_id=?", (run_id,)
    ).fetchone()
    if not art or not art["primary_question_json"]:
        return  # No canonical slots to snapshot

    try:
        pq = json.loads(art["primary_question_json"])
        slot_ids = pq.get("canonical_outcome_slots", [])
        bg_id    = pq.get("primary_branch_group_id", "")
    except Exception:
        return

    if not slot_ids or not bg_id:
        return

    # Build node lookup from world
    nodes = {}
    if hasattr(world, "nodes"):
        nodes = world.nodes if isinstance(world.nodes, dict) else {n.id: n for n in world.nodes.values()}

    # Find nodes in primary branch group
    bg_nodes = {nid: n for nid, n in nodes.items()
                if getattr(n, "branch_group", None) == bg_id}

    # For each canonical slot, find the matching node
    for node in bg_nodes.values():
        nid = node.id
        slot_id = getattr(node, "branch_label", nid)
        # Try to match to a canonical slot ID
        matched_slot = None
        for s in slot_ids:
            if s in nid.lower() or s in slot_id.lower().replace(" ", "_"):
                matched_slot = s
                break
        if not matched_slot:
            matched_slot = nid  # use node ID as fallback

        node_scores = scores.get(nid, {})
        conn.execute(
            """INSERT OR REPLACE INTO slot_snapshots
               ...
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (str(_uuid.uuid4()), series_id, run_id, matched_slot,
             getattr(node, "branch_label", None) or getattr(node, "label", ""),
             getattr(node, "outcome_role", ""),
             "outcome",
             float(getattr(node, "probability", 0.5) or 0.5),
             float(node_scores.get("temporal_energy", 0)),
             float(node_scores.get("temporal_coherence", 0)),
             float(node_scores.get("temporal_entropy", 0)),
             float(node_scores.get("indeterminacy", 0)),
             nid,
             getattr(node, "label", ""),
             1 if nid.startswith("EVT_SYNTH") else 0,
             getattr(node, "temporal_status", "unresolved"),
             float(node_scores.get("record_coherence", 0.5) or 0.5),
             now)
        )

    # Compute field deltas vs previous run in series
    prev = conn.execute(
        """SELECT run_id FROM runs WHERE series_id=? AND id!=?
    ...
    if prev:
        ...
        prev_map = {r["slot_id"]: r for r in prev_snaps}
        curr_snaps = conn.execute(
            "SELECT slot_id, probability, temporal_energy, indeterminacy FROM slot_snapshots WHERE run_id=?",
            (run_id,)
        ).fetchall()
        for snap in curr_snaps:
            ...
            if sid in prev_map:
                p = prev_map[sid]
                p_delta = float(snap["probability"] or 0) - float(p["probability"] or 0)
                e_delta = float(snap["temporal_energy"] or 0) - float(p["temporal_energy"] or 0)
                i_delta = float(snap["indeterminacy"] or 0) - float(p["indeterminacy"] or 0)
                conn.execute(
                    """INSERT OR REPLACE INTO field_deltas
                       (id, series_id, from_run_id, to_run_id, slot_id,
                        probability_delta, energy_delta, indeterminacy_delta, summary, created_at)
                       VALUES (?,?,?,?,?,?,?,?,?,?)""",
                    (str(_uuid.uuid4()), series_id, prev["run_id"], run_id, sid,
                     p_delta, e_delta, i_delta,
                     f"Δp={p_delta:+.3f} Δe={e_delta:+.3f} Δi={i_delta:+.3f}", now)
                )
