...
def capture_slot_snapshots(conn, run_id: str, series_id: str, world=None, scores: dict = None):
    ...
    if prev:
        ...
        for snap in conn.execute(
            ...
            # Phase 7 (Vince's principle): skip carry-forward snapshots entirely —
            # don't write field_deltas for them, so the historical record stays clean.
            # delta=0.0 for an unmapped slot is not real movement and should not
            # appear in Recent Field Deltas or contribute to resonance history.
            _snap_origin = dict(snap).get("snapshot_origin") or ""
            if _snap_origin == "carry_forward":
                continue
            sid = snap["slot_id"]
        ...
        try:
            # Phase 7.4: only include resonance-eligible slots in series resonance.
            # Carry-forward (delta=0.0, would compute score≈1.0) and synthetic
            # slots are excluded — they inflate resonance without representing
            # genuine field movement.
            _eligible_sids = {r["slot_id"] for r in conn.execute(
                "SELECT slot_id FROM slot_snapshots WHERE run_id=? AND resonance_eligible=1",
                (run_id,)
            ).fetchall()}
