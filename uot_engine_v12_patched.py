...
def extract_candidate_events_holistic(topic, source_summaries, observer_basis,
    ...
    tc_prefix = (f"TEMPORAL CONTEXT: {temporal_context}\n"
                 ...
                 "'beyond 2030')\n"
                 "  horizon_fit: 'inside' if the event's expected resolution lies "
                 "within the scenario horizon; 'partial' if it covers a shorter window "
                 "than the full horizon but is still relevant; 'beyond' if it is relevant "
                 "but likely resolves after the horizon end; 'unclear' if timing is "
                 "ambiguous. IMPORTANT: if an event is relevant to a canonical slot but "
                 "only covers a shorter time window than the scenario (e.g. a 1-year "
                 "forecast for a 5-year horizon), include it with horizon_fit='partial' "
                 "rather than omitting it — partial evidence is still evidence.\n\n"
