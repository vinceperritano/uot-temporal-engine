...
Do NOT set branch_group, branch_label, or causal_candidates yet — leave null/empty.
Do NOT estimate downstream_impact, disruption_score, or novelty yet.

After the events array, also return top-level fields:
  follow_up_search_queries: list of 0-3 targeted search queries for under-evidenced areas.
  missing_evidence: list of 0-3 short descriptions of absent evidence.
...
def compress_source_to_packet(
    ...
    if not LIVE_MODE:
        ...
        return SourcePacket(
            ...
            credibility=float(metadata.get("credibility", 0.7)),
            recency=float(metadata.get("recency", 0.7)),
            summary=f"[stub] {title}",
            key_claims=[f"[stub] Key claim from {publisher}"],
            causal_phrases=metadata.get("causal_phrases", []),
            branch_phrases=metadata.get("branch_phrases", []),
            evidence_snippets=[],
            raw_text_ref=metadata.get("cache_key", ""),
    ...
    return SourcePacket(
        ...
        credibility=float(result.get("credibility", 0.7)),
        recency=float(result.get("recency", 0.7)),
        summary=result.get("summary", ""),
        key_claims=result.get("key_claims", []),
        causal_phrases=result.get("causal_phrases", []),
        branch_phrases=result.get("branch_phrases", []),
        evidence_snippets=result.get("evidence_snippets", []),
        raw_text_ref=metadata.get("cache_key", url),
        raw_text_chars=len(raw_text)
    )
...
def call_anthropic_api(system_prompt: str, user_message: str) -> dict:
    """
...
def score_event_fields_specialized(event_candidates, source_summaries):
    """Stage B: three separate specialized scoring prompts."""
    if not LIVE_MODE:
        return event_candidates
    summary = "\n".join(
        f"- {e['id']}: {e['label']} | {e.get('description','')[:100]}"
        for e in event_candidates
    )
    di = call_anthropic_api("Return only valid JSON.", PROMPT_SCORE_DOWNSTREAM_IMPACT.format(events_summary=summary))
    ds = call_anthropic_api("Return only valid JSON.", PROMPT_SCORE_DISRUPTION.format(events_summary=summary))
    nv = call_anthropic_api("Return only valid JSON.", PROMPT_SCORE_NOVELTY.format(events_summary=summary))
    for ev in event_candidates:
        eid = ev["id"]
        ev["downstream_impact"] = clamp(float(di.get(eid,{}).get("score",0.5)))
        ev["disruption_score"]  = clamp(float(ds.get(eid,{}).get("score",0.5)))
        ev["novelty"]           = clamp(float(nv.get(eid,{}).get("score",0.5)))
        ev["extraction_notes"]  = {
            "downstream_impact_rationale": di.get(eid,{}).get("rationale",""),
            "disruption_score_rationale":  ds.get(eid,{}).get("rationale",""),
            "novelty_rationale":           nv.get(eid,{}).get("rationale",""),
...
def reconcile_scores_for_consistency(event_candidates):
    ...
    for ev in event_candidates:
        if ev["id"] in flags and flags[ev["id"]]:
...
def apply_structure_to_event_candidates(
    ...
    """
    Applies Stage C branch_groups and causal_edges back to event candidate dicts.

    Returns (updated_event_dicts, edge_specs) where edge_specs is a list of
    edge specification dicts ready for build_graph_from_estimates().

    Also stores structure rationales in extraction_notes for user review.
    """
    id_to_event = {e["id"]: e for e in event_dicts}

    # Apply branch group assignments
    branch_rationales = {}
    for bg in structure.get("branch_groups", []):
        group_key = bg["branch_group"]
        rationale = bg.get("rationale", "")
        confidence = bg.get("mutual_exclusivity_confidence", 0.8)
        for member in bg.get("members", []):
            eid = member["event_id"]
            if eid in id_to_event:
                id_to_event[eid]["branch_group"]  = group_key
                id_to_event[eid]["branch_label"]  = member["branch_label"]
                branch_rationales[eid] = (
                    f"[Stage C] {group_key}: {rationale} "
                    f"(mutual_exclusivity={confidence:.2f})"
                )

    # Attach branch rationale to extraction_notes
    for eid, rationale in branch_rationales.items():
        notes = id_to_event[eid].get("extraction_notes") or {}
        notes["branch_rationale"] = rationale
        id_to_event[eid]["extraction_notes"] = notes

    # Build edge specs from causal_edges
    edge_specs = []
    for edge in structure.get("causal_edges", []):
        # Attach causal rationale to source event's extraction_notes
        src_id = edge.get("source_id", "")
        tgt_id = edge.get("target_id", "")
        rationale = edge.get("rationale", "")
        if src_id in id_to_event:
            notes = id_to_event[src_id].get("extraction_notes") or {}
            existing = notes.get("causal_rationale", "")
            notes["causal_rationale"] = (
                existing + f"; {src_id}→{tgt_id}: {rationale}" if existing else
                f"{src_id}→{tgt_id}: {rationale}"
            )
            id_to_event[src_id]["extraction_notes"] = notes

        edge_specs.append({
            "source_id":       src_id,
            "target_id":       tgt_id,
            "relation_type":   edge.get("relation_type", "causal"),
            "causal_weight":   float(edge.get("causal_weight", 0.5)),
            "uncertainty":     float(edge.get("uncertainty", 0.5)),
            "feedback_strength": float(edge.get("feedback_strength", 0.0)),
            "provisional":     False,    # Stage C edges are not provisional
            "stage_c_rationale": rationale
        })

    return list(id_to_event.values()), edge_specs
