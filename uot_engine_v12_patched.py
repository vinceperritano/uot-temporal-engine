...
def validate_branch_structure(event_dicts: list) -> tuple:
    ...
    _sq_signals = ('no_', 'not_', 'remain', 'status_quo', 'maintain', 'continues',
                   'does_not', 'fails_to', 'unchanged', 'no change', 'informal_only')
    ...
    for _bg2, _mems in post_groups2.items():
        ...
        has_sq = any(
            any(sig in str(m.get('branch_label','')).lower() for sig in _sq_signals)
            for m in _mems
        )
        if not has_sq:
