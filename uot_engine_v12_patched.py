...
_ENGINE_DEPLOY_TAG = "phase-9.3-cache-bust-001"
...
For each event, include a "sources" array citing which of the provided sources informed it.
Use the publisher and title exactly as given in the SOURCE list. Include 1-3 sources per event —
the ones that most directly support the claim, not every source that merely mentions the topic.
For each source, also classify:
  "stance": how this specific source relates to THIS event's claim —
    "supporting"   = source directly asserts or supports this claim
    "neutral"      = source provides context but doesn't take a clear position on this claim
    "uncertain"    = source hedges, speculates, or expresses doubt about this claim
    "contradicting"= source argues against or undermines this claim
  "relevance": 0.0-1.0, how directly this source bears on THIS specific event (not the broader
    topic) — a source that states the claim directly is ~0.9-1.0; a source that only provides
    tangential background is ~0.3-0.5.
Do not default every source to "neutral"/0.5 — actually assess each one. Most sources directly
cited as evidence for an event should be "supporting" with relevance >= 0.7, since you chose them
specifically because they inform this claim.
Format: "sources": [{{"title": "<exact title from SOURCE list>", "publisher": "<exact publisher>",
"stance": "supporting", "relevance": 0.85}}]
This is for observer review — it lets the person verify why an event exists, not just trust the label.

Return format:
{{
  "events": [
    {{ ... , "sources": [{{"title": "...", "publisher": "...", "stance": "...", "relevance": 0.0}}] }}
  ],
...
def clear_primary_question_state() -> None:
    ...
    global _LAST_PRIMARY_QUESTION, _LAST_SOURCE_PORTFOLIO
...
def build_seeded_events_from_dicts(event_dicts):
    ...
    print(f"[Phase 9.3 diag] build_seeded_events_from_dicts called with "
          f"{len(event_dicts) if event_dicts else 0} event_dicts")
    seeded = []
    # Phase 9.3 follow-up diagnostic: confirm whether Stage A is actually
    # complying with the stance/relevance instruction, or still silently
    # defaulting every source to neutral/0.7 (which would explain why
    # source_coherence numbers haven't moved despite the prompt change).
    _src_total = 0
