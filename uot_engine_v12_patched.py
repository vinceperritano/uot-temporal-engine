...
def _detect_constructed_negated_slot_label(event_label: str, canonical_slots) -> str:
    ...
    for slot in canonical_slots:
        ...
        # If the event label contains most of the slot's distinctive vocabulary
        # AND has a negation signal, it's likely a constructed negation of that slot.
        if len(_overlap) / len(_slot_tokens) >= 0.6:
            return getattr(slot, 'slot_id', '') or ''
    return ""
