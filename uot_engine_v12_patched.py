...
_ENGINE_DEPLOY_TAG = "phase-10.2-crossrun-endpoint-fix"
...
def normalize_and_aggregate_primary_slots(
    ...
    for slot in slots:
        ...
        # Phase 9.10 bugfix (secondary Gate B → primary branch group disappearance):
        # Secondary Gate B candidates are added to assignments as shallow COPIES
        # (dict(cand_ev)) with _rank_weight added. When Step 6 sets branch_group on
        # rep (the copy), the ORIGINAL event dict in event_dicts is not updated.
        # build_seeded_events_from_dicts reads from event_dicts originals, so the
        # SeededEvent.branch_group stays None and the slot never appears in the
        # Primary Branch Group display. Fix: sync branch-group fields back to
        # the original event dict in event_dicts whenever the rep is a copy.
        _rep_id = rep.get("id")
        if _rep_id:
            for _ev_orig in event_dicts:
                if _ev_orig.get("id") == _rep_id and _ev_orig is not rep:
                    _ev_orig["branch_group"]       = bg_id
                    _ev_orig["branch_label"]       = rep["branch_label"]
                    _ev_orig["outcome_role"]       = rep["outcome_role"]
                    _ev_orig["canonical_slot_id"]  = sid
                    _ev_orig["branch_probability"] = rep["branch_probability"]
                    _ev_orig["slot_raw_probability"] = rep["slot_raw_probability"]
                    break
        ...
        if _rep_md.get('inverted_for_slot') == sid:
            ...
            # Phase 10 bugfix (Review Room showing raw probability instead of
            # inverted for secondary-candidate representatives): this mutation
            # only updates `rep`. For Step 1/Step 2 primary-path events, rep IS
            # the same object as the original in event_dicts, so the change is
            # visible everywhere. For secondary Gate B candidates, rep is
            # cand_ev_copy — a separate shallow copy — so the original entry in
            # event_dicts (what the Review Room renders) never received this
            # mutation and kept showing the raw, uninverted probability and
            # label. Sync these fields back to the original whenever rep is a
            # distinct copy, mirroring the existing branch_group sync fix.
            _rep_id_inv = rep.get("id")
            if _rep_id_inv:
                for _ev_orig_inv in event_dicts:
                    if _ev_orig_inv.get("id") == _rep_id_inv and _ev_orig_inv is not rep:
                        _ev_orig_inv["label"] = rep["label"]
                        _ev_orig_inv["probability"] = rep["probability"]
                        _ev_orig_inv["metadata"] = _rep_md
                        break
