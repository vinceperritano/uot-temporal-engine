...
def find_organic_events_for_slot(slot, event_dicts: list) -> list:
    ...
    for ev in event_dicts:
        ...
        if sim >= 0.62:
            ...
            candidates.append((sim, ev))
        elif sim < 0.48 and _polarity_matches_label(slot.slot_polarity, label) and sim >= 0.22:
...
def normalize_and_aggregate_primary_slots(
    ...
    # ── Step 2b: Direct rank assignment when Stage C + Jaccard both fail ─────────
    # Uses only UNRESOLVED events — resolved events are past facts, not future outcomes.
    any_assigned = any(len(v) > 0 for v in assignments.values())
    if not any_assigned:
        # Filter 1: unresolved events (not past facts)
        # Filter 2: not causal_context/evidence_context (those are evidence, not outcomes)
        # Filter 3: not synthetic
        unresolved = [e for e in event_dicts
            if isinstance(e, dict)
            and not str((e.get('metadata') or {}).get('synthetic', '')).lower() == 'true'
            and not str(e.get('id', '')).startswith('EVT_SYNTH')
            and e.get('temporal_status') not in ('resolved', 'historical')
            and e.get('outcome_role') not in ('causal_context', 'evidence_context')]
        organics_ranked = sorted(unresolved,
            key=lambda e: float(e.get('probability', 0) or 0), reverse=True)
        print(f"[v0.12] Step 2b: {len(unresolved)} unresolved organic candidates for {len(slots)} slots")
        used_ids: set = set()
        for slot in slots:
            for ev in organics_ranked:
                eid = ev.get('id', '')
                if eid not in used_ids:
                    assignments[slot.slot_id].append(ev)
                    used_ids.add(eid)
                    print(f"[v0.12] Step 2b: {eid} (p={ev.get('probability',0):.3f}, "
                          f"status={ev.get('temporal_status','?')}) → '{slot.slot_id}'")
                    break
