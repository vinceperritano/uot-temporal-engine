...
def generate_search_queries(topic: str, observer_basis: dict, canonical_slots=None,
    ...
    queries = [
        ...
        {"query": f"{topic} analysis forecast {_horizon_year}",
         "purpose": "horizon_forecast", "source_type": "forecast", "max_results": 5},
        {"query": f"{topic} risks scenarios {_horizon_year}",
         "purpose": "branch_detection", "source_type": "article", "max_results": 4},
    ]
...
def web_search_sources(topic: str, observer_basis: Optional[dict] = None, follow_up_queries: Optional[List[str]] = None, canonical_slots=None) -> List[SourcePacket]:
    """
    ...
    # Phase 8.1: pass slot-targeted queries to live_source_pipeline via follow_up_queries.
    # live_source_pipeline (uot_live_search.py) doesn't accept canonical_slots directly,
    # so we convert slot-targeted queries to plain strings and merge with follow_up_queries.
    _combined_queries = list(follow_up_queries or [])
    if canonical_slots:
        try:
            _slot_qs = generate_search_queries(topic, observer_basis or {},
                                               canonical_slots=canonical_slots)
            # Add slot-targeted and disconfirmation query strings to follow_up_queries
            for _q in _slot_qs:
                _purpose = _q.get('purpose', '')
                if _purpose.startswith(('slot_', 'disconfirmation_', 'slot_threshold_')):
                    _qs = _q.get('query', '')
                    if _qs and _qs not in _combined_queries:
                        _combined_queries.append(_qs)
        except Exception as _sqe:
            print(f"[Phase 8.1] slot query generation error: {_sqe}")
    raw_docs = live_source_pipeline(topic, observer_basis, _combined_queries or None)
