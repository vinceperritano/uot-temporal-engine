...
_STRONG_NEGATION_PATTERNS = (
    'will not occur', 'will not materialize', 'will not happen',
    'does not occur', 'does not materialize', 'will not be achieved',
    'will not be reached', "won't occur", "won't materialize",
    'fails to materialize', 'fails to occur', 'will fail to',
    'will not take place', 'does not take place',
)
...
def _event_negates_yes_slot(ev_label: str) -> bool:
    ...
    "Convergence toward harmonized standards" and the event says "Full
    Trilateral Harmonization Will Not Occur." In that case the event's
    probability (e.g. 82% chance it will NOT occur) must be inverted
    (1 - 0.82 = 0.18) before contributing to the slot's raw probability —
    otherwise an event that is STRONG EVIDENCE AGAINST an outcome gets
    counted as strong evidence FOR it.

    Deliberately narrow (only "X will not occur/happen/materialize"-style
    patterns) rather than _polarity_matches_label's broader NEG_SIGS list
    (which includes "remain"/"frozen"/"status quo" and would misfire on
    status_quo/no-polarity slots whose representative events legitimately
    describe continuation without using these specific negation phrases).
    """
    el = ev_label.lower()
    return any(p in el for p in _STRONG_NEGATION_PATTERNS)
...
def normalize_and_aggregate_primary_slots(
    ...
    def _register_effective_prob(sid, ev):
        ...
        if slot.slot_polarity == 'yes' and _event_negates_yes_slot(ev_label):
    ...
    for pos, member in enumerate(members_raw):
        ...
        if ev and match_conf >= 0.40:
            if match_conf < 0.55:
                md = ev.setdefault('metadata', {})
                md['weak_match'] = 'true'
                md['weak_match_score'] = round(float(match_conf), 3)
