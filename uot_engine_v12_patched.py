...
def normalize_and_aggregate_primary_slots(
    ...
    for member in (pbg.get("members") or []):
        ...
        # 1a. Exact match on slot_id
        matched_sid = slot_id if slot_id in assignments else None
        # 1b. Fuzzy match on slot_id (lowered threshold: Stage C uses different naming)
        if not matched_sid:
        ...
        # 1c. If slot_id hopeless, match member label against canonical slot labels
        if not matched_sid and member_label:
            best_sim, best_cand = 0.0, None
            for s in slots:
                sim = _jaccard_sim(s.label, member_label)
                if sim > best_sim:
                    best_sim, best_cand = sim, s.slot_id
            if best_sim >= 0.30:
                matched_sid = best_cand
