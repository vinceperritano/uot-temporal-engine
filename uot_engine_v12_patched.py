...
def detect_discrepancies(world, scores: dict, observer_basis: dict) -> list:
    ...
    for bg_key, member_ids in branch_groups.items():
        ...
        if len(high) >= 2:
            ...
            sev = min(1.0, avg_sev * (sum(getattr(n,'probability',0) for _,n in high) - 1.0))
            discrepancies.append(Discrepancy(
...
def run_iterative_simulation(world, params, observer_basis: dict,
    ...
    for i in range(max_iterations):
        ...
        world, step_scores = simulation_step(world, params, dt)
        ...
        if discs:
            ...
            normalize_branch_groups(world.event_graph)
...
def _extract_scores(world) -> dict:
    """Extract key scalar scores from world state."""
    ts = world.temporal_state if hasattr(world, 'temporal_state') else None
    graph = world.event_graph
    nodes = graph.nodes if graph else {}
    ...
    if nodes:
        ...
        instab = sum(energies) / len(energies)

    return {
        'global_instability': round(getattr(ts, 'global_instability', instab), 4),
        'temporal_flux':      round(getattr(ts, 'temporal_flux', 0.5), 4),
        'global_branch_potential': round(
            sum(getattr(n, 'indeterminacy', 0.5) for n in nodes.values()) / max(len(nodes), 1), 4
        ),
    }
...
1. BRANCH GROUPS: mutually exclusive alternative outcomes of the same resolution question.
   ...
   - BAD:  "Ceasefire holds OR war resumes"   (describes two contradictory outcomes)
...
{{
  "branch_groups": [
    {{"branch_group": "key", "resolution_question": "...",
      "members": [{{"event_id": "e1", "branch_label": "Single coherent outcome label"}}],
