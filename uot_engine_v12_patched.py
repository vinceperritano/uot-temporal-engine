...
import sqlite3
import hashlib
# Phase 9.3 cache-buster: a module-level constant that changes with every deploy,
# ensuring Railway's bytecode cache (.pyc) is invalidated rather than serving
# a stale compiled version. This explains why print() in build_seeded_events_from_dicts
# never appeared despite being on the function's first line — the old .pyc was being used.
_ENGINE_DEPLOY_TAG = "phase-10.3-cache-session-stats"

import json  # Phase 8 bugfix: module-level import. Previously this file relied
             # entirely on local `import json` statements scattered at individual
             # call sites (no module-level import existed at all), which caused a
             # NameError the first time json.dumps was used in a function that
             # didn't have its own local import (estimate_uot_fields, Phase 8.9).
...
class SourceRef:
    ...
    source_type: str = ""     # Phase 8.9: article|forecast|official_action|market|... (from SourcePacket)
    query_purpose: str = ""   # Phase 8.9: which search class/slot this source was retrieved for
...
class SourcePacket:
    ...
    query_purpose: str = ""        # Phase 8: which slot/class this source was retrieved for
    source_family_key: str = ""    # Phase 8.4: publisher|type|slot_or_topic for anchor tracking
    canonical_slot_hint: str = ""  # Phase 8.4: which slot this source most likely informs
...
class SlotAssignment:
    ...
    rationale:          str              = ""
    # Phase 9.3: source-coherence penalty inputs (GPT guidance)
    scope_compat:       float            = 1.0    # actor-scope compatibility (1.0=clean match)
    threshold_alt_warning: bool          = False  # mapped via threshold rescue/alias path
...
# ============================================================
# Phase 8.11 (GPT guidance) — SlotContribution: a layer between
# SlotAssignment (single representative event per slot) and
# SlotInfluence (contextual pressure from non-branch events).
#
# A SlotContribution is DIRECT evidence bearing on a slot that did NOT
# pass the stricter representative Gate B — it failed to "become the
# face of the outcome" but can still legitimately contribute evidentiary
# weight. This is the architectural fix for the secondary-candidate gap:
# Gate B was being used for two different jobs (representative selection
# AND evidence aggregation) with one standard. The Contributor Gate below
# gives evidence aggregation its own, deliberately looser standard.
# ============================================================
@dataclass
class SlotContribution:
    """
    Additional direct evidence bearing on a canonical slot, accepted via the
    looser Contributor Gate rather than the stricter representative Gate B.
    Does NOT become the slot's representative event in the Review Room —
    it only contributes evidentiary weight to the slot's aggregated support
    via compute_slot_contribution_adjusted_support().
    """
    slot_id:               str
    event_id:               str
    source_event_id:        Optional[str] = None
    relation_to_slot:       str   = "supports"   # supports | opposes | context
    relation_confidence:    float = 0.0
    engine_score:           float = 0.0
    event_kind:             str   = "substantive_development"
    effective_probability:  float = 0.5
    ...
    evidence_family_key:    str   = ""   # source_family_key is used as the practical
                                          # evidence-family grouping key (see Part 8.12
                                          # implementation note: true claim-clustering is
                                          # out of scope for this diagnostic pass)
    independence_group:     str   = ""
    accepted_by:             str   = "contributor_gate"
    rationale:               str   = ""
    # Phase 9.3: source-coherence penalty inputs (GPT guidance)
    scope_compat:            float = 1.0    # actor-scope compatibility (1.0=clean match)
    threshold_alt_warning:   bool  = False  # accepted via threshold rescue/alias path


# ============================================================
# Phase 8.12 (GPT guidance) — Evidence-Family Aggregation Shadow Layer.
# Computes a SHADOW comparison only — does not feed branch normalization yet.
# Family-first aggregation: group contributions by evidence family before
# combining, since individual articles within one family are not independent
# evidence (they often echo the same underlying source/claim).
# ============================================================
def compute_slot_contribution_adjusted_support(
    slot_id: str,
    representative_effective_probability: float,
    contributions: list,
    k_contribution: float = 0.25,
    max_total_shift: float = 0.10,
    max_single_family_shift: float = 0.05,
) -> dict:
    """
    Phase 8.12: shadow-compute what a slot's support WOULD be if secondary
    SlotContribution evidence were incorporated, without committing to it.
    Family-first aggregation: contributions are grouped by evidence_family_key
    before combining, so multiple articles from the same effective source
    don't get counted as independent confirming evidence.

    base = representative_effective_probability
    For each independent evidence family:
        family_p = weighted_mean(effective_probability of contributions in family)
        family_shift = reliability_weight * independence_weight * (logit(family_p) - logit(base))
        family_shift clamped to +/- max_single_family_shift
    adjusted = sigmoid(logit(base) + k_contribution * sum(family_shift))
    total shift clamped to +/- max_total_shift

    independence_weight is fixed at 1.0 for this diagnostic pass — true
    cross-family independence detection (e.g. shared parent organizations,
    shared wire-service origin) is out of scope here. Family-level grouping
    (same publisher+type+slot) already captures the most common non-independence
    case (multiple articles from one source family).
    """
    base = float(representative_effective_probability)
    relevant = [c for c in (contributions or []) if c.slot_id == slot_id]

    if not relevant:
        return {
            "representative_support": round(base, 4),
            "contribution_adjusted_support": round(base, 4),
            "shift": 0.0,
            "family_count": 0,
    ...
    families: dict = {}
    for c in relevant:
        families.setdefault(c.evidence_family_key or "unknown", []).append(c)

    total_shift = 0.0
    family_shifts = {}
    for fam_key, fam_contribs in families.items():
        total_w = sum(max(0.01, c.contribution_weight) for c in fam_contribs)
        family_p = sum(c.effective_probability * max(0.01, c.contribution_weight)
                       for c in fam_contribs) / total_w if total_w else base
        # relation_to_slot 'opposes' contributions already have effective_probability
        # inverted at creation time (see Contributor Gate), so family_p is already
        # in "support for this slot" terms.
        reliability_weight = sum(c.relation_confidence for c in fam_contribs) / len(fam_contribs)
        independence_weight = 1.0  # see docstring — family grouping is the independence proxy for now
        raw_shift = reliability_weight * independence_weight * (_logit_sf(family_p) - _logit_sf(base))
        clamped_shift = max(-max_single_family_shift, min(max_single_family_shift, raw_shift))
        family_shifts[fam_key] = round(clamped_shift, 4)
        total_shift += clamped_shift

    total_shift = max(-max_total_shift, min(max_total_shift, total_shift))
    adjusted = _sigmoid_sf(_logit_sf(base) + k_contribution * total_shift)

    dominant_family = max(families.keys(), key=lambda k: len(families[k])) if families else ""
    contribution_count = len(relevant)
    family_count = len(families)
    confidence = ("strong" if family_count >= 3 else
                  "moderate" if family_count >= 2 else "weak")

    return {
        "representative_support": round(base, 4),
        "contribution_adjusted_support": round(adjusted, 4),
        "shift": round(adjusted - base, 4),
...
_INFLUENCE_KIND_WEIGHT = {
    ...
    "structured_forecast_signal": 0.60,  # Phase 8.7: Metaculus-style — above betting markets, below official
    "expert_assessment":    0.65,
    "market_signal":        0.55,
    "public_opinion_signal": 0.45,
    "causal_context":       0.30,
    "meta_signal":          0.25,
    "reflexive_market_signal": 0.20,     # Phase 8.7: Polymarket/Kalshi/etc reported via journalism — weak-moderate
...
def compute_slot_influence_pressures(
    ...
    for slot in (slots or []):
        ...
        if not sid:
            continue
        # Phase 8 bugfix: "or 0.5" previously masked legitimate 0.0 values,
        # hiding the real raw probability behind a fake 0.5 default in every log.
        _raw_lookup = raw_slot_probs.get(sid)
        raw_p = 0.5 if _raw_lookup is None else float(_raw_lookup)
        ...
        for inf in slot_influences:
            ...
            # Structural pressure channel — handle both dict and SeededEvent
            _sm = (src_ev if isinstance(src_ev, dict) else {})  # safe dict view
            _sm_md = (_sm.get('metadata') or {}) if _sm else {}
            _ts = str(_sm_md.get('temporal_status', '') or _sm.get('temporal_status', '')
                      or getattr(src_ev, 'temporal_status', '') or 'unresolved').lower()
            _status_w = {'resolved': 0.80, 'historical': 0.80, 'active': 0.70,
                         'beyond_horizon': 0.20, 'counterfactual': 0.10}.get(_ts, abs(2*ev_prob - 1))
            _rec_coh = float(_sm_md.get('record_coherence', 0.5)
                             or _sm.get('record_coherence', 0.5)
                             or getattr(src_ev, 'record_coherence', 0.5) or 0.5)
            _src_agr = float(_sm_md.get('source_agreement', 0.5)
                             or _sm.get('source_agreement', 0.5)
                             or getattr(src_ev, 'source_agreement', 0.5) or 0.5)
    ...
    # Phase 8 (GPT guidance — SlotInfluence shadow comparison):
    # Compute what branch probabilities WOULD be if influence were applied,
    # without committing to it. This lets us compare adjusted vs unadjusted
    # branch shares across observations before enabling ENABLE_SLOT_INFLUENCE_APPLY.
    shadow_comparison = _compute_slotinfluence_shadow_branch_comparison(diagnostics, raw_slot_probs)
    for d in diagnostics:
        _sid = d.get('slot_id')
        if _sid in shadow_comparison:
            d['shadow_branch_comparison'] = shadow_comparison[_sid]
    if shadow_comparison:
        _max_shift = max(abs(v['shift_prob']) for v in shadow_comparison.values())
        _avg_shift = sum(abs(v['shift_prob']) for v in shadow_comparison.values()) / len(shadow_comparison)
        print(f"[Phase 8] SlotInfluence shadow branch comparison: "
              f"max_shift={_max_shift:.4f}, avg_shift={_avg_shift:.4f} "
              f"(activation criteria: max<=0.08, avg<=0.03-0.04)")

    return diagnostics


def _compute_slotinfluence_shadow_branch_comparison(diagnostics: list, raw_slot_probs: dict) -> dict:
    """
    Phase 8 (GPT guidance): shadow-apply SlotInfluence pressure to compare
    pre/post BRANCH probabilities (not just raw slot probabilities) without
    committing to the adjustment. Branch probabilities are what the simulation
    actually displays, so this is the comparison that matters for activation review.

    Returns {slot_id: {"unadjusted_branch_share": x, "shadow_branch_share_prob": y,
                        "shadow_branch_share_struct": z, "shift_prob": dy, "shift_struct": dz}}
    """
    if not diagnostics or not raw_slot_probs:
        return {}

    # Phase 8 bugfix: numerator must use the same max(0.0001, ...) floor as the
    # denominator, or shares collapse toward 0 when a raw probability is legitimately
    # 0.0 (which is now possible after fixing the upstream masking bug).
    _sids = list(raw_slot_probs.keys())
    _floored_raw = {s: max(0.0001, raw_slot_probs.get(s, 0.5) if raw_slot_probs.get(s) is not None else 0.5)
                    for s in _sids}
    _raw_total = sum(_floored_raw.values()) or 1.0
    unadjusted_shares = {s: _floored_raw[s] / _raw_total for s in _sids}

    # Build candidate (shadow) probability maps from diagnostics
    _cand_prob = {d['slot_id']: d.get('candidate_probability_after', raw_slot_probs.get(d['slot_id'], 0.5))
                  for d in diagnostics if d.get('slot_id') in _sids}
    _cand_struct = {d['slot_id']: d.get('structural_candidate_probability', raw_slot_probs.get(d['slot_id'], 0.5))
                    for d in diagnostics if d.get('slot_id') in _sids}

    _prob_total = sum(max(0.0001, _cand_prob.get(s, raw_slot_probs.get(s, 0.5))) for s in _sids) or 1.0
    _struct_total = sum(max(0.0001, _cand_struct.get(s, raw_slot_probs.get(s, 0.5))) for s in _sids) or 1.0

    result = {}
    for s in _sids:
        shadow_prob_share = max(0.0001, _cand_prob.get(s, raw_slot_probs.get(s, 0.5))) / _prob_total
        shadow_struct_share = max(0.0001, _cand_struct.get(s, raw_slot_probs.get(s, 0.5))) / _struct_total
        result[s] = {
            "unadjusted_branch_share": round(unadjusted_shares[s], 4),
            "shadow_branch_share_prob": round(shadow_prob_share, 4),
            "shadow_branch_share_struct": round(shadow_struct_share, 4),
            "shift_prob": round(shadow_prob_share - unadjusted_shares[s], 4),
            "shift_struct": round(shadow_struct_share - unadjusted_shares[s], 4),
        }
    return result
...
def compute_source_agreement(sources: List[SourceRef]) -> float:
    ...
    # Phase 9.3 stance diagnostic — runs every time, in the known-working pipeline path.
    # Workaround: identical diagnostic in build_seeded_events_from_dicts never prints
    # (suspected Railway worker caching — old function body running instead of new).
    # This location is confirmed working. Only prints when non-default stance data found,
    # so it's silent when Stage A defaults everything to neutral/0.7.
    _non_default = [s for s in sources if s.stance != "neutral" or s.relevance != 0.7]
    if _non_default:
        _breakdown = {}
        for _s in sources:
            _breakdown[_s.stance] = _breakdown.get(_s.stance, 0) + 1
        print(f"[Phase 9.3 diag] Stage A provided non-default stances: {_breakdown}, "
              f"relevances: {[round(_s.relevance, 2) for _s in sources]}")
...
def estimate_uot_fields(
    ...
    return {
        ...
        # Phase 8.9: structured source list for observer review (title/publisher/
        # source_type/query_purpose). Serialized as JSON string in metadata since
        # EventNode.metadata is Dict[str,str]. Bugfix: local import to match this
        # file's existing convention (no module-level `import json` exists here —
        # every other json.dumps/loads call in this file imports it locally too).
        "sources_json": __import__('json').dumps([
            {"title": s.title, "publisher": s.publisher, "url": s.url,
             "source_type": getattr(s, 'source_type', ''),
             "query_purpose": getattr(s, 'query_purpose', '')}
            for s in (candidate.sources or [])
        ]) if candidate.sources else "",
    ...
    for n in (x for x in estimated_nodes if isinstance(x, dict)):
        node = EventNode(
            ...
            metadata         = {
                ...
                # Phase 8.9: structured sources (title/publisher/source_type/query_purpose)
                # for observer review in the node detail panel.
                "sources_json": n.get("sources_json", ""),
                **(n.get("seed_metadata", {}) or {}),
            },
        )
        if node.metadata.get("evidence_basis_note"):
            print(f"[Phase 8 diag] evidence_basis_note REACHED EventNode.metadata "
                  f"for {node.id}: {node.metadata['evidence_basis_note']!r}")
        if node.metadata.get("sources_json"):
            try:
                _n_srcs = len(__import__('json').loads(node.metadata["sources_json"]))
            except Exception:
                _n_srcs = "?"
            print(f"[Phase 8 diag] sources_json REACHED EventNode.metadata "
                  f"for {node.id}: {_n_srcs} source(s)")
        graph.add_node(node)
...
MANDATORY SOURCE ATTRIBUTION (Phase 9.7 — required for every event):
Every event you extract MUST cite at least one source from the provided source list.
Do not omit sources. Do not invent source IDs. If no source in the provided list
supports an event, do not output that event.

Sources are identified by their short ID at the start of each source block (S01, S02, etc.).
Cite the ID directly — do not reproduce publisher names or titles.

For each cited source, provide:
  "source_id": "S03"       ← the ID exactly as shown (e.g. S01, S02, S03)
  "stance":    "supporting" ← how this source relates to THIS event's specific claim:
                               "supporting"    = source directly asserts or supports this claim
                               "neutral"       = source provides context without taking a position
                               "uncertain"     = source hedges, speculates, or expresses doubt
                               "contradicting" = source argues against this claim
  "relevance": 0.85         ← 0.0–1.0 how directly this source addresses THIS specific event
                               (not just the broader topic). A source that directly states
                               the claim: 0.80–1.0. Tangential background only: 0.30–0.50.

Do not default every source to "neutral" / 0.5. Assess each one. Most directly-cited
sources for an event should be "supporting" with relevance >= 0.70. If a source
contradicts the event's claim, it should be "contradicting" with appropriate relevance.

Example:
  "sources": [
    {{"source_id": "S01", "stance": "supporting", "relevance": 0.90}},
    {{"source_id": "S04", "stance": "uncertain",  "relevance": 0.65}}
  ]

OUTCOME-AWARE SEARCH DISCIPLINE (Phase 8.2 — critical):
The source set you receive includes outcome-targeted searches for every canonical slot.
Do NOT infer that each outcome is equally likely because each outcome has search coverage.
Search coverage means the engine attempted to observe that branch. Probability must still be estimated from source strength, credibility, causal pressure, base-rate context, and horizon fit.

Low-probability outcomes should still be extracted when they are part of the canonical outcome space.
Assign them low probability, higher indeterminacy, or lower record_confidence as appropriate.
Do NOT omit them merely because they seem unlikely.
Do NOT inflate them merely because a targeted search found evidence about them.

The Fed Rate Cuts example: if targeted searches found articles saying "markets do not expect large cuts," extract that as a low-probability event for the large-cut slot with probability 0.05-0.15 and source_agreement="weak". Do not omit it, and do not inflate it to 0.40 because it had a dedicated search.

Identify 8–12 candidate events for a Unified Observer Theory timeline graph from the topic and sources. The source set has been expanded with outcome-targeted searches for every canonical slot — use this coverage to include at least one event per canonical slot when evidence exists, including low-probability events (p=0.05–0.20) for slots with weak coverage.
...
Aim for 8-12 events. Do not stop early if evidence exists for quieter canonical slots. Every canonical slot should have at least one associated event when any evidence exists.

PROBABILITY BANDING (Phase 8.5 — required before assigning numeric probability):
Before assigning a numeric probability, classify the event into one band:
  remote:              0.02–0.15
  minority_plausible:  0.15–0.35
  contested:           0.35–0.65
  dominant:             0.65–0.85
  strong_consensus:    0.85–0.95
Then choose a number inside that band. Do not jump between bands unless the evidence basis
materially changes. Most institutional-forecast-dominant claims belong in dominant (0.65-0.85),
not strong_consensus (0.85-0.95) — reserve strong_consensus for near-unanimous, high-certainty cases.
Include "probability_band" and "probability_inputs" (your four sub-scores from the rubric above)
in each event's output so the probability is auditable.

EVIDENCE REFLEXIVITY CLASSIFICATION (Phase 8.7):
Some sources report prediction-market odds (Polymarket, Kalshi, PredictIt, Manifold, similar platforms).
These are REFLEXIVE — they aggregate existing public belief, not independent analysis. If a reputable
outlet reports such odds, classify that event's source_kind as "reflexive_market_signal", not as
independent journalistic confirmation. Weight this as weak-to-moderate evidence only — the outlet's
credibility does not convert market odds into independent evidence.

Do NOT confuse this with institutional market-implied pricing: Fed funds futures, OIS curves, Treasury
yields, bond-market expectations, or institutional demand/supply projections (IEA, OPEC, Goldman-style
analyst forecasts). These reflect institutional capital allocation, policy expectations, or expert
analysis — not retail prediction-market reflexivity. Classify these as "forecast_signal" (the only
correct category for genuine institutional forecasting evidence) and weight them normally — they
can carry meaningful evidentiary weight and should clear Gate B like any other substantive evidence.

For Metaculus and similar structured/expert forecasting communities, classify as "structured_forecast_signal" —
weight above betting markets but below direct official/substantive evidence. Preserve attribution.

EVIDENCE EXTRACTION:
Your job is to describe what sources claim — what IS happening, what IS being forecast,
what IS the current state — and score the probability that claim is true.

Extract evidence as a positive, source-facing description:

  Source says: "Analysts see strong barriers to AI governance harmonization"
  EXTRACT: "Structural barriers to AI regulatory harmonization remain entrenched" — p=0.82

  Source says: "Fed officials signal caution on large rate moves"
  EXTRACT: "Federal Reserve officials indicate preference for steady-hold rate stance" — p=0.78

  Source says: "North Korea shows no signs of testing pause"
  EXTRACT: "North Korea maintains active nuclear and missile testing program" — p=0.85

EVENT LABELS MUST DESCRIBE STATES OR DEVELOPMENTS, NOT META-PROBABILITY JUDGMENTS.
The label states what condition or development is being claimed.
The probability field states how likely that condition or development is.
These are independent — never let the label carry a probability judgment.

ALLOWED — natural non-occurrence, absence, continuation, status-quo claims:
  "No binding multilateral accord materializes"
  "Status quo of fragmented regulation persists"
  "Article 5 remains uninvoked"
  "Federal Reserve maintains rates near current levels"
  "Binding Multilateral AI Governance Accord Fails to Materialize" (natural non-occurrence)
  "No formal ceasefire is in place"

NOT ALLOWED — meta-probability language (probability judgments embedded in the label):
  "Binding accord unlikely to emerge"
  "Convergence remains remote"
  "Major cuts have low probability"
  "Agreement expected not to occur"
  "Harmonization will not occur" (when the source is asserting low likelihood, not a completed fact)

FORBIDDEN words in labels — these signal a meta-probability claim rather than a state:
  "unlikely," "improbable," "low probability," "little chance," "doubtful," "remains remote"

"Fails to," "no," and "does not" remain allowed when they describe a natural non-occurrence
or completed absence that the source is asserting as a state of affairs, not a prediction
about likelihood.

Score every event's probability as: how likely is it that THIS state or development is real?
A confident source → high probability (0.80-0.90). A hedged forecast → lower (0.40-0.60).


CRITICAL — probability assignment:
  Every event probability is an independent evidence-level estimate:
  "How likely is this event or condition, based on its cited sources and structural
  plausibility within the horizon?"

  Do NOT make event probabilities sum to 1.0.
  Do NOT redistribute probability mass across candidate events.
  Do NOT lower one event merely because another event in the same batch is stronger,
  unless the cited sources directly contradict it.

  This applies to ALL events regardless of outcome_role — primary outcomes, status quo
  outcomes, causal context, and evidence context all receive independent estimates.
  A source that strongly supports incremental weapons development should produce a high
  probability for that event even if other sources in the batch strongly support
  accelerated development. These are independent source-level observations.

  Branch competition and normalization are handled later by Stage C and branch
  aggregation. Stage A estimates evidence strength; it does not compute final branch
  probabilities.
...
CRITICAL — event label tense rules:
  ...
  future-leaning events: modal future  (e.g. "Ceasefire Negotiations Expected to Resume")
  Never write an unresolved event in present or past tense — it causes observer confusion.

For each event, cite sources using ONLY the short source IDs (S01, S02, etc.) from the
source list above. Do NOT reproduce publisher names, titles, or any other metadata —
the engine looks those up from the ID. Include 1-3 source IDs per event (the most
directly relevant ones). Missing sources = this event will be flagged as unattributed.

Return format:
{{
  "events": [
    {{ ... , "sources": [{{"source_id": "S01", "stance": "supporting", "relevance": 0.90}}] }}
  ],
  "follow_up_search_queries": [],
  "missing_evidence": [],
  "slot_evidence_coverage": {{
    "<slot_id>": "strong | moderate | weak | weak_or_absent"
  }}
}}
slot_evidence_coverage: for each canonical slot you were given, assess how much evidence the source set contains:
  "strong"        — multiple credible sources directly address this outcome
  "moderate"      — some sources address it, possibly indirectly
  "weak"          — one source or only indirect references
  "weak_or_absent"— no credible source addresses this outcome in the current source set
This is for diagnostics. It does NOT affect probability. A weak-evidence slot should still be extracted if evidence exists.
...
def build_slot_vocabulary_profile(slot) -> dict:
    """
    Phase 9.8 (GPT guidance): one canonical slot vocabulary profile combining
    slot_id tokens, label, description, aliases, evidence_terms, action_exemplars,
    negative_examples, actor_scope, directionality, and scale_level into a
    single normalized token set for reuse across compute_topic_relevance_score,
    _rationale_overlaps_slot, and Stage C slot context.

    Returned dict keys:
      positive_tokens: set of normalized tokens for matching (label + all positive signals)
      negative_tokens: set of normalized tokens from negative_examples (penalty on match)
      directionality: str or None (continuation|intensification|reversal|breakthrough|...)
      evidence_terms: list of str (raw, for phrase-level matching)
      action_exemplars: list of str
      negative_examples: list of str
    """
    _stopwords = frozenset({
        'the', 'a', 'an', 'of', 'in', 'on', 'at', 'to', 'for', 'and', 'or',
        'is', 'are', 'be', 'will', 'with', 'by', 'from', 'that', 'this',
        'which', 'its', 'not', 'no',
    })
    ...
    # Core: slot_id tokens, label, description
    positive_tokens |= _tok(slot.slot_id.replace('_', ' '))
    positive_tokens |= _tok(slot.label or '')
    positive_tokens |= _tok(str(slot.description or ''))

    # Enrichment: aliases, evidence_terms, action_exemplars
    for _a in (getattr(slot, 'aliases', []) or []):
        positive_tokens |= _tok(str(_a))
    for _et in (getattr(slot, 'evidence_terms', []) or []):
        positive_tokens |= _tok(str(_et))
    for _ae in (getattr(slot, 'action_exemplars', []) or []):
        positive_tokens |= _tok(str(_ae))

    # Directionality hint: add as tokens too
    _directionality = str(getattr(slot, 'directionality', '') or '')
    if _directionality:
        positive_tokens |= _tok(_directionality)

    # Negative examples: tokens for penalty scoring
    negative_tokens = set()
    for _ne in (getattr(slot, 'negative_examples', []) or []):
        negative_tokens |= _tok(str(_ne))
    # Remove words that appear in positive set (penalty only for truly negative-specific terms)
    negative_tokens -= positive_tokens

    return {
        'positive_tokens': positive_tokens,
        'negative_tokens': negative_tokens,
        'directionality': _directionality or None,
        'evidence_terms': list(getattr(slot, 'evidence_terms', []) or []),
        'action_exemplars': list(getattr(slot, 'action_exemplars', []) or []),
        'negative_examples': list(getattr(slot, 'negative_examples', []) or []),
    }


def compute_topic_relevance_components(slot, ev: dict, rationale: str = "") -> dict:
    """
    Phase 9.11 (GPT guidance): return ALL scoring components as a dict so
    Gate B can inspect them for the Semantic Bridge Rescue without needing
    to recompute them separately. compute_topic_relevance_score calls this
    and sums the components to a single float.
    """
    label = ev.get('label', '') or ev.get('branch_label', '') or ''
    if not label:
        return {"entity_sim": 0.0, "action_sim": 0.0, "affinity": 0.0,
                "rat_sim": 0.0, "hint_sim": 0.0, "horiz_sim": 0.0,
                "evidence_term_hit": 0.0, "exemplar_hit": 0.0,
                "directionality_match": 0.5, "negative_penalty": 0.0,
                "status_factor": 1.0, "label_sim": 0.0,
                "final_score": 0.0}
    ...
    _ev_entities   = (_extract_named_entities(label) | _extract_named_entities(ev_text))
    _slot_entities = (_extract_named_entities(slot.label) | _extract_named_entities(slot_text))
    ...
    entity_sim = (len(_ev_entities & _slot_entities) / len(_ev_entities | _slot_entities)
                  if (_ev_entities | _slot_entities) else 0.0)

    _ev_actions   = (_extract_action_terms(_label_for_topic) | _extract_action_terms(ev_text))
    _slot_actions = (_extract_action_terms(slot.label) | _extract_action_terms(slot_text)
    ...
    action_sim = (len(_ev_actions & _slot_actions) / len(_ev_actions | _slot_actions)
    ...
    horiz_sim = 1.0 if _hf in ('inside', '') else 0.7 if _hf == 'partial' else 0.3 if _hf == 'beyond' else 0.5

    _vocab = build_slot_vocabulary_profile(slot)
    _ev_all_tokens = (set(re.findall(r'[a-z]{3,}', ev_text.lower())) |
                      set(re.findall(r'[a-z]{3,}', _label_for_topic.lower())))
    _ev_all_tokens -= frozenset({'the','a','an','of','in','on','at','to','for',
                                  'and','or','is','are','be','will','with','by'})

    _et_hits = sum(1 for _et in _vocab['evidence_terms']
                   if any(tok in _ev_all_tokens for tok in re.findall(r'[a-z]{3,}', _et.lower())))
    evidence_term_hit = (min(1.0, _et_hits / max(1, len(_vocab['evidence_terms'])))
                         if _vocab['evidence_terms'] else 0.0)

    exemplar_hit = 0.0
    for _ae in _vocab['action_exemplars']:
        _ae_toks = set(re.findall(r'[a-z]{3,}', _ae.lower()))
        if len(_ae_toks & _ev_all_tokens) >= 2:
            exemplar_hit = 1.0
            break

    _ev_dir = str((ev.get('metadata') or {}).get('directionality', '') or '').lower()
    _slot_dir = (_vocab['directionality'] or '').lower()
    directionality_match = (1.0 if (_slot_dir and _ev_dir and _slot_dir == _ev_dir)
                            else 0.5 if not _slot_dir or not _ev_dir else 0.0)

    _neg_overlap = len(_vocab['negative_tokens'] & _ev_all_tokens)
    negative_penalty = min(0.20, _neg_overlap * 0.05)
    ...
    base_score = (0.22 * entity_sim + 0.17 * action_sim + 0.12 * affinity
                  + 0.12 * rat_sim + 0.10 * evidence_term_hit + 0.08 * exemplar_hit
                  + 0.08 * hint_sim + 0.05 * directionality_match
                  + 0.05 * horiz_sim + 0.01 * label_sim) * status_factor - negative_penalty

    return {
        "entity_sim":         round(entity_sim, 3),
        "action_sim":         round(action_sim, 3),
        "affinity":           round(affinity, 3),
        "rat_sim":            round(rat_sim, 3),
        "hint_sim":           round(hint_sim, 3),
        "horiz_sim":          round(horiz_sim, 3),
        "evidence_term_hit":  round(evidence_term_hit, 3),
        "exemplar_hit":       round(exemplar_hit, 3),
        "directionality_match": round(directionality_match, 3),
        "negative_penalty":   round(negative_penalty, 3),
        "status_factor":      round(status_factor, 3),
...
def compute_topic_relevance_score(slot, ev: dict, rationale: str = "") -> float:
    """Public API — delegates to compute_topic_relevance_components."""
    return compute_topic_relevance_components(slot, ev, rationale)["final_score"]
    ...
    # Phase 9.8: Slot Vocabulary Bridge signals (GPT guidance).
    # build_slot_vocabulary_profile gives us a canonical normalized token set
    # covering the slot's full evidence vocabulary, not just its label.
    # This is what allows "steady cadence" to match incremental_development and
    # "technical disruption" to match slowed_or_disrupted_development even when
    # the token Jaccard between event label and slot label is low.
    _vocab = build_slot_vocabulary_profile(slot)
    _ev_all_tokens = set(re.findall(r'[a-z]{3,}', ev_text.lower())) | \
                     set(re.findall(r'[a-z]{3,}', _label_for_topic.lower()))
    _ev_all_tokens -= frozenset({'the','a','an','of','in','on','at','to','for',
    ...
    # evidence_term_hit: any evidence_term from the slot profile appears in the event text?
    _et_hits = sum(1 for _et in _vocab['evidence_terms']
                   if any(tok in _ev_all_tokens
                          for tok in re.findall(r'[a-z]{3,}', _et.lower())))
    _evidence_term_hit = min(1.0, _et_hits / max(1, len(_vocab['evidence_terms']))) \
                         if _vocab['evidence_terms'] else 0.0

    # exemplar_hit: any action_exemplar shares ≥ 2 tokens with the event?
    _exemplar_hit = 0.0
    for _ae in _vocab['action_exemplars']:
        _ae_toks = set(re.findall(r'[a-z]{3,}', _ae.lower()))
        if len(_ae_toks & _ev_all_tokens) >= 2:
    ...
    # directionality_match: does the event's directionality match the slot's?
    _ev_dir = str((ev.get('metadata') or {}).get('directionality', '') or '').lower()
    _slot_dir = (_vocab['directionality'] or '').lower()
    _directionality_match = (1.0 if (_slot_dir and _ev_dir and _slot_dir == _ev_dir)
                             else 0.5 if not _slot_dir or not _ev_dir
                             else 0.0)

    # negative_example_penalty: event shares vocabulary with negative_examples → lower score
    _neg_overlap = len(_vocab['negative_tokens'] & _ev_all_tokens)
    _negative_penalty = min(0.20, _neg_overlap * 0.05)  # capped at −0.20

    # Resolve-event evidence relevance (GPT Phase 7 guidance, point 4)
    _ev_kind_for_sf = str((ev.get('metadata') or {}).get('event_kind', '')
                          or ev.get('event_kind', '') or '').lower()
    _is_resolved = ev.get('temporal_status') in ('resolved', 'historical')
    if _is_resolved and _ev_kind_for_sf == 'official_action':
    ...
    # matches remain a floor when slot descriptions are generic.
    label_sim = _jaccard_sim(slot.label, _label_for_topic)

    # Phase 9.8 revised formula: vocabulary bridge adds evidence_term_hit (0.10),
    # exemplar_hit (0.08), directionality_match (0.05), negative_penalty (−).
    # Existing weights compressed proportionally to accommodate new signals.
    base_score = (0.22 * _entity_sim
                  + 0.17 * _action_sim
                  + 0.12 * affinity
                  + 0.12 * rat_sim
                  + 0.10 * _evidence_term_hit
                  + 0.08 * _exemplar_hit
                  + 0.08 * hint_sim
                  + 0.05 * _directionality_match
                  + 0.05 * _horiz_sim
                  + 0.01 * label_sim) * _status_factor - _negative_penalty
...
def _rationale_overlaps_slot(rationale: str, slot) -> bool:
    """
    Phase 6.7 (GPT guidance): check whether Stage C's stated rationale engages
    with THIS slot's proposition — a guard against high-confidence relations
    without grounding.

    Phase 9.8 (Slot Vocabulary Bridge): extended to also check rationale against
    the slot's evidence_terms and action_exemplars, not just label/description.
    "Maintained steady cadence" in a rationale should overlap with
    incremental_development's evidence_terms, even if the raw Jaccard against
    the slot label is low.
    """
    if not rationale:
        return False
    slot_text = _slot_text_field(slot)
    # Base Jaccard checks (existing)
    sim = max(
    ...
    if sim >= 0.10:
        return True
    # Phase 9.8: vocabulary bridge — check evidence_terms and action_exemplars
    _vocab = build_slot_vocabulary_profile(slot)
    _rat_tokens = set(re.findall(r'[a-z]{3,}', rationale.lower()))
    _rat_tokens -= frozenset({'the','a','an','of','in','on','at','to','for',
                               'and','or','is','are','be','will','with','by'})
    # evidence_term match: any evidence term has substantial overlap with rationale
    for _et in _vocab['evidence_terms']:
        _et_toks = set(re.findall(r'[a-z]{3,}', _et.lower()))
        if len(_et_toks & _rat_tokens) >= 2:
            return True
    # exemplar match: any exemplar shares 2+ tokens with rationale
    for _ae in _vocab['action_exemplars']:
        _ae_toks = set(re.findall(r'[a-z]{3,}', _ae.lower()))
        if len(_ae_toks & _rat_tokens) >= 2:
            return True
    # positive vocabulary token overlap
    if len(_vocab['positive_tokens'] & _rat_tokens) >= 3:
        return True
    return False
...
def _sync_rep_to_event_dict(rep: dict, event_dicts: list) -> None:
    """
    Phase 10.3 (GPT guidance): shared helper to sync all display/structure fields
    from a representative object back to its original entry in event_dicts.

    Secondary Gate B candidates are added via dict(original) shallow copy.
    Any field mutated on the copy after that point is NOT visible to
    build_seeded_events_from_dicts, which reads from the originals. This helper
    closes that gap for ALL display fields in one place, rather than patching
    field-by-field each time a new field surfaces the same issue.

    Fields synced: label, probability, metadata, branch_group, branch_label,
    outcome_role, canonical_slot_id, branch_probability, slot_raw_probability,
    positive_framing_label, mapped_event_ids, evidence_composition.
    """
    _rep_id = rep.get("id")
    if not _rep_id:
        return
    _sync_fields = (
        "label", "probability", "metadata",
        "branch_group", "branch_label", "outcome_role", "canonical_slot_id",
        "branch_probability", "slot_raw_probability",
        "positive_framing_label", "mapped_event_ids", "evidence_composition",
    )
    for _ev_orig in event_dicts:
        if _ev_orig.get("id") == _rep_id and _ev_orig is not rep:
...
def normalize_and_aggregate_primary_slots(
    ...
    slot_contributions: list = []  # Phase 8.11: SlotContribution records (looser standard,
                                    # does not become representative — see Contributor Gate).
                                    # Bugfix: must be initialized here, BEFORE the Step 1
                                    # matching loop that appends to it — it was previously
                                    # initialized in Step 6 (much later in this function),
                                    # causing UnboundLocalError the moment any Contributor
                                    # Gate evaluation ran before that point was reached.
    ...
    def _register_effective_prob(sid, ev, member=None):
        ...
        if should_invert:
            ...
            # Phase 8: double-negation risk flag. If the event's OWN label already
            # contains negation language (e.g. "Will Not Occur") AND it's being
            # inverted, there's elevated risk Stage A misjudged which proposition
            # the raw probability refers to. Flag for manual review rather than
            # silently trusting the inversion.
            _label_has_negation = _is_direct_negation(ev_label, '', None)
            if _label_has_negation:
                md['double_negation_risk'] = True
                print(f"[v0.12] ⚠ Slot '{sid}': event {ev.get('id')} label already contains "
                      f"negation ('{ev_label[:60]}...') AND is being inverted — "
                      f"double-negation risk. Verify raw probability {orig:.2f} is calibrated "
                      f"to the negated proposition, not the underlying claim. "
                      f"Resulting slot probability: {eff:.2f}")
            ...
            if _engine_score >= 0.40:
                ...
                if not _accept:
                    if (_relation in ('supports', 'opposes')
                            ...
                            and _ev_kind not in ('meta_signal', 'causal_context', 'evidence_context', 'reflexive_market_signal')
                            and _rationale_overlaps_slot(_rationale, _slot_obj)):
                        _accept = True
                        _accept_reason = f"relation_to_slot={_relation}"
                    # Near-miss path: score very close to Gate A (>=0.35) with strong Stage C
                    # confidence (>=0.80) — relax rationale_overlap. Stage C's semantic judgment
                    # is the primary signal when the lexical score is near-threshold.
                    elif (_relation in ('supports', 'opposes')
                              and _rel_conf >= 0.80
                              and _engine_score >= 0.35
                              and _scope_compat_b >= 0.60
                              and _ev_kind not in ('meta_signal', 'causal_context', 'evidence_context', 'reflexive_market_signal')):
                        _accept = True
                        _accept_reason = f"near_gate_a(score={_engine_score:.3f},conf={_rel_conf:.2f})"
                    # Threshold-meta path: for threshold_bucket slots, meta_signal events
                    # (Polymarket, Fed forecasts, bank projections) ARE the primary evidence type.
                    # Gate B's meta_signal exclusion blocks all meaningful threshold evidence.
                    # Allow meta_signal through for threshold_bucket slots with high confidence.
                    elif (_ev_kind in ('meta_signal', 'forecast_signal')
                              and _slot_obj is not None
                              and getattr(_slot_obj, 'slot_kind', '') == 'threshold_bucket'
                              and _rel_conf >= 0.75
                              and _engine_score >= 0.20
                              and _scope_compat_b >= 0.60):
                        _accept = True
                        _accept_reason = f"threshold_meta(conf={_rel_conf:.2f},score={_engine_score:.3f})"
                    # Phase 10 (GPT guidance): threshold_forecast_soft.
                    # Narrow soft path for forecast_signal evidence on threshold_bucket slots.
                    # threshold_meta requires rel_conf >= 0.75 — too strict for abstract forecast
                    # signals ("30% chance of two cuts") which are the natural evidence type for
                    # rate/threshold scenarios but naturally receive lower Stage C confidence.
                    # Does NOT include reflexive_market_signal — betting/market signals remain
                    # subject to stricter threshold_meta requirements.
                    # Requires at least one bucket-specific signal to prevent over-acceptance.
                    elif (_ev_kind in ('forecast_signal', 'structured_forecast_signal')
                              and _slot_obj is not None
                              and getattr(_slot_obj, 'slot_kind', '') == 'threshold_bucket'
                              and _relation in ('supports', 'opposes')
                              and _rel_conf >= 0.60
                              and _engine_score >= 0.18
                              and _scope_compat_b >= 0.60):
                        _comps_tfs = compute_topic_relevance_components(
                            _slot_obj, ev, rationale=_member_rationale)
                        _bucket_signals = [
                            _comps_tfs.get('affinity', 0.0) >= 0.20,
                            _comps_tfs.get('hint_sim', 0.0) >= 0.10,
                            _comps_tfs.get('rat_sim', 0.0) > 0.0,
                            _comps_tfs.get('evidence_term_hit', 0.0) > 0.0,
                        ]
                        if any(_bucket_signals):
                            _accept = True
                            _accept_reason = (f"threshold_forecast_soft(conf={_rel_conf:.2f},"
                                              f"score={_engine_score:.3f},"
                                              f"signals={sum(1 for b in _bucket_signals if b)})")
                    # Phase 9.11 (GPT guidance): Semantic Bridge Rescue.
                    # When Stage C has sufficient confidence and vocabulary-bridge signals
                    # confirm semantic alignment, rescue mappings where Jaccard-sparse engine
                    # scores fall below 0.40.
                    if not _accept and _engine_score >= 0.22:
                        _comps = compute_topic_relevance_components(
                            _slot_obj, ev, rationale=_member_rationale)
                        _bridge_signals = [
                            _comps.get('rat_sim', 0.0) > 0.0,
                            _comps.get('evidence_term_hit', 0.0) > 0.0,
                            _comps.get('exemplar_hit', 0.0) > 0.0,
                            _comps.get('affinity', 0.0) >= 0.30,
                            _comps.get('directionality_match', 0.0) == 1.0,
                        ]
                        _bridge_count = sum(1 for b in _bridge_signals if b)
                        _rel_in_soc = _relation in ('supports', 'opposes', 'context')
                        _kind_ok = _ev_kind not in (
                            'irrelevant', 'evidence_context', 'reflexive_market_signal')
                        if (_rel_in_soc and _rel_conf >= 0.65 and _scope_compat_b >= 0.60
                                and _kind_ok and _bridge_count >= 1):
                            _accept = True
                            _accept_reason = (f"semantic_bridge(conf={_rel_conf:.2f},"
                                              f"score={_engine_score:.3f},bridges={_bridge_count})")
                        elif (_rel_in_soc and 0.60 <= _rel_conf < 0.65 and _scope_compat_b >= 0.60
                                  and _kind_ok and _engine_score >= 0.28 and _bridge_count >= 2):
                            _accept = True
                            _accept_reason = (f"semantic_bridge_strict(conf={_rel_conf:.2f},"
                                              f"score={_engine_score:.3f},bridges={_bridge_count})")
                        if _accept:
                            print(f"[Phase 9.11] Semantic Bridge Rescue: {eid} -> '{matched_sid}' "
                                  f"entity={_comps.get('entity_sim',0):.3f} "
                                  f"action={_comps.get('action_sim',0):.3f} "
                                  f"et_hit={_comps.get('evidence_term_hit',0):.1f} "
                                  f"ex_hit={_comps.get('exemplar_hit',0):.1f} "
                                  f"rat={_comps.get('rat_sim',0):.3f} "
                                  f"bridges={_bridge_count}")

            if not _accept:
                # Phase 9.7 (GPT guidance): log all score components for rejected
                # candidates so we can diagnose whether failures are from
                # low relation_confidence, rationale_overlap, scope, engine_score,
                # event_kind exclusion, or token/omission in candidate_evaluations.
                _b_why = []
                ...
                # Compute the individual match-score sub-components for the rejection record
                _hint_s   = _slot_obj.canonical_slot_hint_score if _slot_obj and hasattr(_slot_obj, 'canonical_slot_hint_score') else 0.0
                ...
                print(f"[Phase 9.7] Rejection score components: slot={matched_sid} event={eid} "
                      f"rel={_relation!r} rel_conf={_rel_conf:.3f} engine_score={_engine_score:.3f} "
                      f"scope_compat={_scope_compat_b if isinstance(_scope_compat_b, float) else 1.0:.3f} "
                      f"event_kind={_ev_kind!r}")
            else:
                ...
                # Phase 8: note if representative is a resolved historical event.
                # Resolved events inflate branch probability to near-certainty.
                # The Stage C prompt now instructs Stage C to prefer unresolved events,
                # so this flag is diagnostic — the issue should reduce over time.
                _ev_ts = str(ev.get('temporal_status', '') or
                             ev.get('metadata', {}).get('temporal_status', '') or '').lower()
                if _ev_ts in ('resolved', 'historical') and pos == 1:
                    ev.setdefault('metadata', {})['resolved_as_representative'] = True
                    print(f"[v0.12] Slot '{matched_sid}': resolved event {eid} used as "
                          f"primary representative (p={ev.get('probability','?')}) — "
                          f"branch probability may be inflated; prefer unresolved alternatives.")

                # Phase 9.3: persist accept_reason and scope_compat into metadata so
                # later SlotAssignment construction can read them back for the
                # source-coherence penalty computation (actor-scope warning,
                # threshold-bucket-alternative warning).
                ev.setdefault('metadata', {})[f'accept_reason__{matched_sid}'] = str(_accept_reason or '')
                ev['metadata'][f'scope_compat__{matched_sid}'] = float(
                    locals().get('_scope_compat_b', locals().get('_scope_compat', 1.0)) or 1.0)
        ...
        if candidate_ids and isinstance(candidate_ids, list):
            ...
            for cand_id in candidate_ids[1:]:
                ...
                if _cand_score >= 0.40:
                    ...
                    # Bugfix (found via testing): relation_to_slot/relation_confidence/
                    # event_kind/slot_relevance_rationale were being read off cand_ev
                    # (the RAW Stage A event dict), but Stage C only ever populated these
                    # fields on the SLOT-SPECIFIC best_event_id mapping — never on any
                    # other candidate_event_ids entry. Every secondary candidate was
                    # therefore reading empty/zero relation data by construction,
                    # regardless of gate threshold. This is why secondary Gate B (and
                    # now the Contributor Gate) rejected essentially everything: the
                    # data needed to evaluate them was never actually present.
                    # Fix: read from member['candidate_evaluations'][cand_id] (Phase 8.11
                    # schema addition) first; fall back to cand_ev for older/non-compliant
                    # Stage C responses so this degrades gracefully rather than breaking.
                    _cand_eval = {}
                    _all_evals = member.get('candidate_evaluations')
                    if isinstance(_all_evals, dict):
                        _cand_eval = _all_evals.get(cand_id) or _all_evals.get(str(cand_id)) or {}
                    if not isinstance(_cand_eval, dict):
                        _cand_eval = {}

                    _s_rel   = str(_cand_eval.get('relation_to_slot', '') or
                                   cand_ev.get('relation_to_slot', '') or '').lower()
                    _s_rconf = float(_cand_eval.get('relation_confidence') or
                                     cand_ev.get('relation_confidence') or
                                     cand_ev.get('match_confidence') or 0.0)
                    _s_kind  = str(_cand_eval.get('event_kind', '') or
                                   cand_ev.get('event_kind', '') or '').lower()
                    _s_rat   = str(_cand_eval.get('slot_relevance_rationale', '') or
                                   cand_ev.get('slot_relevance_rationale', '') or '')
                    if not _cand_eval:
                        print(f"[v0.12] Note: no candidate_evaluations entry for {cand_id} "
                              f"on slot '{matched_sid}' — Stage C did not provide per-candidate "
                              f"relation data; falling back to raw event fields (likely empty)")
                    _s_scope = _compute_scope_compatibility(_slot_obj_s, cand_ev) if _slot_obj_s else 1.0
                    # Phase 8: relaxed from 0.80 to 0.70 — primary rejection should not
                    # trigger stricter standards for the next alternative candidate.
                    _slot_kind_s = getattr(_slot_obj_s, 'slot_kind', '') if _slot_obj_s else ''
                    if (_s_rel in ('supports', 'opposes')
                            and _s_rconf >= 0.70
                            and _cand_score >= 0.16
                            and _s_scope >= 0.60
                            and _s_kind not in ('meta_signal', 'causal_context', 'evidence_context', 'reflexive_market_signal')
                    ...
                    elif (_s_kind in ('meta_signal', 'forecast_signal')
                              and _slot_kind_s == 'threshold_bucket'
                              and _s_rconf >= 0.70 and _cand_score >= 0.20
                              and _s_scope >= 0.60):
                        _cand_accepted = True
                        _cand_reason = f"secondary_threshold_meta(conf={_s_rconf:.2f})"
                    else:
                        ...
                        # Phase 8.11 (GPT guidance): Contributor Gate — a deliberately looser
                        # standard than representative Gate B. An event that can't stand as
                        # the slot's REPRESENTATIVE may still legitimately CONTRIBUTE evidence.
                        # This does not touch `assignments[sid]` (representative selection) —
                        # it produces a separate SlotContribution record consumed only by
                        # compute_slot_contribution_adjusted_support() as a shadow diagnostic.
                        _contrib_neg_ok = True
                        if _s_rel == 'opposes':
                            _contrib_neg_ok = _is_direct_negation(
                                str(cand_ev.get('label', '') or ''), _s_rat,
                                cand_ev.get('direct_negation'))
                        if (_s_rel in ('supports', 'opposes', 'context')
                                and _s_rel != 'alternative_pathway'
                                and _contrib_neg_ok
                                and _s_scope >= 0.60
                                and _s_rconf >= 0.55
                                and (_cand_score >= 0.12
                                     or (_slot_obj_s and _rationale_overlaps_slot(_s_rat, _slot_obj_s)))
                                and _s_kind not in ('irrelevant', 'evidence_context', 'reflexive_market_signal')):
                            _contrib_fam = _compute_source_family_key(
                                str(cand_ev.get('publisher', '') or cand_ev.get('source_publisher', '') or ''),
                                _s_kind, str(cand_ev.get('query_purpose', '') or ''))
                            # Down-weight if this source family already contributed to this slot
                            _existing_fams = {c.evidence_family_key for c in slot_contributions
                                              if c.slot_id == matched_sid}
                            _dup_family = _contrib_fam in _existing_fams
                            _contrib_weight = (0.5 if _dup_family else 1.0) * _s_rconf * max(0.2, _cand_score)
                            _eff_prob_c = float(cand_ev.get('probability', 0.5) or 0.5)
                            if _s_rel == 'opposes':
                                _eff_prob_c = 1.0 - _eff_prob_c
                            # Phase 9.3: threshold-bucket-alternative warning proxy —
                            # a threshold_bucket slot mapped via meta/forecast-type
                            # evidence signals the kind of disambiguation-dependent
                            # mapping GPT's penalty is meant to catch.
                            _thresh_warn = (
                                getattr(_slot_obj_s, 'slot_kind', '') == 'threshold_bucket'
                                and _s_kind in ('meta_signal', 'forecast_signal', 'reflexive_market_signal')
                            )
                            slot_contributions.append(SlotContribution(
                                slot_id=matched_sid,
                                event_id=str(cand_ev.get('id', '') or cand_id),
                                source_event_id=str(cand_id),
                                relation_to_slot=_s_rel,
                                relation_confidence=_s_rconf,
                                engine_score=_cand_score,
                                event_kind=_s_kind or 'substantive_development',
                                effective_probability=_eff_prob_c,
                                contribution_weight=round(_contrib_weight, 4),
                                source_family_key=_contrib_fam,
                                evidence_family_key=_contrib_fam,
                                accepted_by='contributor_gate',
                                rationale=_s_rat[:150],
                                scope_compat=float(_s_scope or 1.0),
                                threshold_alt_warning=_thresh_warn,
                            ))
                            print(f"[Phase 8.11] Contributor Gate accepted {cand_id} -> slot "
                                  f"'{matched_sid}' as CONTRIBUTION (not representative): "
                                  f"weight={_contrib_weight:.3f}, family={_contrib_fam}"
                                  f"{' (dup family, down-weighted)' if _dup_family else ''}")
                        else:
                            # Phase 8.11 bugfix: this case was previously completely silent —
                            # the Contributor Gate evaluated and rejected with no log output,
                            # making it indistinguishable from "never reached." Show exactly
                            # which condition failed so this is diagnosable going forward.
                            _fail_reasons = []
                            if _s_rel not in ('supports', 'opposes', 'context') or _s_rel == 'alternative_pathway':
                                _fail_reasons.append(f"relation='{_s_rel}'")
                            if not _contrib_neg_ok:
                                _fail_reasons.append("opposes-without-direct-negation")
                            if _s_scope < 0.60:
                                _fail_reasons.append(f"scope_compat={_s_scope:.2f}<0.60")
                            if _s_rconf < 0.55:
                                _fail_reasons.append(f"relation_confidence={_s_rconf:.2f}<0.55")
                            if _cand_score < 0.12 and not (_slot_obj_s and _rationale_overlaps_slot(_s_rat, _slot_obj_s)):
                                _fail_reasons.append(f"engine_score={_cand_score:.2f}<0.12 and no rationale overlap")
                            if _s_kind in ('irrelevant', 'evidence_context', 'reflexive_market_signal'):
                                _fail_reasons.append(f"event_kind='{_s_kind}' excluded")
                            print(f"[Phase 8.11] Contributor Gate rejected {cand_id} -> slot "
                                  f"'{matched_sid}': {', '.join(_fail_reasons) if _fail_reasons else 'unknown'}")
                if _cand_accepted:
    ...
    for slot in slots:
        ...
        # Phase 7 continuation: also allow synthetics for continuation/status-quo
        # slots that Stage Q classified as 'outcome' rather than 'status_quo'.
        # The status quo is logically required — if nothing else maps to it, it
        # should have a residual placeholder just like explicit status_quo slots.
        _is_continuation_slot = (
            getattr(slot, 'directionality', '') == 'continuation'
            or slot.slot_kind == 'status_quo'
            or any(t in slot.slot_id.lower() for t in ('continu', 'status_quo', 'status quo', 'no_change'))
            or any(t in slot.label.lower() for t in ('continued', 'continuation', 'status quo', 'no major'))
        )
        if not slot.synthetic_allowed and not _is_continuation_slot:
            unmapped_slots.append(slot.slot_id)
            print(f"[v0.12] Slot '{slot.slot_id}' — no organic match (synthetic_allowed=False), "
                  f"leaving unmapped")
            continue
        if not slot.synthetic_allowed and _is_continuation_slot:
            print(f"[v0.12] Slot '{slot.slot_id}' — continuation slot, "
                  f"creating synthetic despite synthetic_allowed=False")
    ...
    for slot in claim_order:
        ...
        if unclaimed:
            ...
            # Bugfix (found via testing — Fed Rate Cuts, two slots both showing
            # 72% from the SAME source event "Markets Price In Zero Fed Rate
            # Cuts"): the slot-view clone preserved the WINNING slot's raw
            # probability for the LOSING slot too, even though that probability
            # describes evidence for a DIFFERENT, often mutually-exclusive
            # outcome. For Fed Rate Cuts specifically, "zero cuts priced in"
            # does not support "moderate cuts" at 72% — it actively argues
            # against it. Displaying the same 72% for both produced a
            # misleading branch group where two contradictory outcomes shared
            # near-equal probability shares, as flagged by Vince directly.
            #
            # This was always meant to be a short-term patch (Phase 6.7, GPT
            # guidance) pending something better. Rather than build the full
            # SlotAssignment-view rendering layer GPT described (which would
            # require reworking how the Review Room renders branch groups from
            # EventNode.branch_group, a much larger change), this fix reuses
            # the EXISTING, well-tested synthetic-placeholder pathway: when a
            # slot's only candidate evidence was claimed by a higher-priority
            # slot, that slot genuinely has no UNIQUE evidence of its own —
            # treat it exactly like "no organic match" (same decision tree:
            # synthetic_allowed / continuation-slot / series-prior inheritance,
            # or leave honestly unmapped) rather than borrowing a probability
            # that describes something else.
            _is_continuation_slot_cv = (
                getattr(slot, 'directionality', '') == 'continuation'
                or slot.slot_kind == 'status_quo'
                or any(t in slot.slot_id.lower() for t in ('continu', 'status_quo', 'status quo', 'no_change'))
                or any(t in slot.label.lower() for t in ('continued', 'continuation', 'status quo', 'no major'))
            )
            if not slot.synthetic_allowed and not _is_continuation_slot_cv:
                unmapped_slots.append(sid)
                # Bugfix: must clear assignments[sid] here — it's still non-empty
                # (Stage C DID map something to this slot, it just got claimed by
                # a higher-priority slot first). Without clearing it, the downstream
                # loop at Step 3 (`if not evs: continue; rep = slot_rep[sid]`) would
                # see evs non-empty for this slot but find no slot_rep entry for it
                # (since we never reach the `slot_rep[sid] = rep` assignment below),
                # causing a KeyError.
                assignments[sid] = []
                print(f"[v0.12] Slot '{sid}': only candidate evidence was claimed by a "
                      f"higher-priority slot — no unique evidence remains, leaving unmapped "
                      f"(previously would have cloned the other slot's probability)")
                continue
            _claim_synth_id = "EVT_SYNTH_" + _uuid_agg.uuid4().hex[:6].upper()
            _or_role_cv = "status_quo_outcome" if slot.slot_kind == "status_quo" else "primary_outcome"
            _series_prior_cv = (prior_slot_probs or {}).get(sid)
            _synth_prob_cv = float(_series_prior_cv) if _series_prior_cv is not None else 0.08
            _reason_cv = ("logical_complement_residual_prior" if _series_prior_cv is None
                          else "series_prior_inherited")
            rep = {
                "id": _claim_synth_id, "label": slot.label, "probability": _synth_prob_cv,
                "time_estimate": 2, "time_uncertainty": 0.4,
                "temporal_status": "unresolved",
                "branch_group": bg_id, "branch_label": slot.label,
                "outcome_role": _or_role_cv, "disruption_score": 0.2,
                "record_coherence": 0.5, "categories": {},
                "canonical_slot_id": sid,
                "metadata": {
                    "synthetic": "true",
                    "synthetic_reason": _reason_cv,
                    "slot_id": sid, "slot_polarity": slot.slot_polarity,
                    "claim_collision_with": str(max(evs, key=_coherence_key).get('id', '')),
                },
            }
            event_dicts.append(rep)
            claimed_rep_ids.add(rep['id'])
            # Bugfix: assignments[sid] still holds the ORIGINAL claimed-elsewhere
            # entry at this point (Stage C's mapping, before we knew it would lose
            # the collision). Step 4's probability aggregation reads from
            # assignments[sid], not slot_rep[sid] — without replacing it here,
            # the displayed representative would be this new synthetic while the
            # AGGREGATED PROBABILITY would still be computed from the stale,
            # claimed-elsewhere event. Replace, don't append, since this synthetic
            # is the slot's only representative now.
            assignments[sid] = [rep]
            print(f"[v0.12] Slot '{sid}': only candidate evidence was claimed by a "
                  f"higher-priority slot — created synthetic {_claim_synth_id} "
                  f"(p={_synth_prob_cv:.2f}, reason={_reason_cv}) instead of cloning "
                  f"the other slot's probability")
        slot_rep[sid] = rep
    ...
    for slot in slots:
        ...
        evs   = assignments[sid]
        if not evs: continue
        # Bugfix (root cause of "slot mapped in Step 2 but missing from primary
        # branch group"): slot_rep[sid] is only populated for slots that went
        # through the Step 1 claim loop (those that already had candidates at
        # that point). Step 2 (semantic search) adds events to assignments[sid]
        # AFTER the claim loop, but never adds a slot_rep entry — so Step 6 would
        # throw a KeyError on slot_rep[sid] for any Step-2-found representative.
        # Python's dict lookup raises KeyError, which is caught by whatever try/
        # except wraps this block, silently dropping the slot from the branch group.
        # Fix: fall back to selecting the best event directly from assignments[sid]
        # when slot_rep has no entry. This path is also taken for synthetic events
        # created in Steps 3-5 which similarly bypass the claim loop.
        if sid not in slot_rep:
            slot_rep[sid] = max(evs, key=_coherence_key)
        rep = slot_rep[sid]
        ...
        # Phase 10.3: use shared helper to sync ALL display/structure fields back
        # to the original event_dicts entry — replaces the field-by-field sync
        # patches from Phase 9.10 (branch_group family) and Phase 10.2 (probability/
        # label/metadata family). Prevents the same shallow-copy bug from resurfacing
        # when future fields are added to representatives.
        _sync_rep_to_event_dict(rep, event_dicts)
        ...
        if _ec:
            ...
            if _dom not in ("substantive", "official", ""):
                ...
                print(f"[Phase 8 diag] evidence_basis_note SET on rep {rep.get('id')} "
                      f"for slot '{sid}': dominant_evidence_kind={_dom!r}")
            else:
                print(f"[Phase 8 diag] evidence_basis_note NOT triggered for slot '{sid}' "
                      f"(dominant_evidence_kind={_dom!r}, rep={rep.get('id')})")
        ...
        _sa = SlotAssignment(
            ...
            # Phase 9.3: source-coherence penalty inputs, read back from the
            # metadata persisted at acceptance time (see Step 1 matching loop).
            scope_compat=float(_sa_md.get(f'scope_compat__{sid}', 1.0) or 1.0),
            threshold_alt_warning=any(
                _tag in str(_sa_md.get(f'accept_reason__{sid}', '') or '')
                for _tag in ('gate_t_threshold', 'threshold_meta')
            ),
        )
        ...
        if _rep_md.get('inverted_for_slot') == sid:
            ...
            # Phase 10.3: replaced the field-by-field sync from Phase 10.2 with
            # the shared _sync_rep_to_event_dict helper, which covers all display
            # fields at once rather than patching individual fields incrementally.
            _sync_rep_to_event_dict(rep, event_dicts)
    ...
    # Phase 8.10 (GPT guidance): if any mapped event was flagged as a possible
    # constructed-negated-slot-label, downgrade quality to amber regardless of
    # organic_rate — this is a reliability concern about HOW evidence was
    # described, not just whether evidence was found.
    _constructed_negation_count = 0
    for _sid_check, _evs_check in assignments.items():
        for _ev_check in (_evs_check or []):
            if isinstance(_ev_check, dict) and (_ev_check.get('metadata', {}) or {}).get('constructed_negated_slot_label'):
                _constructed_negation_count += 1
    if _constructed_negation_count > 0 and quality_flag == "green":
        quality_flag = "amber"
        print(f"[Phase 8.10] Seed quality downgraded to amber: "
              f"{_constructed_negation_count} constructed_negated_slot_label flag(s) present")
    seed_quality["constructed_negation_count"] = _constructed_negation_count
    # Phase 8.11: contribution counts for diagnostic visibility (not UI yet)
    seed_quality["contribution_count"] = len(slot_contributions)
    seed_quality["contribution_family_count"] = len({c.evidence_family_key for c in slot_contributions})
    ...
    # Phase 10.3 (GPT guidance): Stage A partition behavior detection.
    # For binary and threshold_bucket question types, Stage A naturally produces a
    # posterior distribution over the closed outcome set rather than independent
    # evidence-level estimates — the raw representative probabilities sum to ~1.0
    # regardless of prompt instructions. This is not a prompt failure; it is a
    # structural property of how language models handle mutually exclusive,
    # exhaustive outcome sets. Log it explicitly so the epistemic meaning is honest.
    def _synthetic_or_unmapped(sid, _assignments):
        """Return True if slot has no organic representative (synthetic or unmapped)."""
        evs = list(_assignments.get(sid) or [])
        if not evs:
            return True  # unmapped
        rep_ev = max(evs, key=lambda e: float(e.get('rank_weight', 0.0) or 0.0))
        rc = float(rep_ev.get('relation_confidence', 1.0) or 1.0)
        es = float(rep_ev.get('engine_score', 1.0) or 1.0)
        return rc == 0.0 and es == 0.0  # synthetic signature

    _q_type_for_pb = getattr(primary_question, 'question_type', '') or ''
    _partition_q_types = ('binary', 'threshold_bucket', 'timeline')
    if _q_type_for_pb in _partition_q_types and raw_probs:
        _organic_raw_probs = {_sid: _p for _sid, _p in raw_probs.items()
                               if _p is not None and not _synthetic_or_unmapped(_sid, assignments)}
        _rep_raw_sum = sum(_organic_raw_probs.values()) if _organic_raw_probs else 0.0
        if _organic_raw_probs and 0.97 <= _rep_raw_sum <= 1.03:
            print(f"[Phase 10.3] Stage A partition behavior detected: "
                  f"raw representative probabilities sum to {_rep_raw_sum:.3f} "
                  f"for closed outcome set ({_q_type_for_pb}, {len(_organic_raw_probs)} slots). "
                  f"Stage A is behaving as a holistic branch-posterior estimator for this "
                  f"scenario type, not as an independent evidence scorer. "
                  f"This is expected and acceptable; probabilities are treated as a coherent "
                  f"branch distribution rather than independent confidence readings.")
        elif _organic_raw_probs:
            _deviation = abs(_rep_raw_sum - 1.0)
            if _deviation < 0.15:
                print(f"[Phase 10.3] Stage A partial partition behavior: "
                      f"sum={_rep_raw_sum:.3f} (deviation {_deviation:.3f} from 1.0) "
                      f"for {_q_type_for_pb} question type.")
    ...
    return event_dicts, {"slot_assignments": assignments, "slot_probabilities": normalized,
                          ...
                          "slot_contribution_records": slot_contributions,  # Phase 8.11
                          "seed_quality": seed_quality}
...
Given the question and time horizon, classify the question type and define the canonical outcome slots that directly and exhaustively answer the question within the horizon.
...
In each slot description, name the scenario's primary actors and concrete evidence patterns.

For threshold questions only, include: threshold_unit, lower_bound, upper_bound, aliases (natural-language phrases for the bucket).
Optional for all slots: actor_scope, geographic_scope, scale_level, directionality, action_exemplars (2-3 concrete example events), negative_examples (2-3 event types that do NOT support this slot), evidence_terms (vocabulary sources would actually use).
...
HORIZON-ENDPOINT FRAMING (for open_scenario and multi_outcome questions):
Canonical slots must represent the condition of the field AT the target horizon, not
merely a pathway that could lead to another slot.

If one proposed slot is mainly a cause, precursor, or pathway toward another, do not
make both primary slots unless they describe distinct endpoint states at the horizon.
If two outcomes can co-occur at the horizon, either merge them, sharpen them into
mutually distinct endpoint states, or treat one as causal/contextual evidence rather
than a primary branch slot.

Example: "Escalatory divergence" and "fragmentation into three blocs" can be distinct
endpoints if escalation dominates the horizon state vs. stable blocs have solidified.
But if escalation typically leads to fragmentation before the horizon, they may be the
same endpoint at different stages — in which case keep only the terminal state as a slot.

BEFORE RETURNING: perform two silent checks.

COMPLETENESS CHECK:
Ask: "What meaningfully distinct world-state could occur within the horizon that is not represented by these slots?"
Consider: continuation/status quo, acceleration, slowdown/disruption, negotiated/de-escalatory path, catastrophic accident or failure mode, surprise reversal, threshold tail outcome, null outcome.
Add a slot only if it represents a meaningfully distinct world-state with distinct evidence patterns.
For multi_outcome and open_scenario: prefer 4-5 slots; include a 6th only for a genuinely distinct tail-risk or failure-mode outcome.

OVERLAP CHECK:
Compare every pair of proposed slots. Ask: "Would the same body of evidence normally support both?"
If yes, either merge them or sharpen the distinction so each slot has distinct evidence patterns, action_exemplars, and negative_examples.
No two slots should consistently compete for the same representative evidence.
Example requiring differentiation: "fragmented status quo" vs "divergence into competing regimes" — unless you clearly distinguish active bloc-formation (divergence) from passive non-cooperation (status quo), they will receive the same evidence.
...
{{
  ...
  "design_notes": {{
    "completeness_check": "One sentence explaining why the answer space is complete.",
    "overlap_check": "One sentence confirming no two slots compete for the same evidence, or how overlapping slots were differentiated."
  }}
}}

Fields threshold_unit/lower_bound/upper_bound/aliases are only for threshold_bucket slots.
evidence_terms: vocabulary that sources would actually use (e.g. for a continuation slot: "steady pace", "routine testing", "incremental development").

IMPORTANT: For any slot representing continuation, status quo, stability, or non-change, always set slot_kind=status_quo.
Synthetic placeholders are allowed only for logically required complement slots (binary "no", status_quo, timeline "not within horizon"). Use a synthetic placeholder when no organic evidence maps to this slot, preserving it as a tracked outcome.
""".strip()
...
def run_stage_q(topic: str, horizon_label: str) -> 'PrimaryQuestion':
    ...
    result = call_anthropic_api(
        ...
        max_tokens=4000,  # Phase 9.11: raised from 2000 — richer slot definitions with
                          # evidence_terms, action_exemplars, negative_examples, and
                          # design_notes now produce responses of ~9000 chars (~2300 tokens).
        timeout=60,
    )

    if not isinstance(result, dict) or not result.get("canonical_slots"):
        # Defensive check: never fall back to binary for clearly open-scenario questions.
        # "how will", "what will", "which path", named alternatives → must not be binary.
        _topic_lower = topic.lower()
        _is_open = any(sig in _topic_lower for sig in (
            "how will", "how might", "how could", "what will", "what might",
            "which path", "which scenario", "what happens", "how does",
            "evolve", "trajectory", "scenarios", "outcomes",
        ))
        if _is_open:
            print("[v0.12] Stage Q: parse failed for open-scenario question — raising error "
                  "rather than defaulting to binary (binary would be structurally wrong)")
            raise RuntimeError(
                f"Stage Q returned invalid JSON for open-scenario question: {topic[:80]}. "
                "Cannot safely fall back to binary. Check Stage Q prompt or Haiku response."
            )
...
QUERY PROVENANCE DISCIPLINE (Phase 8.3):
The source set includes outcome-targeted searches for every canonical slot. Some events were retrieved specifically because a query targeted a particular slot.
Do NOT treat search origin as evidence that the event belongs to that slot.
Independently verify the event's actor scope, horizon, threshold bucket, relation_to_slot, and action pattern before mapping.
Query purpose is a retrieval hint only, not a mapping decision.
Example: a search for "Fed 200 bps cuts" may retrieve an article saying "markets do not expect 200 bps" — that event may belong to the no-cuts slot as opposing evidence, or to a different slot entirely. Stage C must reason independently.
...
REPRESENTATIVE EVENT SELECTION — critical for probability calibration:
When choosing the best_event_id for a slot, strongly prefer UNRESOLVED or ACTIVE events over RESOLVED ones.
A resolved historical event (temporal_status="resolved") describes something that has already happened.
It can be causal context supporting a future slot, but it should not be the slot's representative event
unless no unresolved candidate exists. Using a resolved event as a representative inflates the slot's
branch probability to near-certainty because its probability is already close to 1.0.
Instead: use the resolved event's probability as causal evidence, and prefer an unresolved forward-looking
event as the representative — even if the unresolved event has lower relation_confidence than the resolved one.
Only choose a resolved event as representative if no unresolved or active alternative exists for the slot.
...
LOW-PROBABILITY SUPPORTING EVENTS — critical:
  If an event's label directly describes the canonical slot outcome occurring (high semantic
  overlap with the slot label, slot_id, or description), classify it as "supports" regardless
  of how low its probability is. A low-probability event that describes the slot outcome is
  evidence that the outcome is UNLIKELY — that is its correct function. Do NOT reclassify it
  as "opposes" because the probability is low.

  The probability field tells you how likely the outcome is. The relation_to_slot field tells
  you what direction the event pushes. These are independent. An event at 10% probability
  that directly describes an outcome supports that outcome at 10% probability.

  WRONG pattern — DO NOT DO THIS:
    Event: "Formal US-DPRK Negotiated Freeze on Nuclear Development — 10%"
    Slot: "negotiated_freeze_or_reduction"
    Reasoning: "Only 10% likely, so this source implies it won't happen → opposes"
    Classification: opposes → inverts to 90%    ← INCORRECT

  CORRECT pattern:
    Event: "Formal US-DPRK Negotiated Freeze on Nuclear Development — 10%"
    Slot: "negotiated_freeze_or_reduction"
    Reasoning: "Event directly describes the slot outcome occurring → supports at 10%"
    Classification: supports → uses 10% directly    ← CORRECT

  Only use "opposes" when the event label describes the ABSENCE or NEGATION of the outcome,
  not when it describes the outcome itself with low probability.
...
For each slot, return:
  ...
  - candidate_evaluations: a dict keyed by event_id, with ONE entry for EVERY event_id
    listed in candidate_event_ids (not just best_event_id). This is REQUIRED whenever
    candidate_event_ids has more than one entry — secondary candidates need their own
    relation data to be usable as supporting evidence, even when they aren't strong enough
    to be the slot's representative. Each entry has the same shape described below
    (relation_to_slot, relation_confidence, event_kind, slot_relevance_rationale):
      "candidate_evaluations": {{
        "EVT_003": {{"relation_to_slot": "supports", "relation_confidence": 0.72,
                     "event_kind": "substantive_development", "slot_relevance_rationale": "..."}},
        "EVT_007": {{"relation_to_slot": "context", "relation_confidence": 0.45,
                     "event_kind": "causal_context", "slot_relevance_rationale": "..."}}
      }}

  When best_event_id is set, ALSO return (this is the same data as best_event_id's own
  entry in candidate_evaluations above — provide both for backward compatibility):
  ...
  - event_kind: classify the event itself —
      ...
      "forecast_signal" = an analyst/expert prediction OR institutional projection about
                  the future — THIS is the category for genuine forecasting evidence:
                  IEA/OPEC/Goldman-style demand projections, Fed funds futures pricing,
                  bond-market expectations, analyst forecasts. If a forecast/projection IS
                  the primary available evidence for a forward-looking question (e.g. "will
                  oil demand peak by 2030" has no evidence EXCEPT institutional projections),
                  use forecast_signal — do not downgrade it to meta_signal or market_signal.
      "reflexive_market_signal" = a prediction market or betting platform's odds
                  (Polymarket, Kalshi, PredictIt, Manifold, similar) reported by any
                  source. These are REFLEXIVE — they aggregate existing public belief,
                  not independent analysis. Do NOT use this for institutional market
                  pricing (Fed funds futures, bond yields, OIS curves) — those belong
                  in forecast_signal above; they reflect capital allocation, not betting.
      "structured_forecast_signal" = an expert/crowd structured-forecasting platform's
                  aggregated prediction (e.g. Metaculus) — weight above betting markets,
                  below direct official/substantive evidence.
      ...
      Note: "market_signal" and "meta_signal" are NOT valid event_kind choices —
      use forecast_signal for genuine institutional projections/pricing, or
      reflexive_market_signal specifically for betting-platform odds.
...
def _normalize_publisher(publisher: str) -> str:
    """Phase 8.4: normalize publisher names for stable family-key matching."""
    if not publisher:
        return "unknown"
    p = publisher.lower().strip()
    p = re.sub(r'\b(inc|llc|corp|corporation|ltd|co)\.?$', '', p).strip()
    p = re.sub(r'[^a-z0-9]+', '_', p).strip('_')
    return p or "unknown"


def _compute_source_family_key(publisher: str, source_type: str, query_purpose: str) -> str:
    """
    Phase 8.4: stable identity for a source family across observations.
    Format: publisher_normalized|source_type|slot_or_topic
    Used for anchor-source continuity tracking and churn computation.
    Does NOT use exact URLs — those change/duplicate across syndication.
    """
    pub_norm = _normalize_publisher(publisher)
    stype = (source_type or "article").lower().strip()
    # Extract slot/topic identifier from query_purpose (e.g. "slot_evidence_fed_cuts_100_to_200_bps")
    qp = (query_purpose or "").lower()
    if qp.startswith("slot_evidence_") or qp.startswith("slot_threshold_"):
        topic_part = qp.split("slot_evidence_", 1)[-1].split("slot_threshold_", 1)[-1]
    elif qp.startswith("disconfirmation_"):
        topic_part = qp.split("disconfirmation_", 1)[-1]
    elif qp in ("field_scan", "horizon_forecast", "branch_detection"):
        topic_part = "general"
    else:
        topic_part = qp or "general"
    return f"{pub_norm}|{stype}|{topic_part}"


# ──────────────────────────────────────────────────────────────────────────
# Phase 10 (GPT guidance): SourcePacket compression caching.
# Caches only the compression PAYLOAD (summary, key_claims, causal_phrases,
# branch_phrases, evidence_snippets, source_type, credibility, recency,
# raw_text_chars) — never the full SourcePacket. query_purpose,
# source_family_key, and canonical_slot_hint are run/query-specific and are
# always freshly computed by compress_source_to_packet for the current run.
# Stage 0 still searches and fetches fresh every run; this only skips the
# expensive compression API call when the exact fetched content is unchanged.
# ──────────────────────────────────────────────────────────────────────────

SOURCE_PACKET_CACHE_SCHEMA_VERSION = "v1"
COMPRESSION_PROMPT_VERSION = "v1"  # bump if PROMPT_COMPRESS_SOURCE changes meaningfully
_MODEL_VERSION_FOR_CACHE = "claude-haiku-4-5-20251001"

_SOURCE_PACKET_CACHE_STATS = {"hits": 0, "misses": 0, "writes": 0}


def _source_packet_db_path() -> str:
    """Same DB file the API uses, so cache persists across requests/deploys."""
    return os.getenv("UOT_DB_PATH", os.path.join(os.path.dirname(__file__), "uot_runs.db"))


def _source_packet_cache_conn():
    try:
        conn = sqlite3.connect(_source_packet_db_path(), timeout=5)
        conn.execute("""CREATE TABLE IF NOT EXISTS source_packet_cache (
            cache_key TEXT PRIMARY KEY,
            normalized_url TEXT NOT NULL,
            content_hash TEXT NOT NULL,
            compression_prompt_version TEXT NOT NULL,
            model_version TEXT NOT NULL,
            source_packet_schema_version TEXT NOT NULL,
        ...
        print(f"[SourcePacket cache] connection error: {e}")
        return None
...
def _normalize_url_for_cache(url: str) -> str:
    ...
    if "?" in u:
        ...
        keep = [p for p in qs.split("&")
                if not p.startswith(("utm_", "fbclid", "gclid", "ref=", "src="))]
        u = base + ("?" + "&".join(keep) if keep else "")
    return u.rstrip("/")


def _compute_source_packet_cache_key(url: str, raw_text: str) -> str:
    if not url or not raw_text:
        return ""
    norm_url = _normalize_url_for_cache(url)
    content_hash = hashlib.sha256((raw_text or "").encode("utf-8", "ignore")).hexdigest()[:24]
    key_material = "|".join([
        norm_url, content_hash, COMPRESSION_PROMPT_VERSION,
        _MODEL_VERSION_FOR_CACHE, SOURCE_PACKET_CACHE_SCHEMA_VERSION,
    ])
    return hashlib.sha256(key_material.encode("utf-8")).hexdigest()


def _source_packet_cache_get(cache_key: str):
    conn = _source_packet_cache_conn()
    if conn is None:
        return None
    try:
        row = conn.execute(
            "SELECT packet_json FROM source_packet_cache WHERE cache_key=?",
            (cache_key,)
        ).fetchone()
        if row:
            conn.execute(
                "UPDATE source_packet_cache SET last_used_at=? WHERE cache_key=?",
        ...
        print(f"[SourcePacket cache] get error: {e}")
        return None
    finally:
        conn.close()


def _source_packet_cache_set(cache_key: str, url: str, raw_text: str, payload: dict,
    ...
    try:
        ...
        content_hash = hashlib.sha256((raw_text or "").encode("utf-8", "ignore")).hexdigest()[:24]
        conn.execute(
            """INSERT OR REPLACE INTO source_packet_cache
               (cache_key, normalized_url, content_hash, compression_prompt_version,
                model_version, source_packet_schema_version, packet_json,
                raw_text_chars, title, publisher, url, created_at, last_used_at)
               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (cache_key, norm_url, content_hash, COMPRESSION_PROMPT_VERSION,
             _MODEL_VERSION_FOR_CACHE, SOURCE_PACKET_CACHE_SCHEMA_VERSION,
             json.dumps(payload), payload.get("raw_text_chars", 0),
             title, publisher, url, now, now)
        )
        conn.commit()
    except Exception as e:
        print(f"[SourcePacket cache] set error: {e}")
    finally:
        conn.close()


def _log_source_packet_cache_stats():
    s = _SOURCE_PACKET_CACHE_STATS
    print(f"[SourcePacket cache] hits={s['hits']} misses={s['misses']} writes={s['writes']} "
          f"(cumulative this seeding session)")
...
def compress_source_to_packet(
    ...
    Phase 10 (GPT guidance): SourcePacket compression caching.
    Stage 0 still searches fresh every run — this only skips the expensive
    compression call when the exact fetched content is unchanged. The cache
    stores only the compression PAYLOAD (summary, key_claims, causal_phrases,
    branch_phrases, evidence_snippets, source_type, credibility, recency,
    raw_text_chars) — NOT the full SourcePacket, because query_purpose,
    source_family_key, and canonical_slot_hint are run/query-specific and
    must be freshly computed for the current run.
    """
    ...
    _q_purpose = str(meta.get("purpose", "") or "")
    _stype_for_key = meta.get("source_type", "article")
    _family_key = _compute_source_family_key(publisher, _stype_for_key, _q_purpose)
    _slot_hint = _q_purpose.split("slot_evidence_", 1)[-1].split("slot_threshold_", 1)[-1].split(
        "disconfirmation_", 1)[-1] if any(
        _q_purpose.startswith(p) for p in ("slot_evidence_", "slot_threshold_", "disconfirmation_")) else ""
    ...
    if not LIVE_MODE:
        ...
        return SourcePacket(
            ...
            query_purpose=_q_purpose,
            source_family_key=_family_key,
            canonical_slot_hint=_slot_hint,
        )

    # Phase 10: check SourcePacket compression cache before calling the API
    _cache_key = _compute_source_packet_cache_key(url, raw_text)
    _cached_payload = _source_packet_cache_get(_cache_key) if _cache_key else None
    if _cached_payload:
        _SOURCE_PACKET_CACHE_STATS["hits"] += 1
        return SourcePacket(
            title=title, publisher=publisher, url=url, date=date,
            source_type=_cached_payload.get("source_type", "article"),
            credibility=clamp(_safe_float(_cached_payload.get("credibility"), 0.7)),
            recency    =clamp(_safe_float(_cached_payload.get("recency"),     0.7)),
            summary    =str(_cached_payload.get("summary", "")),
            key_claims      =_as_list(_cached_payload.get("key_claims",        [])),
            causal_phrases  =_as_list(_cached_payload.get("causal_phrases",    [])),
            branch_phrases  =_as_list(_cached_payload.get("branch_phrases",    [])),
            evidence_snippets=_as_list(_cached_payload.get("evidence_snippets", [])),
            raw_text_ref    =meta.get("cache_key", url),
            raw_text_chars  =int(_cached_payload.get("raw_text_chars", len(raw_text or ""))),
            query_purpose   =_q_purpose,
            source_family_key=_family_key,
            canonical_slot_hint=_slot_hint,
        )
    _SOURCE_PACKET_CACHE_STATS["misses"] += 1
    ...
    _stype_final = str(result.get("source_type", "article"))
    _family_key_final = _compute_source_family_key(publisher, _stype_final, _q_purpose)

    # Phase 10: write compression payload to cache (not the full SourcePacket —
    # only the run-independent compression fields)
    if _cache_key:
        _source_packet_cache_set(_cache_key, url, raw_text, {
        ...
        _SOURCE_PACKET_CACHE_STATS["writes"] += 1
    ...
    return SourcePacket(
        ...
        raw_text_chars  =len(raw_text or ""),
        query_purpose   =_q_purpose,
        source_family_key=_family_key_final,
        canonical_slot_hint=_slot_hint,
    )
...
def format_source_packets_for_stage_a(packets: List[SourcePacket]) -> str:
    ...
    Phase 9.7 (GPT guidance): Each source now gets a short, visible ID
    (S01, S02, ...) displayed prominently at the start of its block.
    Stage A only needs to cite source_id, stance, and relevance — not
    reproduce full metadata. The engine looks up the full SourcePacket
    from _LAST_SOURCE_REGISTRY when building SourceRef objects.
    ...
    """
    global _LAST_SOURCE_REGISTRY
    _LAST_SOURCE_REGISTRY = {}   # reset each time sources are formatted
    lines = []
    for i, pkt in enumerate(packets, 1):
        sid = f"S{i:02d}"
        _LAST_SOURCE_REGISTRY[sid] = pkt
        lines.append(f"[{sid}] {pkt.publisher} — {pkt.title}")
        lines.append(f"  type: {pkt.source_type}  |  "
                     f"query_purpose: {pkt.query_purpose or 'general'}  |  "
        ...
        if pkt.causal_phrases:
            for phrase in pkt.causal_phrases[:2]:
...
# Phase 9.7: module-level source registry — populated by format_source_packets_for_stage_a
# and consumed by build_seeded_events_from_dicts for deterministic source enrichment.
_LAST_SOURCE_REGISTRY: dict = {}   # {sid: SourcePacket}


def diversify_and_rank_sources(packets: List[SourcePacket], anchor_families: Optional[List[str]] = None) -> List[SourcePacket]:
    """
    Two-pass deduplication and ranking (v0.12 fix), with Phase 8.4 anchor boost.
    ...
    Pass 1: Deduplicate by URL; score each packet by credibility + recency,
            with a boost for packets matching an anchor_families source_family_key
            from the prior observation (Phase 8.4 source portfolio continuity).
    Pass 2: Type-aware interleaving — round-robin by source_type first,
            then fill remaining slots from the sorted remainder.
    """
    _anchor_set = set(anchor_families or [])

    # Pass 1: deduplicate + score (+ anchor continuity boost)
    seen_urls: set = set()
    scored: List[tuple] = []
    for pkt in packets:
        ...
        if key not in seen_urls:
            ...
            # Phase 8.4: boost packets whose source_family_key matches a prior
            # observation's anchor family — preserves source continuity for
            # resonance without forcing search behavior toward specific publishers.
            if _anchor_set and getattr(pkt, 'source_family_key', '') in _anchor_set:
                base_score += 0.15
    ...
    # Phase 8.1: raised cap to 16 + per-slot quota logic.
    # Step 1: one best from each source type (diversity pass) up to 6
    ...
    # Step 2: slot-quota pass — ensure at least 1 packet per slot_evidence purpose.
    # Uses the query_purpose field set during Phase 8.1 query generation.
    _slot_purposes_seen: set = {p.query_purpose for p in result if p.query_purpose.startswith('slot_')}
    _all_remaining = sorted(
        [item for queue in type_queues for item in queue],
        key=lambda x: x[0], reverse=True
    )
    for score_val, pkt in list(_all_remaining):
        if len(result) >= 16:
            break
        _pq = getattr(pkt, 'query_purpose', '') or ''
        if _pq.startswith('slot_') and _pq not in _slot_purposes_seen:
            result.append(pkt)
            _slot_purposes_seen.add(_pq)
            _all_remaining.remove((score_val, pkt))

    # Step 3: fill remaining slots from best-scoring remainder
    for _, pkt in _all_remaining:
        if len(result) >= 16:
            break
        result.append(pkt)
...
def generate_search_queries(topic: str, observer_basis: dict, canonical_slots=None,
    ...
    """
    Phase 8.1 — Outcome-aware Stage 0 (GPT Phase 8 guidance).

    Four query classes ensure every canonical outcome has a chance to be observed:

    Class 1 — Field scan (2-3 queries): broad topic, recent developments, risks.
    Class 2 — Observer-basis (capped at 2): measurement dimensions from observer_basis.
    Class 3 — Slot-targeted (1+ per slot): uses evidence_terms, aliases, action_exemplars,
               actor_scope, threshold ranges. Extra query for threshold_bucket slots.
    Class 4 — Disconfirmation (1 per status_quo/continuation/threshold slot): actively
               searches for evidence of quieter, low-salience, or non-dramatic outcomes
               that dominant-narrative searches miss.

    Budget: 10-16 queries total. Retained source packets: 12-16.
    UOT principle: every canonical slot deserves a chance to be observed.
    No slot deserves equal probability merely because it received a targeted search.
    Stage A must still estimate probability from source strength, not search coverage.
    """
    ...
    # ── Class 1: Field scan (3 queries) ──────────────────────────────────────
    queries = [
        {"query": f"{topic} latest developments {_today_year}",
         "purpose": "field_scan", "source_type": "article", "max_results": 4},
        {"query": f"{topic} analysis forecast {_horizon_year}",
         "purpose": "horizon_forecast", "source_type": "forecast", "max_results": 3},
        {"query": f"{topic} risks scenarios {_horizon_year}",
         "purpose": "branch_detection", "source_type": "article", "max_results": 3},
    ]

    # ── Class 2: Observer-basis queries (cap at 2) ────────────────────────────
    _cat_q = {
        "democratic_institutions":
            {"query": f"{topic} constitutional legal institutional analysis {_today_year}",
             "purpose": "democratic_institutions", "source_type": "legal_analysis"},
        "geopolitical_alliances":
            {"query": f"{topic} foreign policy alliances geopolitical {_today_year}",
             "purpose": "geopolitical_alliances", "source_type": "article"},
        "social_cohesion":
            {"query": f"{topic} political movement factions leadership {_today_year}",
             "purpose": "social_cohesion", "source_type": "article"},
        "economic_stability":
            {"query": f"{topic} economy market financial impact {_today_year}",
             "purpose": "economic_stability", "source_type": "market"},
    }
    _obs_added = 0
    for _cat, _wt in (observer_basis or {}).items():
        if _obs_added >= 2:
            break
        if _wt >= 0.6 and _cat in _cat_q:
            queries.append(_cat_q[_cat])
            _obs_added += 1

    # ── Class 3 + 4: Slot-targeted and disconfirmation ────────────────────────
    if canonical_slots:
        for slot in canonical_slots:
            _sid        = getattr(slot, 'slot_id', '') or ''
            _label      = getattr(slot, 'label', '') or ''
            _actor      = getattr(slot, 'actor_scope', '') or ''
            _ev_terms   = getattr(slot, 'evidence_terms', []) or []
            _aliases    = getattr(slot, 'aliases', []) or []
            _exemplars  = getattr(slot, 'action_exemplars', []) or []
            _t_unit     = getattr(slot, 'threshold_unit', '') or ''
            _t_lower    = getattr(slot, 'lower_bound', None)
            _t_upper    = getattr(slot, 'upper_bound', None)
            _kind       = getattr(slot, 'slot_kind', 'outcome') or 'outcome'
            _direction  = getattr(slot, 'directionality', '') or ''

            # Build best evidence-vocabulary query for this slot
            _subject = _actor or topic
            if _ev_terms:
                _ev_phrase = ' '.join(_ev_terms[:2])
                _q3 = f"{_subject} {_ev_phrase} {_horizon_year}"
            elif _kind == 'threshold_bucket' and _aliases:
                _q3 = f"{topic} {_aliases[0]} {_t_unit} {_horizon_year}"
            elif _exemplars:
                _q3 = f"{_subject} {_exemplars[0]} {_horizon_year}"
            else:
                _q3 = f"{topic} {_label} {_horizon_str or _horizon_year}"

            queries.append({
                "query": _q3.strip(),
                "purpose": f"slot_evidence_{_sid}",
                "source_type": "article",
                "max_results": 3,
                "slot_id": _sid,
            })

            # Extra query for threshold_bucket: numeric range alias
            if _kind == 'threshold_bucket' and _t_lower is not None and _t_upper is not None:
                _range_str = f"{int(_t_lower)}-{int(_t_upper)} {_t_unit}".strip()
                queries.append({
                    "query": f"{topic} {_range_str} {_horizon_year}",
                    "purpose": f"slot_threshold_{_sid}",
                    "source_type": "forecast",
                    "max_results": 3,
                    "slot_id": _sid,
                })

            # Class 4: disconfirmation for status_quo / continuation / threshold slots
            _needs_counter = (
                _kind in ('status_quo', 'threshold_bucket')
                or _direction == 'continuation'
                or any(t in _sid.lower() for t in ('continu', 'status_quo', 'no_change'))
            )
            if _needs_counter:
                if _ev_terms:
                    _counter_phrase = ' '.join(_ev_terms[:2])
                elif _kind == 'threshold_bucket' and _aliases:
                    _counter_phrase = _aliases[-1]  # use alternate alias
                else:
                    _counter_phrase = f"no major change {_label.lower()[:40]}"
                queries.append({
                    "query": f"{_subject} {_counter_phrase} {_horizon_year}".strip(),
                    "purpose": f"disconfirmation_{_sid}",
                    "source_type": "article",
                    "max_results": 3,
                    "slot_id": _sid,
                })

    # Hard cap at 16 queries total (generous upper bound)
    return queries[:16]
...
def web_search_sources(topic: str, observer_basis: Optional[dict] = None, follow_up_queries: Optional[List[str]] = None, canonical_slots=None, horizon=None, anchor_families: Optional[List[str]] = None) -> List[SourcePacket]:
    """
    ...
    # Phase 8.1: pass canonical_slots and horizon directly to live_source_pipeline.
    # live_source_pipeline (uot_live_search.py) now accepts these parameters and
    # passes them through to generate_search_queries for slot-targeted search.
    raw_docs = live_source_pipeline(topic, observer_basis, follow_up_queries,
                                    canonical_slots=canonical_slots,
                                    horizon=horizon if 'horizon' in dir() else None)

    # Phase 10.3 (GPT guidance): per-query result counts, grouped by purpose.
    # This directly answers the 16 vs 45 source packet variance question —
    # query generation is stable but live search returns variable result counts.
    _qp_counts = {}
    for _doc in raw_docs:
        _qp = (_doc.metadata.get('purpose', 'unknown')
               if hasattr(_doc, 'metadata') and isinstance(_doc.metadata, dict)
               else 'unknown')
        _qp_counts[_qp] = _qp_counts.get(_qp, 0) + 1
    print(f"[Stage 0] Per-purpose result counts: "
          + ", ".join(f"{p}={n}" for p, n in sorted(_qp_counts.items()))
          + f" | total_docs={len(raw_docs)}")

    # Note: cache stats are NOT reset here. The pipeline runs two passes per
    # seeding session (slot-targeted queries then Stage A follow-ups), each
    # calling this function separately. Accumulating across both passes gives
    # one combined stats log per seeding session rather than two partial logs.

    packets  = [compress_source_to_packet(doc.text, doc.metadata) for doc in raw_docs]
    _log_source_packet_cache_stats()
    # Phase 10.3: compression savings estimate
    _cache_hits = _SOURCE_PACKET_CACHE_STATS["hits"]
    if _cache_hits > 0:
        print(f"[SourcePacket cache] estimated_compression_calls_saved={_cache_hits}")
    return diversify_and_rank_sources(packets, anchor_families=anchor_families)
...
def call_anthropic_api(system_prompt: str, user_message: str,
    ...
    # Bugfix: simple retry-with-backoff for TRANSIENT server-side errors only
    # (529 overloaded, 500, 503) — these are Anthropic capacity issues, not
    # anything wrong with our request, and very commonly resolve on a quick
    # retry. Does NOT retry on 4xx (bad request, auth, etc.) since those will
    # never succeed by retrying the same payload.
    _TRANSIENT_CODES = (529, 500, 503, 502)
    _max_retries = 2
    _last_err = None
    for _attempt in range(_max_retries + 1):
        try:
            ...
            if e.code in _TRANSIENT_CODES and _attempt < _max_retries:
                _wait = 2 ** _attempt  # 1s, 2s
                print(f"[v0.12] Anthropic API HTTP {e.code} (transient) — "
                      f"retrying in {_wait}s (attempt {_attempt + 1}/{_max_retries})")
                import time as _time_mod
                _time_mod.sleep(_wait)
                _last_err = RuntimeError(f"Anthropic API HTTP {e.code}: {body}")
                continue
    ...
    try:
        ...
        try:
            ...
            # Bugfix: this was a COMPLETELY SILENT failure path — returning {}
            # here looks identical to "the model legitimately returned an empty
            # object," with no way for any caller to distinguish a parse failure
            # from a real empty response. This caused Stage C's 5000-token JSON
            # to truncate mid-response (after the Phase 8.11 candidate_evaluations
            # schema addition increased expected output size), fail to parse,
            # fail to repair, and silently produce "0 branch group members" with
            # NO error anywhere in the logs. Always log this now — every caller
            # benefits, not just Stage C.
            print(f"[v0.12] ⚠ call_anthropic_api: JSON parse AND repair both failed "
                  f"(model={model}, max_tokens={max_tokens}, response_chars={len(text)}). "
                  f"This usually means the response was truncated by max_tokens — "
                  f"consider raising it for this call. First/last 150 chars of response: "
                  f"{text[:150]!r} ... {text[-150:]!r}")
...
def extract_candidate_events_holistic(topic, source_summaries, observer_basis,
                                       ...
                                       series_calibration_context: dict = None):
    """
    Stage A: holistic extraction + evidence gap detection.
    Returns (event_dicts, follow_up_queries, missing_evidence, slot_evidence_coverage).
    ...
    Phase 8.5 (GPT guidance): series_calibration_context carries prior observation
    probabilities and dominant source families per slot, for calibration CONTINUITY
    — not as a prior to obey. Used only on re-observations of tracked scenarios.
    """
    if not LIVE_MODE:
        return [], [], [], {}
    ...
    # Phase 8.5: series calibration context — continuity guidance, not a prior to obey
    _calib_text = ""
    if series_calibration_context:
        calib_lines = ["SERIES CALIBRATION CONTEXT (for continuity, NOT a prior to obey):"]
        for sid, info in series_calibration_context.items():
            if not isinstance(info, dict):
                continue
            calib_lines.append(
                f"  {sid}: last_probability={info.get('last_probability', '?')}, "
                f"dominant_source_families={info.get('dominant_source_families', [])}, "
                f"evidence_basis={info.get('evidence_basis', '?')}"
            )
        calib_lines.append(
            "If current evidence is structurally similar to prior evidence for a slot, avoid "
            "large probability-band changes. If current evidence materially differs, briefly "
            "note why in your rationale. This is NOT a target to match — assess the evidence "
            "in front of you; this context only flags when you'd be making a large unexplained shift.\n"
        )
        _calib_text = "\n".join(calib_lines) + "\n"
    ...
    result = call_anthropic_api(
        ...
        max_tokens=8000,   # Phase 8: raised to accommodate 10-15 events with expanded source pool
        timeout=150        # Phase 8: raised from 90 to accommodate larger source pool processing
    )
    ...
    # Phase 8.10: detect events where Stage A appears to have constructed a negated
    # version of a canonical slot's own label, rather than describing natural source
    # evidence. Diagnostic only — does not auto-correct or auto-invert. Flags for
    # manual review and marks seed quality amber via the constructed_negated_slot_label
    # metadata field, which feeds into seed quality computation downstream.
    _flagged_count = 0
    for _ev in events:
        if not isinstance(_ev, dict):
            continue
        _ev_label = str(_ev.get("label", "") or "")
        _flag_sid = _detect_constructed_negated_slot_label(_ev_label, canonical_slots)
        if _flag_sid:
            _ev.setdefault("metadata", {})["constructed_negated_slot_label"] = True
            _ev["metadata"]["constructed_negated_slot_label_slot"] = _flag_sid
            _flagged_count += 1
            print(f"[Phase 8.10] ⚠ constructed_negated_slot_label flagged: event {_ev.get('id','?')} "
                  f"label={_ev_label[:70]!r} appears to negate slot '{_flag_sid}' rather than "
                  f"describe natural source evidence — review before trusting any inversion")
    if _flagged_count:
        print(f"[Phase 8.10] {_flagged_count} event(s) flagged for constructed_negated_slot_label this run")

    # Phase 8.2: extract slot_evidence_coverage for diagnostic logging and storage
    _sec = result.get("slot_evidence_coverage", {})
    if _sec and isinstance(_sec, dict):
        print(f"[Phase 8.1] slot_evidence_coverage: " +
              ", ".join(f"{sid}={cov}" for sid, cov in _sec.items()))
    return events, result.get("follow_up_search_queries", []), result.get("missing_evidence", []), _sec


def _detect_constructed_negated_slot_label(event_label: str, canonical_slots) -> str:
    """
    Phase 8.10 (GPT guidance): detect when Stage A appears to have invented a
    negated version of a canonical slot's own positive label, rather than
    describing what a source actually claims. This is the failure pattern that
    caused backwards probability assignment (e.g. 0.05 for a self-constructed
    "will not occur" label, meaning Stage A actually believed the opposite).

    Heuristic: event label contains most of a slot's distinctive content words
    AND contains an explicit negation signal. This doesn't prove construction —
    a natural source claim could coincidentally overlap — but it's a useful
    diagnostic flag for manual review, not an auto-correction trigger.

    Returns the matched slot_id, or "" if no match.
    """
    if not event_label or not canonical_slots:
        return ""
    _label_lower = event_label.lower()
    _has_negation = any(sig in _label_lower for sig in _NEGATION_SIGNALS)
    if not _has_negation:
        return ""

    _stopwords = frozenset({
        'the', 'a', 'an', 'of', 'in', 'on', 'at', 'to', 'for', 'and', 'or',
        'will', 'not', 'does', 'is', 'are', 'be', 'occur', 'occurs', 'within',
        'horizon', 'no', 'ruled', 'out', 'fails', 'fail', 'never', 'cannot',
    })
    _label_tokens = set(re.findall(r'[a-z]+', _label_lower)) - _stopwords

    # Bugfix (found via testing): the original version flagged the FIRST slot
    # crossing the overlap threshold, including legitimately negative-outcome
    # slots. For a true binary question (e.g. "China conducts military
    # operation" yes/no), the NO slot's own natural label is itself something
    # like "China does NOT conduct..." -- which shares almost all its vocabulary
    # with the YES slot AND contains a negation signal BY DESIGN. That is not
    # a constructed-negation problem; it's a correct match to the slot's own
    # legitimately-negative phrasing. Fix: find the BEST-matching slot across
    # ALL slots, and only flag if that best match is a POSITIVELY-phrased slot
    # (no negation signal in the slot's own label) -- i.e. the event appears to
    # negate a slot that was never meant to be described in negative terms.
    _best_slot = None
    _best_overlap_ratio = 0.0
    for slot in canonical_slots:
        _slot_label = str(getattr(slot, 'label', '') or '').lower()
        if not _slot_label:
            continue
        _slot_tokens = set(re.findall(r'[a-z]+', _slot_label)) - _stopwords
        if len(_slot_tokens) < 2:
            continue  # too short to meaningfully compare
        _overlap = _label_tokens & _slot_tokens
        _ratio = len(_overlap) / len(_slot_tokens)
        if _ratio > _best_overlap_ratio:
            _best_overlap_ratio = _ratio
            _best_slot = slot

    if _best_slot is None or _best_overlap_ratio < 0.6:
        return ""

    _best_slot_label = str(getattr(_best_slot, 'label', '') or '').lower()
    _best_slot_polarity = str(getattr(_best_slot, 'slot_polarity', '') or '').lower()
    _slot_itself_is_negative = (
        any(sig in _best_slot_label for sig in _NEGATION_SIGNALS)
        or _best_slot_polarity in ('no', 'status_quo')
    )
    if _slot_itself_is_negative:
        return ""  # legitimate match to a naturally-negative slot, not a constructed wrapper

    return getattr(_best_slot, 'slot_id', '') or ""
...
def score_event_fields_specialized(event_candidates, source_summaries):
    ...
    try:
        scores = call_anthropic_api(
            ...
            model="claude-haiku-4-5-20251001",
            # Bugfix: raised from 2000, then found still insufficient at 3500 —
            # a second truncation (response cut off mid-string, ~7888 chars,
            # confirmed via the new call_anthropic_api logging) showed Haiku
            # still running out of budget on 12-15 events × 5 text fields each
            # (downstream_impact/disruption_score/novelty + two rationale
            # strings). Raised generously since Haiku is fast/cheap and there's
            # little cost to extra headroom here.
            max_tokens=6000,
            timeout=90
        )
    except Exception as e:
...
_LAST_SOURCE_PORTFOLIO: dict = {}  # Phase 8.4: {"packets": [...]} for API churn/anchor computation

def clear_primary_question_state() -> None:
    """Call at the very start of each seeding pipeline to prevent state bleed."""
    global _LAST_PRIMARY_QUESTION, _LAST_SOURCE_PORTFOLIO, _LAST_SOURCE_REGISTRY
    _LAST_PRIMARY_QUESTION = {}
    _LAST_SOURCE_PORTFOLIO = {}
    _LAST_SOURCE_REGISTRY = {}
    # Phase 9.3: print the deploy tag every pipeline run so we can confirm the
    # correct (non-cached) version of this file is being executed by Railway.
    print(f"[v0.12] Engine deploy tag: {_ENGINE_DEPLOY_TAG}")
...
def canonicalize_extraction(event_dicts: list, stage_c_edges: list = None) -> tuple:
    """
    ...
    Phase 9.8 fix: now returns (event_dicts, id_map) so callers can update
    SlotAssignment.event_id values to match the new canonical IDs — without
    this, world.event_graph.nodes uses EVT_NNN keys while SlotAssignments
    hold pre-canonical IDs (e.g. "E09"), causing all node lookups to fail
    silently in the source coherence computation.
    """
...
def build_seeded_events_from_dicts(event_dicts):
    ...
    Phase 9.7 (GPT guidance): Handles the new source_id schema — Stage A
    now cites {"source_id": "S01", "stance": "supporting", "relevance": 0.85}
    instead of reproducing full metadata. This function:
      1. Looks up each source_id in _LAST_SOURCE_REGISTRY to build full SourceRef.
      2. Validates that every event has at least one source.
      3. For events missing sources, runs a deterministic text-similarity linker
         using SourcePacket summaries/key_claims against event label/description.
      4. Records source_attribution_missing_count and linked_by_fallback_count
         into each event's metadata for seed_quality tracking.
    """
    print(f"[Phase 9.7] build_seeded_events_from_dicts: {len(event_dicts) if event_dicts else 0} events, "
          f"{len(_LAST_SOURCE_REGISTRY)} sources in registry")
    seeded = []
    _src_total = 0
    _src_from_registry = 0
    _events_missing_sources = 0
    _events_linked_by_fallback = 0
    _stance_breakdown: dict = {}
    ...
    for d in (x for x in (event_dicts or []) if isinstance(x, dict)):
        ...
        # Phase 9.7: Build SourceRef objects. Handle BOTH schemas:
        # - New: {"source_id": "S01", "stance": "supporting", "relevance": 0.85}
        # - Legacy fallback: {"title": "...", "publisher": "...", ...}
        sources = []
        ...
        if isinstance(raw_sources, list):
            for s in raw_sources:
                if not isinstance(s, dict):
                    continue
                _sid = s.get("source_id")
                if _sid and _sid in _LAST_SOURCE_REGISTRY:
                    # New schema: enrich from registry
                    pkt = _LAST_SOURCE_REGISTRY[_sid]
                    sources.append(SourceRef(
                        title=pkt.title, publisher=pkt.publisher,
                        url=pkt.url, date=pkt.date,
                        relevance=float(s.get("relevance", 0.7)),
                        stance=s.get("stance", "supporting"),
                        source_type=pkt.source_type,
                        query_purpose=pkt.query_purpose,
                    ))
                    _src_from_registry += 1
                elif not _sid:
                    # Legacy schema: title/publisher keys
                    sources.append(SourceRef(
                        ...
                        query_purpose=s.get("query_purpose",""),
                    ))
                # else: unknown source_id — skip and let the validator catch it

        _src_total += len(sources)
        for _sr in sources:
            _stance_breakdown[_sr.stance] = _stance_breakdown.get(_sr.stance, 0) + 1

        # Phase 9.7: Validator — every event must have at least one source.
        _attribution_missing = len(sources) == 0
        _linked_by_fallback = False
        if _attribution_missing and _LAST_SOURCE_REGISTRY:
            # Deterministic source linker: compare event text against source summaries/claims.
            _ev_text = " ".join(filter(None, [
                str(d.get("label", "") or ""),
                str(d.get("description", "") or ""),
                str((nd or {}).get("probability_rationale", "") or ""),
            ])).lower()
            _best_sid, _best_score = None, 0.0
            for _sid, pkt in _LAST_SOURCE_REGISTRY.items():
                _pkt_text = " ".join(filter(None, [
                    str(pkt.summary or ""),
                    " ".join(pkt.key_claims or []),
                    " ".join(pkt.evidence_snippets or []),
                ])).lower()
                # Simple word-overlap similarity (fast, deterministic, no model needed)
                _ev_words = set(_ev_text.split())
                _pkt_words = set(_pkt_text.split())
                _overlap = _ev_words & _pkt_words
                # Filter stopwords
                _stopwords = {'the','a','an','of','in','on','at','to','for','and','or',
                              'is','are','be','will','not','by','with','that','this','as','has','have'}
                _meaningful = _overlap - _stopwords
                _score = len(_meaningful) / max(1, len(_ev_words - _stopwords)) if _ev_words else 0.0
                if _score > _best_score:
                    _best_score, _best_sid = _score, _sid
            # GPT thresholds: strong link >= 0.45, usable >= 0.30
            if _best_score >= 0.30 and _best_sid:
                pkt = _LAST_SOURCE_REGISTRY[_best_sid]
                sources.append(SourceRef(
                    title=pkt.title, publisher=pkt.publisher,
                    url=pkt.url, date=pkt.date,
                    relevance=round(min(0.85, _best_score * 1.5), 2),
                    stance="supporting",   # deterministically linked = by definition supporting
                    source_type=pkt.source_type,
                    query_purpose=pkt.query_purpose,
                ))
                _linked_by_fallback = True
                _events_linked_by_fallback += 1
                _attribution_missing = False
                print(f"[Phase 9.7] Deterministic source linker: {d.get('id')} -> {_best_sid} "
                      f"(score={_best_score:.3f}, pkt='{pkt.title[:50]}')")
            else:
                _events_missing_sources += 1
                d.setdefault("metadata", {})["source_attribution_missing"] = True
                d["metadata"]["source_coherence_eligible"] = False
                print(f"[Phase 9.7] ⚠ source_attribution_missing for {d.get('id')} "
                      f"(best_score={_best_score:.3f} < 0.30)")

        if _linked_by_fallback:
            d.setdefault("metadata", {})["source_linked_by_fallback"] = True
        ...
        if isinstance(d.get("metadata"), dict) and d["metadata"].get("evidence_basis_note"):
            print(f"[Phase 8 diag] evidence_basis_note SURVIVED into SeededEvent.seed_metadata "
                  f"for {d.get('id')}: {d['metadata']['evidence_basis_note']!r}")

    # Phase 9.7 summary — also write to _LAST_PRIMARY_QUESTION so the API
    # can read attribution counts into primary_question_json.seed_quality.
    global _LAST_PRIMARY_QUESTION
    _LAST_PRIMARY_QUESTION.setdefault("seed_quality", {}).update({
        "source_attribution_missing_count": _events_missing_sources,
        "source_linked_by_fallback_count": _events_linked_by_fallback,
        "source_from_registry_count": _src_from_registry,
        "stance_breakdown": _stance_breakdown,
    })
    print(f"[Phase 9.7] Source attribution summary: "
          f"{_src_total} sources across {len(seeded)} events "
          f"({_src_from_registry} from registry, "
          f"{_events_linked_by_fallback} deterministically linked, "
          f"{_events_missing_sources} still unattributed). "
          f"Stance breakdown: {_stance_breakdown}")
    return seeded


    _src_total = 0
    _src_explicit_stance = 0
    _src_stance_breakdown: dict = {}
    for d in (x for x in (event_dicts or []) if isinstance(x, dict)):
        _nd_raw = d.get("extraction_notes"); nd = _nd_raw if isinstance(_nd_raw, dict) else {}
        diag = ExtractionDiagnostics(
            downstream_impact_rationale = nd.get("downstream_impact_rationale",""),
        ...
        for _s_diag in d.get("sources", []):
            if isinstance(_s_diag, dict):
                _src_total += 1
                _st = _s_diag.get("stance")
                if _st:
                    _src_explicit_stance += 1
                    _src_stance_breakdown[_st] = _src_stance_breakdown.get(_st, 0) + 1
        sources = [SourceRef(
            title=s.get("title",""), publisher=s.get("publisher",""),
            url=s.get("url",""), date=s.get("date",""),
            relevance=float(s.get("relevance",0.7)), stance=s.get("stance","neutral"),
            source_type=s.get("source_type",""),      # Phase 8.9
            query_purpose=s.get("query_purpose",""),  # Phase 8.9
        ...
        if isinstance(d.get("metadata"), dict) and d["metadata"].get("evidence_basis_note"):
            print(f"[Phase 8 diag] evidence_basis_note SURVIVED into SeededEvent.seed_metadata "
                  f"for {d.get('id')}: {d['metadata']['evidence_basis_note']!r}")
    # Phase 9.3 follow-up diagnostic: report whether Stage A actually used the
    # stance/relevance instruction this run, or silently defaulted everything.
    if _src_total > 0:
        _pct_explicit = 100.0 * _src_explicit_stance / _src_total
        print(f"[Phase 9.3 diag] Source stance compliance: {_src_explicit_stance}/{_src_total} "
              f"sources ({_pct_explicit:.0f}%) had explicit stance from Stage A. "
              f"Breakdown: {_src_stance_breakdown}")
        if _pct_explicit < 50:
            print(f"[Phase 9.3 diag] ⚠ Stage A is NOT reliably classifying source stance — "
                  f"most sources still falling back to the 'neutral' default, which means "
                  f"source_agreement/source_coherence will keep clustering near their "
                  f"formula floor regardless of actual evidence quality.")
    else:
        print(f"[Phase 9.3 diag] No sources cited by Stage A this run at all "
              f"(0 sources across {len(seeded)} events) — source_agreement/source_coherence "
              f"will fall back to defaults for everything.")
    return seeded
    ...
    **kwargs
    ...
    try:
        ...
        if _pq_param and hasattr(_pq_param, "canonical_slots"):
            def _slot_context(s):
                parts = [f"  - slot_id={s.slot_id}, label={s.label!r},"
                         f" polarity={s.slot_polarity}, kind={s.slot_kind}"]
                if getattr(s, 'description', ''):
                    parts.append(f"    description: {s.description}")
                if getattr(s, 'action_exemplars', []):
                    parts.append(f"    action_exemplars: {s.action_exemplars}")
                if getattr(s, 'evidence_terms', []):
                    parts.append(f"    evidence_terms: {s.evidence_terms}")
                if getattr(s, 'aliases', []):
                    parts.append(f"    aliases: {s.aliases}")
                # Phase 9.8: vocabulary bridge — add negative_examples and
                # directionality so Stage C can distinguish continuation slots
                # from disruption slots even when event labels are abstract.
                if getattr(s, 'negative_examples', []):
                    parts.append(f"    negative_examples (do NOT map these event types here): {s.negative_examples}")
                _dir = getattr(s, 'directionality', '')
                if _dir:
                    parts.append(f"    directionality: {_dir}")
                actor = getattr(s, 'actor_scope', '')
                if actor:
                    parts.append(f"    actor_scope: {actor!r}")
                return chr(10).join(parts)
            _slots_text = chr(10).join(_slot_context(s) for s in _pq_param.canonical_slots)
        ...
        result = call_anthropic_api(
            ...
            model="claude-sonnet-4-6",
            # Bugfix: raised from 5000. The Phase 8.11 candidate_evaluations
            # requirement asks Stage C to provide full relation data (relation_to_slot,
            # relation_confidence, event_kind, rationale) for EVERY candidate per slot,
            # not just the best one — this can roughly double or triple the expected
            # output size for multi-slot, multi-candidate scenarios. At 5000 tokens
            # this was silently truncating mid-JSON, producing a parse failure that
            # fell back (see below) to a completely empty structure with NO error
            # logged anywhere — exactly the "12 events but 0 branch group members,
            # no errors in the logs" failure mode found in testing.
            max_tokens=9000,
            timeout=150
        )
    ...
    except Exception as _stage_c_err:
        # Bugfix: this was previously `except Exception: pass` — a COMPLETELY
        # SILENT failure path. Any exception here (truncated JSON from hitting
        # max_tokens, API timeout, malformed response) fell back to an empty
        # structure with zero log output, making the failure indistinguishable
        # from "Stage C legitimately found nothing" in the logs. Always log now.
        print(f"[v0.12] ⚠ Stage C structure inference FAILED — falling back to empty "
              f"structure (0 branch group members): {type(_stage_c_err).__name__}: "
              f"{str(_stage_c_err)[:300]}")
...
def collect_and_extract_seeded_events(topic: str, observer_basis: dict,
                                       ...
                                       locked_canonical_slots=None,
                                       prior_slot_probs: dict = None,
                                       anchor_families: Optional[List[str]] = None,
                                       series_calibration_context: dict = None) -> tuple:
    # anchor_families: source_family_keys from the prior observation (Phase 8.4) —
    #   biases source ranking toward continuity without forcing search behavior.
    # series_calibration_context: prior slot probabilities + dominant source families
    #   (Phase 8.5) — passed to Stage A as calibration context, not a prior to obey.
    """
    ...
    # Reset SourcePacket cache stats at the start of each seeding session.
    # The pipeline runs two fetch-and-compress passes per session; stats are
    # cumulative across both passes and logged once at the end of each pass.
    _SOURCE_PACKET_CACHE_STATS["hits"] = 0
    _SOURCE_PACKET_CACHE_STATS["misses"] = 0
    _SOURCE_PACKET_CACHE_STATS["writes"] = 0
    ...
    if locked_canonical_slots and isinstance(locked_canonical_slots, list) and len(locked_canonical_slots) > 0:
        ...
        if _locked_slot_objs:
            # Bugfix: locked re-observations were NEVER passing through
            # primary_branch_group_id, silently defaulting to the dataclass
            # default ("primary_outcome") every single re-observation,
            # regardless of what bg_id the series' original fresh Stage Q run
            # actually chose (e.g. a topic-specific name like
            # "peak_timing_window"). This caused this run's seeded events to
            # be tagged with the WRONG branch_group, breaking any downstream
            # comparison against the series' true, historical bg_id (e.g. the
            # frontend's primary-branch-group highlighting, which compares the
            # node's branch_group against primary_question.primary_branch_group_id).
            _locked_bg_id = (
                locked_primary_question.get("primary_branch_group_id", "primary_outcome")
                if isinstance(locked_primary_question, dict) and locked_primary_question
            ...
            _pq = PrimaryQuestion(
                ...
                primary_branch_group_id=_locked_bg_id,
            )
            ...
            print(f"[v0.12] Locked primary_branch_group_id: {_pq.primary_branch_group_id!r}")
        else:
    ...
    event_dicts, follow_up_queries, missing_evidence, _slot_coverage = extract_candidate_events_holistic(
        topic, source_summaries, observer_basis, temporal_context=temporal_context,
        canonical_slots=_pq.canonical_slots if _pq else None,
        series_calibration_context=series_calibration_context
    )
    if _slot_coverage:
        # Store for later use by Stage C and diagnostics
        if isinstance(_pq, object) and hasattr(_pq, '__dict__'):
            try:
                _pq.slot_evidence_coverage = _slot_coverage
            except Exception:
                pass
        # Phase 9.1 bugfix: slot_evidence_coverage was being set on _pq but never
        # actually persisted into _LAST_PRIMARY_QUESTION (the dict that gets
        # serialized into primary_question_json for storage). This made it
        # invisible to the API, which needs it for per-slot navigation overrides.
        _LAST_PRIMARY_QUESTION['slot_evidence_coverage'] = _slot_coverage
    ...
    if follow_up_queries:
        ...
        _final_packets = all_packets

    # Phase 8.4: record final source portfolio for API churn/anchor computation.
    # Stored on a module-level variable (same pattern as _LAST_PRIMARY_QUESTION)
    # since this function's return signature is already a large tuple.
    global _LAST_SOURCE_PORTFOLIO
    _LAST_SOURCE_PORTFOLIO = {
        "packets": [
            {
                "url": p.url, "publisher": p.publisher, "source_type": p.source_type,
                "query_purpose": p.query_purpose,
                "source_family_key": p.source_family_key,
                "canonical_slot_hint": p.canonical_slot_hint,
                "credibility": p.credibility, "recency": p.recency,
            }
            for p in _final_packets
        ]
    }

    # Phase 8.9: enrich each event's "sources" (Stage A cites title+publisher only)
    # with source_type and query_purpose by matching against the actual SourcePackets
    # used this run. This is more reliable than asking Stage A to repeat structured
    # fields it would have to recall — the engine already has the authoritative data.
    _packet_lookup = {}
    for _p in _final_packets:
        _key = (str(_p.title or '').strip().lower(), str(_p.publisher or '').strip().lower())
        _packet_lookup[_key] = _p
    for _ev in event_dicts:
        if not isinstance(_ev, dict):
            continue
        _ev_sources = _ev.get('sources', [])
        if not isinstance(_ev_sources, list):
            continue
        for _src_item in _ev_sources:
            if not isinstance(_src_item, dict):
                continue
            _skey = (str(_src_item.get('title', '')).strip().lower(),
                     str(_src_item.get('publisher', '')).strip().lower())
            _matched = _packet_lookup.get(_skey)
            if _matched:
                _src_item['source_type'] = _matched.source_type
                _src_item['query_purpose'] = _matched.query_purpose
                _src_item['url'] = _matched.url
    ...
    # Phase 9.8 fix: canonicalize_extraction now returns (event_dicts, id_map)
    # so we can update SlotAssignment.event_id values to the canonical form.
    # Without this, world.event_graph.nodes uses EVT_NNN keys while the
    # SlotAssignments stored in _slot_info hold the original pre-canonical IDs
    # (e.g. "E09"), causing all node lookups to fail silently in the API's
    # source coherence computation. The fix: apply id_map to every SA and SC.
    event_dicts, _id_map = canonicalize_extraction(event_dicts, stage_c_edges)
    if _id_map and _slot_info:
        # Update SlotAssignment.event_id values to match the canonical node IDs
        # so the API's source coherence node lookup succeeds.
        for _sa in (_slot_info.get('slot_assignment_records') or []):
            if hasattr(_sa, 'event_id') and _sa.event_id in _id_map:
                _sa.event_id = _id_map[_sa.event_id]
        for _sc in (_slot_info.get('slot_contribution_records') or []):
            if hasattr(_sc, 'event_id') and _sc.event_id in _id_map:
                _sc.event_id = _id_map[_sc.event_id]
        # Phase 9.8 bugfix (SlotInfluence 0.000): SlotInfluence records use
        # source_event_id (not event_id). Same pre-canonical ID mismatch was
        # causing all SlotInfluence node lookups to fail silently, producing
        # Δprob=0.000 and Δstruct=0.000 for every influence on every run.
        for _si in (_slot_info.get('slot_influence_records') or []):
            if hasattr(_si, 'source_event_id') and _si.source_event_id in _id_map:
                _si.source_event_id = _id_map[_si.source_event_id]
    # ── Fallback: ensure at least one branch group exists ────────────────────
    # ── Validate and flag branch structure ───────────────────────────────────
    # Bugfix (root cause of "Step 6 tags branch_group correctly but slot still
    # missing from primary branch group"): _primary_bg_id was being read from
    # _LAST_PRIMARY_QUESTION, which is not yet updated when skip_primary=True
    # (apply_structure_to_event_candidates skips the primary group update).
    # If the series uses a non-default branch group ID (e.g. "regulatory_futures"
    # instead of "primary_outcome"), the global still holds the old/default value
    # at this point. validate_branch_structure then receives the WRONG ID for its
    # SINGLE_MEMBER_GROUP exemption check — so a correctly-tagged 1-member primary
    # slot (like a Step-2-found fallback representative) gets stripped because
    # bg_key ("regulatory_futures") != primary_branch_group_id ("primary_outcome").
    # Fix: read from _pq (the authoritative PrimaryQuestion object), which has
    # the correct value — set by my earlier locked-bg_id preservation fix.
    _primary_bg_id = (getattr(_pq, 'primary_branch_group_id', None)
                      or _LAST_PRIMARY_QUESTION.get("primary_branch_group_id")
                      or "primary_outcome")
    ...
    _slot_contribution_records = _slot_info.get("slot_contribution_records", []) if isinstance(_slot_info, dict) else []

    # Phase 7.3: run SlotInfluence diagnostics (diagnostic-only; apply=False until flag set)
    try:
        # Phase 8 bugfix: SlotAssignment has no 'branch_probability' field — it has
        # 'effective_probability' (the post-inversion probability used for the
        # branch) and 'raw_probability' (pre-inversion). The old code looked up a
        # nonexistent attribute, always got None/False, and silently stored 0.0
        # for every slot — masked by an "or 0.5" fallback elsewhere that made every
        # diagnostic log show "raw=0.500" regardless of the real value.
        # Only the primary (non-secondary) assignment per slot should set the
        # representative raw probability — prefer accepted_by='engine_score' or
        # the first assignment encountered per slot_id.
        _raw_probs = {}
        for _sa in _slot_assignment_records:
            if isinstance(_sa, dict):
                _sid = _sa.get('slot_id')
                _prob = _sa.get('effective_probability', _sa.get('raw_probability'))
            else:
                _sid = getattr(_sa, 'slot_id', None)
                _prob = getattr(_sa, 'effective_probability', None)
                if _prob is None:
                    _prob = getattr(_sa, 'raw_probability', None)
            if _sid and _prob is not None and _sid not in _raw_probs:
    ...
    # Phase 8.12: compute evidence-family aggregation shadow comparison per slot.
    # Diagnostic only — does not feed branch normalization. Uses the same
    # _raw_probs (real representative probabilities, post-bugfix) computed above.
    _contribution_shadow = {}
    try:
        if _slot_contribution_records and _raw_probs:
            for _sid_c, _base_p in _raw_probs.items():
                _shadow_result = compute_slot_contribution_adjusted_support(
                    slot_id=_sid_c,
                    representative_effective_probability=_base_p,
                    contributions=_slot_contribution_records,
                )
                if _shadow_result.get("contribution_count", 0) > 0:
                    _contribution_shadow[_sid_c] = _shadow_result
                    print(f"[Phase 8.12] Contribution-adjusted shadow for '{_sid_c}': "
                          f"representative={_shadow_result['representative_support']:.3f} -> "
                          f"adjusted={_shadow_result['contribution_adjusted_support']:.3f} "
                          f"(shift={_shadow_result['shift']:+.3f}, "
                          f"{_shadow_result['family_count']} family/families, "
                          f"{_shadow_result['contribution_count']} contribution(s), "
                          f"confidence={_shadow_result['confidence']})")
                    # Phase 10 (GPT guidance): Representative Variance Diagnostic.
                    # Flag slots where the representative event's probability differs
                    # meaningfully from the contribution-adjusted support — a signal
                    # that the chosen representative may be an outlier within the
                    # evidence family and that selection variance is inflating the
                    # field movement for this slot. Threshold: abs(shift) >= 0.07.
                    if abs(_shadow_result.get("shift", 0.0)) >= 0.07:
                        print(f"[Phase 10] ⚠ Representative variance risk for '{_sid_c}': "
                              f"representative={_shadow_result['representative_support']:.3f} "
                              f"vs adjusted={_shadow_result['contribution_adjusted_support']:.3f} "
                              f"(shift={_shadow_result['shift']:+.3f} >= 0.07 threshold). "
                              f"Representative selection may be contributing to field movement; "
                              f"evidence families suggest a different support level.")
        if isinstance(_slot_info, dict):
            _slot_info['slot_contribution_shadow'] = _contribution_shadow
    except Exception as _contrib_err:
        print(f"[Phase 8.12] contribution-adjusted shadow error: {_contrib_err}")
    ...
    return (seeded, stage_c_edges, _slot_assignment_records, _slot_influence_records, _pressure_diags,
            _slot_contribution_records, _contribution_shadow)
