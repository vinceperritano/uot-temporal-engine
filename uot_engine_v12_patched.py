...
STEP 0 — PRIMARY BRANCH GROUP (mandatory before anything else):
Identify the user's primary question from the topic. Then identify 2–3 mutually exclusive
answers to that question from the candidate events. These MUST become the first branch group.
If no existing event captures an outcome, note it but do not invent an event.
...
{{
  "primary_question": {{
    "question_text": "Restate the user's core question in one sentence",
    "primary_branch_group_id": "the_snake_case_name_of_the_primary_branch_group",
    "canonical_outcome_slots": ["exceeds_threshold", "meets_threshold", "falls_short"],
    "confidence": 0.85
  }},
  "branch_groups": [
...
The primary_branch_group_id MUST match one of the branch_group keys you return above.
canonical_outcome_slots should be 2–4 short snake_case labels describing the possible outcomes.
For binary questions use: ["yes", "no"] or ["threshold_exceeded", "threshold_not_reached"].
For nuanced questions use 3–4 slots, e.g. ["full_agreement", "partial_agreement", "collapse"].
""".strip()
...
def validate_branch_structure(event_dicts: list) -> tuple:
    ...
    # ── Third pass: add status-quo member if ALL members are positive/escalation outcomes ──
    # A branch group without a "no-change" or "does not occur" option is incomplete.
    # Words that signal a stable/negative/status-quo outcome:
    _sq_signals = ('no_', 'not_', 'remain', 'status_quo', 'maintain', 'sustain',
                   'continu', 'uphold', 'preserv', 'persist', 'deadlock', 'stall',
                   'does_not', 'fails_to', 'unchanged', 'no change', 'no significant',
                   'informal_only', 'current', 'existing', 'keeps', 'retains')
    import uuid as _uuid_sq
    post_groups2: dict = {}
    for ev in event_dicts:
        if isinstance(ev, dict) and ev.get('branch_group'):
            post_groups2.setdefault(ev['branch_group'], []).append(ev)
    for _bg2, _mems in post_groups2.items():
        if len(_mems) < 2:
            continue  # already handled
        def _is_sq(ev_dict):
            # Normalize underscores to spaces so plain-English and snake_case labels match equally
            raw = str(ev_dict.get('branch_label','') + ' ' + ev_dict.get('label','')).lower()
            combined = raw.replace('_', ' ')
            # Space-separated signal words (after underscore normalization)
            _sq_words = [
                'no ', 'not ', 'does not ', 'fails to ', 'will not ',
                'remain', 'status quo', 'maintain', 'sustain', 'continu',
                'uphold', 'preserv', 'persist', 'deadlock', 'stall',
                'unchanged', 'no change', 'no significant', 'current',
                'keeps ', 'retains ', 'no talks', 'no dialogue', 'no resumption',
            ]
            return any(sig in combined for sig in _sq_words)
        has_sq = any(_is_sq(m) for m in _mems)
        if not has_sq:
            # All members represent escalation/change outcomes — add a status-quo member
            avg_p = sum(m.get('probability', 0.4) for m in _mems) / len(_mems)
            sq_p  = max(0.05, round(1.0 - sum(m.get('probability', 0.4) for m in _mems), 3))
            # Extra validation: if has_sq=True was triggered by "remains active" type phrases
            # (causal factors rather than genuine no-answers), override it
            if has_sq:
                _genuine_no_sigs = [
                    'no ', 'not ', 'does not ', 'fails to ', 'will not ', 'cannot ',
                    'no talks', 'no dialogue', 'no resumption', 'no change',
                    'status quo', 'unchanged', 'deadlock', 'stall', 'absence',
                    'block', 'reject', 'refus', 'frozen', 'breakdown', 'collapse',
                    'absent', 'fail', 'prevent', 'obstruct', 'halt', 'suspend',
                ]
                _has_real_no = any(
                    any(sig in (str(m.get('branch_label','')) + ' ' + str(m.get('label',''))).lower().replace('_', ' ')
                        for sig in _genuine_no_sigs)
                    for m in _mems
                )
                if not _has_real_no:
                    # "remains active/stable" matched _sq_signals but is a causal factor, not a no-answer
                    has_sq = False

            # Prefer an organic event outside any branch group that signals continuity
            _stability_labels = ('maintain', 'remain', 'continu', 'active', 'status quo',
                                 'no change', 'no significant', 'current', 'persist', 'existing')
            # Causal-factor keywords — events describing conditions/pressure/support
            # are causal inputs, not outcome members of a branch group
            _causal_factor_signals = (
                'support', 'pressure', 'domestic', 'political will', 'stance',
                'momentum', 'sentiment', 'public opinion', 'backing', 'coalition',
            )
            _organic_sq = None
            for _cand in event_dicts:
                if not isinstance(_cand, dict): continue
                if _cand.get('branch_group'):  continue  # already in a group
                if _cand.get('temporal_status') == 'resolved': continue  # past fact
                _lbl = str(_cand.get('label', '') or _cand.get('branch_label', '')).lower()
                # Skip events that look like causal factors rather than outcome members
                if any(sig in _lbl for sig in _causal_factor_signals):
                    continue
                if any(sig in _lbl for sig in _stability_labels):
                    _organic_sq = _cand
                    break

            if _organic_sq:
                # Promote organic event into this branch group as the status-quo member
                # Derive branch_label from the event's real label (snake_case) rather than generic name
                import re as _re_sq
                # Use the event's real label directly as plain English — branch labels
                # are displayed as-is, so plain language beats snake_case
                _real_lbl = str(_organic_sq.get('label', '') or 'Status quo maintained')
                _organic_sq['branch_group'] = _bg2
                _organic_sq['branch_label'] = _real_lbl
                flags.append(f"STATUS_QUO_ORGANIC: promoted '{_organic_sq.get('id')}' into '{_bg2}' as status-quo member.")
            else:
                sq_id  = 'EVT_SQ_' + _bg2.upper()[:8]
                sq_lbl = 'status_quo_maintained_no_significant_change'
                sq_ev  = {
                    'id':              sq_id,
                    'label':           'Status quo maintained — no significant change before horizon',
                    'probability':     max(0.05, sq_p),
                    'time_estimate':   2,
                    ...
                    'categories':      _mems[0].get('categories', {}),
                    'metadata':        {'synthetic_status_quo': True},
                }
                event_dicts.append(sq_ev)
                flags.append(f"STATUS_QUO_ADDED: '{_bg2}' missing stable outcome — added synthetic '{sq_lbl}'.")

    # ── Deduplication: if group has 2 status-quo-type members, keep organic/non-synthetic ──
    dedup_groups: dict = {}
    for ev in event_dicts:
        if isinstance(ev, dict) and ev.get('branch_group'):
            dedup_groups.setdefault(ev['branch_group'], []).append(ev)
    for _dgk, _dgm in dedup_groups.items():
        def _is_sq2(e):
            raw = (str(e.get('branch_label','')) + ' ' + str(e.get('label',''))).lower().replace('_', ' ')
            _sq_words2 = ['no ', 'not ', 'does not ', 'fails to ', 'will not ', 'remain', 
                          'status quo', 'maintain', 'sustain', 'continu', 'persist', 
                          'deadlock', 'unchanged', 'no change', 'no significant']
            return any(sig in raw for sig in _sq_words2)
        sq_members = [m for m in _dgm if _is_sq2(m)]
        if len(sq_members) >= 2:
            # Keep the one that is NOT synthetic (prefer organic), remove synthetic duplicate
            synthetics  = [m for m in sq_members if m.get('metadata', {}).get('synthetic_status_quo') or m.get('metadata', {}).get('synthetic_counterpart')]
            organic_sqs = [m for m in sq_members if m not in synthetics]
            if synthetics and organic_sqs:
                for _syn in synthetics:
                    _syn['branch_group'] = None
                    _syn['branch_label'] = None
                    flags.append(f"DEDUP_SQ: removed synthetic status-quo from '{_dgk}' — organic event serves same purpose.")
...
def ensure_primary_branch_group(event_dicts: list, topic: str, live_mode: bool) -> list:
    """
    Fallback: if Stage C produced zero branch groups, make a lightweight Haiku call
    to infer the primary binary outcome from the topic and assign it.
    Only runs in live mode when no groups exist at all.
    """
    # Only count groups with 2+ members — single-member groups will be nulled
    # by validate_branch_structure, so they don't count as "has groups"
    _bg_counts: dict = {}
    for _ev in event_dicts:
        if isinstance(_ev, dict) and _ev.get('branch_group'):
            _bg_counts[_ev['branch_group']] = _bg_counts.get(_ev['branch_group'], 0) + 1
    has_groups = any(c >= 2 for c in _bg_counts.values())
    if has_groups or not live_mode:
        return event_dicts

    print("[v0.12] Stage C produced no branch groups — running primary branch fallback...")
    lines_out = []
    for ev in event_dicts:
        if isinstance(ev, dict):
            lines_out.append(
                "- " + str(ev.get("id","?")) + ": " + str(ev.get("label","?")) +
                " (P=" + str(round(ev.get("probability",0.5),2)) +
                ", status=" + str(ev.get("temporal_status","?")) + ")"
            )
    events_summary = chr(10).join(lines_out)

    prompt_parts = [
        'Scenario question: "' + topic + '"',
        "",
        "Extracted events:",
        events_summary,
        "",
        "Task: Identify the primary branch group that directly answers the user question.",
        "Choose 2-3 existing events representing mutually exclusive outcomes.",
        "Use only event IDs from the list above.",
        "",
        'Return JSON only: {"primary_branch_group": "descriptive_snake_case",'
        ' "members": [{"event_id": "EVT_001", "branch_label": "short outcome label"}]}',
    ]
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("members"):
            bg_name  = str(result.get("primary_branch_group", "primary_outcome"))
            ev_map   = {ev.get("id"): ev for ev in event_dicts if isinstance(ev, dict)}
            assigned = 0
            pending  = []
            for m in result["members"]:
                ev = ev_map.get(m.get("event_id"))
                if ev:
                    pending.append((ev, str(m.get("branch_label", ev.get("label", "")))))
                    assigned += 1
            # Only assign if 2+ members — single-member groups are invalid
            if assigned >= 2:
                for ev, lbl in pending:
                    ev["branch_group"] = bg_name
                    ev["branch_label"] = lbl
                print("[v0.12] Primary branch fallback: " + bg_name + " (" + str(assigned) + " members)")
            elif assigned == 1:
                # Only one event found — add a synthetic "does not occur" counterpart
                sole_ev, sole_lbl = pending[0]
                sole_ev["branch_group"] = bg_name
                sole_ev["branch_label"] = sole_lbl
                # Create a synthetic counterpart event dict representing the opposite outcome
                import re as _re_ens
                neg_label = "does_not_occur_before_horizon"
                neg_id    = "EVT_COUNTER_001"
                # Derive a plain-language negation from the sole label
                short = _re_ens.sub(r'[^a-z0-9 ]', '', str(sole_lbl).lower())[:40].strip()
                neg_label = "no_" + _re_ens.sub(r' +', '_', short) if short else "status_quo_maintained"
                import uuid as _uuid_ens
                neg_id = "EVT_COUNTER_" + _uuid_ens.uuid4().hex[:6].upper()
                counter = {
                    "id":              neg_id,
                    "label":           "Status quo maintained — " + str(sole_ev.get("label","outcome does not occur")),
                    "probability":     round(1.0 - float(sole_ev.get("probability", 0.4)), 3),
                    "time_estimate":   sole_ev.get("time_estimate", 2),
                    "time_uncertainty": 0.4,
                    "temporal_status": "unresolved",
                    "branch_group":    bg_name,
                    "branch_label":    neg_label,
                    "disruption_score": 0.2,
                    "record_coherence": 0.5,
                    "categories":      sole_ev.get("categories", {}),
                    "metadata":        {"synthetic_counterpart": True},
                }
                event_dicts.append(counter)
                print("[v0.12] Primary branch fallback: added synthetic counterpart " + neg_id + " → " + bg_name)
            else:
                print("[v0.12] Primary branch fallback: no matching events found")
    except Exception as fallback_err:
        print("[v0.12] Primary branch fallback failed: " + str(fallback_err))
    return event_dicts
...
def infer_branch_groups_and_causal_candidates_specialized(
    ...
    try:
        result = call_anthropic_api(
            ...
            max_tokens=5000,             # Increased: primary_question + branch_groups + causal_edges
            timeout=120
        )
...
def apply_structure_to_event_candidates(
    ...
    # Case-insensitive fallback: Stage C may uppercase IDs even when event_dicts use lowercase
    id_to_event_ci  = {k.upper(): v for k, v in id_to_event.items()}
    stage_c_edges = []

    # ── PrimaryQuestion extraction (Phase 4) ──────────────────────────────────
    primary_q_raw = structure.get("primary_question")
    if isinstance(primary_q_raw, dict):
        # Store on a module-level variable so the seeding pipeline can access it
        # It will be written into run_artifacts by the API layer
        _LAST_PRIMARY_QUESTION.update({
            "question_text":          primary_q_raw.get("question_text", ""),
            "primary_branch_group_id": primary_q_raw.get("primary_branch_group_id", ""),
            "canonical_outcome_slots": primary_q_raw.get("canonical_outcome_slots", []),
            "confidence":             float(primary_q_raw.get("confidence", 0.7) or 0.7),
        })
        print(f"[v0.12] PrimaryQuestion: {_LAST_PRIMARY_QUESTION.get('primary_branch_group_id')!r} "
              f"slots={_LAST_PRIMARY_QUESTION.get('canonical_outcome_slots')}")
    ...
    for edge in structure.get("causal_edges", []):
        ...
        if not src_id or not tgt_id:
            continue
        # Try exact match first, then case-insensitive (Stage C may uppercase IDs)
        src_ev = id_to_event.get(src_id) or id_to_event_ci.get(str(src_id).upper())
        tgt_ev = id_to_event.get(tgt_id) or id_to_event_ci.get(str(tgt_id).upper())
        if not src_ev or not tgt_ev:
            continue
        # Use original-format IDs so canonicalize_extraction can remap them consistently
        src_id = src_ev["id"]
        tgt_id = tgt_ev["id"]
...
def collect_and_extract_seeded_events(topic: str, observer_basis: dict,
                                       horizon: "HorizonConfig" = None) -> tuple:
    """
    ...
    event_dicts = ensure_primary_branch_group(event_dicts, topic, LIVE_MODE)
