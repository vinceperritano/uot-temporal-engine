...
def compute_resolution_state(scores: dict, loop_converged: bool,
    ...
    if loop_converged and instability <= 0.35 and len(hard_remaining) == 0:
        return 'coherent'
    elif not loop_converged and instability <= 0.40:
        return 'unresolved'
    elif instability > 0.40 or len(hard_remaining) > 2:
        return 'strained'
    else:
        return 'failed'
...
def canonicalize_extraction(event_dicts: list) -> list:
    """
...
def collect_and_extract_seeded_events(topic: str, observer_basis: dict,
    ...
    event_dicts = canonicalize_extraction(event_dicts)
