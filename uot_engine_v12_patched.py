...
NEGATION CALIBRATION (critical — common error source):
If an event label contains explicit negation ("will NOT occur", "fails to materialize",
"does not happen", "is ruled out"), the probability you assign is the probability of
THAT EXACT NEGATED PROPOSITION being true — not the probability of the positive/underlying claim.

Example: "Trilateral Convergence Will NOT Occur" — if convergence is genuinely unlikely
(say there is strong reason to believe convergence is hard and rare), then "will NOT occur"
is the LIKELY proposition, so its probability should be HIGH (e.g. 0.85-0.95), not low.
A probability of 0.08 for "will NOT occur" means you believe convergence WILL occur with
92% confidence — verify this is truly what you mean before assigning the number.

Common failure mode: reasoning correctly about whether the underlying outcome (convergence)
is likely, then mistakenly writing that same probability onto the negated-proposition label
without flipping it. Always re-read the exact label text and ask: "is THIS sentence, as
literally written, likely or unlikely?" before assigning probability.

PREFER POSITIVE FRAMING WHEN A CLEAR POSITIVE COUNTERPART EXISTS (Phase 8.6):
When a source claim is phrased as non-occurrence, failure, absence, or impossibility, and a
clear positive counterpart exists (e.g. source says "convergence will not occur" and the
canonical slot is "convergence occurs"), prefer to create the event in POSITIVE framing.
Set the event probability to the likelihood of the positive-framed event — i.e. 1 minus the
source's claim about non-occurrence — and include both numbers so the calculation is auditable:

  "label": "<positive-framed event label>",
  "original_source_claim": "<the source's actual negated claim, verbatim or close to it>",
  "proposition_polarity": "positive_framed",
  "source_claim_probability": <probability the SOURCE'S claim, as stated, is true>,
  "probability": <probability of the POSITIVE-framed label — usually 1 - source_claim_probability>,
  "negation_calibration_check": "<one sentence explaining the flip, e.g. 'Source says non-occurrence is likely (0.78); event is framed as occurrence, so probability is 1 - 0.78 = 0.22.'>"

This does NOT mean always avoid negative framing. For TRUE no-outcome slots — "Article 5 is
not invoked", "no recession occurs", "status quo persists" — negative framing is the actual
canonical outcome and should stay negative. The rule is not "never use negation." The rule is:
make clear what proposition the probability belongs to. If the event label is negative AND it
will be mapped against a slot via inversion, this is exactly the double-negation risk case —
be especially careful to verify the number using the check above.
