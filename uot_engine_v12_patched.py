...
def compute_topic_relevance_score(slot, ev: dict, rationale: str = "") -> float:
    ...
    _status_factor = 0.7 if ev.get('temporal_status') in ('resolved', 'historical') else 1.0
        ...
        if ev and match_conf >= 0.40:
            ...
            if _engine_score >= 0.40:
                ...
                if (_relation in ('supports', 'opposes')
                        and _rel_conf >= 0.70
                        and _engine_score >= 0.20  # Phase 7 (lowered from 0.22 — 0.22 floats fail at 0.2199)
            ...
            if not _accept:
                ...
                if _engine_score < 0.20: _b_why.append(f"engine_score={_engine_score:.2f}<0.20")
