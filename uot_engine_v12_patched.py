...
def normalize_and_aggregate_primary_slots(
    ...
    def _register_effective_prob(sid, ev):
        slot = next((s for s in slots if s.slot_id == sid), None)
        if not slot: return
        ev_label = str(ev.get('label', '') or '')
        # Phase 6.6.4: Stage Q only assigns slot_polarity="yes" for BINARY
        # questions. For open_scenario/multi_outcome questions, named outcome
        # slots ("Convergence toward harmonized standards", etc.) get
        # slot_polarity="alternative" (or sometimes "partial") — these
        # represent "this specific outcome occurs" just as much as a binary
        # "yes" slot does, and need the same negation-inversion treatment.
        # "no" and "status_quo" slots represent absence/continuation, where a
        # "will not X" event may be directly SUPPORTIVE rather than opposing
        # — so they are excluded here.
        if slot.slot_polarity in ('yes', 'alternative', 'partial') and _event_negates_yes_slot(ev_label):
            eff = max(0.0, min(1.0, 1.0 - float(ev.get('probability', 0.5) or 0.5)))
            slot_effective_probs[sid][ev.get('id')] = eff
            md = ev.setdefault('metadata', {})
            md['inverted_for_slot'] = sid
            md['effective_probability'] = round(eff, 3)
            print(f"[v0.12] Slot '{sid}': event {ev.get('id')} label negates this "
                  f"yes-polarity slot — inverting probability "
                  f"{ev.get('probability'):.2f} → {eff:.2f}")
    ...
    for pos, member in enumerate(members_raw):
        ...
        if ev and match_conf >= 0.40:
            _engine_score = _slot_event_match_score(slot_by_id[matched_sid], ev)
            if _engine_score < 0.40:
                print(f"[v0.12] Stage C mapped {eid} -> '{matched_sid}' "
                      f"(confidence={match_conf:.2f}) but engine independent "
                      f"score is {_engine_score:.2f} (<0.40) - rejecting mapping, "
                      f"slot falls through to Step 2")
            else:
                ...
                _register_effective_prob(matched_sid, ev)
            print(f"[v0.12] Stage C mapped {eid} → slot '{matched_sid}' "
                  f"(confidence={match_conf:.2f}, pos={pos})")
    ...
    for slot in claim_order:
        ...
        if unclaimed:
            ...
            rep = max(evs, key=_coherence_key)
            print(f"[v0.12] Slot '{sid}': no unclaimed representative available, "
                  f"reusing {rep.get('id')} (shared with another slot)")
