...
def normalize_and_aggregate_primary_slots(
    ...
    # KEY PRINCIPLE: Stage C is the semantic authority for event→outcome mapping.
    # Never discard a Stage C member with a valid event_id — always place it somewhere.
    ...
    if not any_assigned:
        print(f"[v0.12] Step 2b: all slots empty after Stage C + Jaccard — assigning by probability rank")
        organics_ranked = sorted(
        ...
        for slot in slots:
            for ev in organics_ranked:
                ...
                if eid not in used_ids:
                    ...
                    used_ids.add(eid)
                    print(f"[v0.12] Step 2b: {eid} (p={ev.get('probability',0):.3f}) → '{slot.slot_id}'")
                    break
