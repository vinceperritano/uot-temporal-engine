...
def apply_structure_to_event_candidates(
    ...
    for edge in structure.get("causal_edges", []):
        ...
        if src_id not in id_to_event or tgt_id not in id_to_event:
            # Log first mismatch to diagnose ID format discrepancy
            if len(stage_c_edges) == 0:  # only log first miss
                sample_keys = list(id_to_event.keys())[:3]
                print(f"[APPLY_DEBUG] edge {src_id!r}→{tgt_id!r} NOT in id_to_event")
                print(f"[APPLY_DEBUG] id_to_event sample keys: {sample_keys}")
            continue
