...
Probability guidance (CRITICAL):

Estimate probability from structural evidence, causal momentum, source consistency, base-rate context, and temporal horizon. Do not assign low probability merely because sources use cautious or hedged language. Hedging may reduce confidence or increase indeterminacy, but it does not automatically mean the event is unlikely.

For example: if sources describe regulatory fragmentation as ongoing and no convergence mechanism exists, this may be high probability even if sources hedge their language. Distinguish source confidence from event probability. A cautious article can describe a highly persistent trajectory.
...
def compute_topic_relevance_score(slot, ev: dict, rationale: str = "") -> float:
    ...
    Polarity terms (polarity_match, opposition_penalty) are NOT part of this
    score. status_validity is folded into a small weight penalty for resolved
    events rather than an additive term.

    Negation stripping is now STRUCTURAL preprocessing, not a special case:
    "X will not occur" → "X will occur" for label_sim only. This reflects
    that negation marks the direction of a claim about a topic, not a different
    topic. The original label is unchanged for display and inversion detection.
    """
    ...
    # Resolved events carry partial relevance (they happened, but not future evidence)
    # Phase 7 (GPT guidance, point 4): separate temporal_status handling for
    # slot-evidence relevance vs. simulation energy. For evidence relevance,
    # resolved events remain valid — a resolved official action (e.g. EU AI Act
    # taking effect) IS strong evidence for a divergence slot. The old 0.7x penalty
    # was applying simulation-energy logic to topic relevance scoring.
    ...
    if _is_resolved and _ev_kind_for_sf == 'official_action':
        _status_factor = 1.05   # resolved official actions: strong slot evidence
    elif _is_resolved and _ev_kind_for_sf in ('causal_context', 'evidence_context'):
        _status_factor = 0.80   # context events: modest reduction when resolved
    elif _is_resolved:
        _status_factor = 0.90   # other resolved: small reduction (was 0.70)
    else:
        _status_factor = 1.00

    base_score = (0.35 * label_sim + 0.25 * description_sim + 0.15 * affinity
                  + 0.15 * rat_sim + 0.10 * hint_sim) * _status_factor
    ...
    for pos, member in enumerate(members_raw):
        ...
        if ev and match_conf >= 0.40:
            ...
            if _engine_score >= 0.40:
                ...
                # acceptance path because their label vocabulary ("0–25 bps", "25–100 bps")
                # systematically differs from natural-language evidence ("one cut", "hold",
                # "modest easing"). For threshold slots, defer to Stage C's semantic judgment
                # when relation_confidence is high, regardless of lexical score.
                ...
                # Gate B (Phase 7, GPT guidance): semantic-confidence override.
                # Numeric floor (0.19) is now understood as "minimum evidence of topical
                # contact," not a calibrated semantic score. Stage C can rescue a lexically
                # weak match when relation and rationale together provide sufficient
                # semantic grounding.
                if not _accept:
            ...
            if not _accept:
                ...
                if _rel_conf < 0.70: _b_why.append(f"rel_conf={_rel_conf:.2f}<0.70")
                if _engine_score < 0.19: _b_why.append(f"engine_score={_engine_score:.3f}<0.19")
...
PROMPT_STAGE_Q = TEMPORAL_OBSERVER_PROTOCOL + """

---

Act as a Temporal Observer defining the answer space before evidence is interpreted.

Identify the primary question being asked, the selected horizon, and the canonical outcome slots that would answer the question clearly. Do not infer outcomes from sources yet. First define the logical possibility space implied by the question.

Every primary outcome slot should be:
- mutually distinct,
- collectively sufficient for answering the question,
- stable enough to compare across future re-observations,
- named with a durable canonical slot_id,
- clear about whether it represents yes, no, partial, status quo, alternative, threshold bucket, or timeline bucket.

Preserve the possibility of non-occurrence or status quo when logically required, even if the user does not state it explicitly.
...
For threshold questions, output slot metadata fields:
  threshold_unit (e.g. "basis_points", "percentage_points", "degrees_celsius"),
  lower_bound (numeric), upper_bound (numeric),
  aliases (list of natural-language phrases that map to this bucket, e.g. ["one cut", "25 bps", "modest easing"]).
...
{{
  ...
  "canonical_slots": [
    {{
      ...
      "threshold_unit": "(string, only for threshold_bucket — e.g. \"basis_points\", \"percentage_points\")",
      "lower_bound": "(number or null, only for threshold_bucket)",
      "upper_bound": "(number or null, only for threshold_bucket)",
      "aliases": ["(natural-language phrases that map to this bucket, only for threshold_bucket)"]
    }}
  ]
}}
""".strip()
...
def collect_and_extract_seeded_events(topic: str, observer_basis: dict,
    ...
    return seeded, stage_c_edges, _slot_assignment_records, _slot_influence_records
