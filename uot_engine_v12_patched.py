...
def _slot_event_match_score(slot, ev: dict) -> float:
    ...
    # Phase 6.8.1: de-negate the event label before computing label_sim.
    # An event "X Will Not Occur" has the same TOPIC relevance to a slot
    # about X as "X Will Occur" — only the polarity differs. Polarity is
    # handled separately by relation_to_slot and the inversion pipeline;
    # here we want to measure pure topic overlap. Strip the negation pattern
    # before computing label_sim only (ev_text / description_sim is left
    # unchanged — descriptions are factual and we want accuracy there).
    _label_for_sim = label
    for _neg_re, _pos_repl in _NEGATION_STRIP_PATTERNS:
        if _neg_re.search(_label_for_sim):
            _label_for_sim = _neg_re.sub(_pos_repl, _label_for_sim, count=1)
            break
    _no_x_m = _NO_X_WILL_Y.match(_label_for_sim)
    if _no_x_m:
        _rest = _no_x_m.group(1).strip()
        _label_for_sim = f"{(_rest[0].upper()+_rest[1:]) if _rest else _rest} Will {_no_x_m.group(2)}"
    label_sim       = _jaccard_sim(slot.label, _label_for_sim)
    ...
    for pos, member in enumerate(members_raw):
        ...
        if ev and match_conf >= 0.40:
            ...
            if _engine_score >= 0.40:
                ...
                # Phase 6.8.1: fall back to match_confidence when Stage C
                # omits relation_confidence (a separate field from match_confidence
                # in the schema — Stage C often provides one but not the other).
                # match_confidence is a reasonable proxy: if Stage C is highly
                # confident this event maps to this slot, that confidence
                # implicitly covers the relation it stated.
            ...
            if not _accept:
                ...
                if _engine_score < 0.22: _b_why.append(f"engine_score={_engine_score:.2f}<0.22")
