...
def compute_slot_influence_pressures(
    ...
    # Index events by id for lookup — handle both dict and dataclass (SeededEvent) objects
    ev_by_id = {}
    ...
    for slot in (slots or []):
        ...
        for inf in slot_influences:
            ...
            src_ev = ev_by_id.get(str(src_ev_id), {})
            ...
            applied_list.append({
                ...
                "pressure": round(k_slot_pressure * pressure, 4),
                "rationale": rationale[:100],
            })

        # Clamp total logit delta
        ...
        diag = {
            ...
            "influence_logit_delta":      round(logit_delta, 4),
        ...
        if ENABLE_APPLY and logit_delta != 0.0:
            ...
            print(f"[Phase 7.3] SlotInfluence applied: {sid} "
                  f"{raw_p:.3f} → {candidate_p:.3f} (Δlogit={logit_delta:+.3f})")
        else:
            print(f"[Phase 7.3] SlotInfluence diagnostic: {sid} "
                  f"raw={raw_p:.3f} → candidate={candidate_p:.3f} (Δlogit={logit_delta:+.3f}, "
...
def compute_topic_relevance_score(slot, ev: dict, rationale: str = "") -> float:
    ...
    # Include slot semantic_aliases if Stage Q provided them
...
CRITICAL — Description quality (GPT Phase 7 guidance):
For every canonical slot, write the description using the scenario primary actors, geographic scope, and concrete evidence patterns. Do not write generic descriptions such as "escalation occurs" or "status quo continues." Instead write actor-specific descriptions: "North Korea escalates regional tensions through ICBM tests, nuclear demonstrations, missile deployments, or provocative military actions." Include action_exemplars (2-4 evidence patterns that would support the slot) and negative_examples (1-2 patterns that would NOT support it despite surface similarity). Populate actor_scope, geographic_scope, scale_level, and directionality for each slot.
...
{{
  ...
  "canonical_slots": [
    {{
      "slot_id": "...",
      "label": "...",
      "description": "(REQUIRED: write actor-specific, not generic — see instructions above)",
      "slot_polarity": "yes|no|partial|status_quo|alternative",
      "slot_kind": "outcome|status_quo|threshold_bucket|timeline_bucket",
      "actor_scope": "(primary actor(s) for this slot, e.g. \"North Korea\", \"US-EU-China\", \"Federal Reserve\")",
      "geographic_scope": "(domestic_us | international | regional | global | bilateral | etc.)",
      "scale_level": "(subnational | national | bilateral | multilateral | global)",
      "directionality": "(continuation | intensification | reversal | breakthrough | decline | uncertain)",
      "action_exemplars": ["(2-4 concrete evidence patterns that would support this slot)"],
      "negative_examples": ["(1-2 evidence patterns that would NOT support this slot despite surface similarity)"],
