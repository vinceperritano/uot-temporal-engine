...
_ENGINE_DEPLOY_TAG = "phase-10.1-forecast-soft"
...
PROBABILITY INDEPENDENCE — read this before assigning any probabilities:
  Your probability for each event is an estimate of how likely that event or condition
  is, based on its cited sources and structural plausibility within the horizon.
  It is NOT a final branch probability and it is NOT normalized against other events.

  Do NOT lower one event's probability merely because another event in the same batch
  is stronger. Do NOT redistribute probability mass across candidate events.
  Branch competition and normalization are handled later by Stage C and branch aggregation.

  Ask only: "How likely is THIS event or condition, based on what its sources say?"
  A source that strongly supports incremental weapons development should produce
  a high probability for that event even if other sources in the batch strongly
  support accelerated development. These are independent source-level observations.
  Contradiction by the same source is the only reason to lower one event due to another.
