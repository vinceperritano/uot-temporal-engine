...
_ENGINE_DEPLOY_TAG = "phase-9.8-slotinfluence-ids"
...
NATURAL EVIDENCE EXTRACTION (Phase 8.10 — replaces earlier negation-calibration approach):
Describe evidence in natural source-facing language. Do NOT construct a negated version of a
canonical slot label merely because the source weakens or disconfirms that slot. This is the
root fix for a recurring failure mode: when Stage A invented negated-slot labels like
"Trilateral Convergence Will Not Occur," it sometimes assigned probability to the wrong side
of its own constructed proposition, producing backwards results (e.g. 0.05 for "will not occur"
meaning 95% confidence convergence WOULD happen, when the evidence actually meant convergence
was unlikely).

You are NOT responsible for inverting evidence against a slot. That is Stage C's job, using the
natural event you extract. Your only job is to describe what the source actually claims and assign
probability to THAT natural claim.

PROBABILITY CALIBRATION FOR NEGATIVE FRAMINGS (critical):
If a source uses language like "X will not occur," "X is ruled out," "X fails to happen," your
probability for that event should reflect confidence that the negative claim is TRUE — i.e. how
likely it is that X will NOT occur. Recurring failure mode to avoid:

  Source says: "Trilateral AI harmonization will not occur within the horizon"
  WRONG: probability=0.10  ← means only 10% chance it will not occur = 90% chance it WILL occur
                              (backwards: source says harmonization is UNLIKELY)
  RIGHT: probability=0.82  ← means 82% chance it will not occur (matches the source's claim)

The test: if your label contains "Will Not Occur" or similar, your probability should be HIGH
(> 0.50) when the source believes the negative claim strongly. LOW (< 0.50) only if the source
is explicitly uncertain about whether the non-occurrence will actually happen.

For disconfirming evidence, describe the actual development reported by the source:
  "Bilateral AI coordination continues without China"
  "Regional frameworks remain fragmented"
  "No comprehensive agreement is scheduled"
  "Central bank officials signal caution on large cuts"

Do NOT rewrite these as constructed negations of a slot's own positive phrasing:
  "Global harmonization will not occur"          ← WRONG: invented negation of a slot label
  "The 100-200 bps bucket will not happen"        ← WRONG: invented negation of a slot label
  "The positive slot fails"                       ← WRONG: invented negation of a slot label

TRUE negative/status-quo outcomes remain completely normal and should NOT be avoided:
  "Article 5 remains uninvoked"                   ← fine: this IS what happened/is claimed
  "No formal ceasefire agreement occurs"          ← fine: this IS the natural claim
  "Status quo persists"                           ← fine: this IS the natural claim
The distinction: allowed = a natural non-occurrence or status-quo claim the source actually
makes. Not allowed = inventing "[positive slot label] will not occur" as a wrapper around
disconfirming evidence when the source never phrased it that way.

If you find yourself about to write a label containing both (a) the core wording of a
canonical slot's positive label and (b) a negation word ("will not," "does not," "fails to,"
"no," "ruled out") — stop. Rewrite the label to describe what the source actually claims in
its own terms instead. Stage C will determine how your natural event relates to the canonical
slots; your task is to extract what the evidence says, not to convert evidence into slot
negations.
...
IMPORTANT: For any slot representing a continuation, status quo, stability, or non-change outcome, always set slot_kind="status_quo". Examples: "North Korea maintains current trajectory", "Continued incremental development", "No major change", "Status quo persists". This ensures the engine creates a synthetic placeholder when no organic evidence maps to this slot, preserving it as a tracked outcome.
""".strip()
