...
def compute_branch_potential_details(world):
    ...
    for gname, gnodes in groups.items():
        ...
        # Phase 6.7: for primary-group members, slot_raw_probability (set by
        # normalize_and_aggregate_primary_slots Step 4) already reflects
        # relation-aware inversion — an "opposes" event's raw probability has
        # already been inverted INTO slot_raw_probability for this slot's
        # aggregation, while node.probability remains the event's own
        # un-inverted independent estimate (Phase 6.6). Normalizing the branch
        # display from node.probability would silently undo the inversion
        # (e.g. showing 60% instead of the inverted ~13.5% for an "opposes"
        # mapping). Prefer slot_raw_probability when present; fall back to
        # node.probability for secondary groups and pre-6.7 runs.
        probs  = [max(getattr(nd, 'slot_raw_probability', None)
                      if getattr(nd, 'slot_raw_probability', None) is not None
                      else nd.probability, 1e-9) for nd in gnodes]
...
_SEMANTIC_CANON_MAP = {
    ...
    'persists': 'continue', 'persisting': 'continue',
...
def aggregate_slot_probability(events: list, effective_probs: dict = None) -> float:
    """
    ...
    Phase 6.6.3 (GPT guidance, point 1): effective_probs is an optional
    {event_id: effective_probability} map. When an event's label is a
    negation of the slot's proposition (see _event_negates_yes_slot),
    its EFFECTIVE probability for THIS slot is 1 - event.probability,
    NOT its own raw probability. event.probability itself is never
    mutated — it remains the event's own independent measured value
    (Phase 6.6). The inversion applies only to this slot's aggregation.
    """
    if not events:
        return 0.5
...
def _slot_event_match_score(slot, ev: dict) -> float:
    """
    Phase 6.6.5 (GPT guidance): deterministic slot-match score for a single
    (slot, event) pair. Used both by find_organic_events_for_slot (Step 2's
    own search) AND to independently VERIFY Stage C's proposed mappings in
    Step 1 — Stage C is advisory, the engine's score is authoritative.

    Score formula:
      match_score = 0.45*label_sim + 0.25*description_sim + 0.15*hint_sim
                     + 0.10*polarity_match + 0.05*status_validity

    Phase 6.7 (GPT guidance, point 1): the opposition penalty was REMOVED.
    Under the pre-6.6 model, an event opposing a slot's proposition was
    treated as a worse match (penalized). Under the inversion-aware model
    (6.6.3/6.6.4 and the relation_to_slot-based two-gate acceptance below),
    "this event is the negation of the slot's proposition" is a VALID match
    with a known relationship (relation_to_slot="opposes") — not a sign of
    irrelevance. Penalizing it here double-counts what relation_to_slot and
    inversion already handle correctly.
    """
    ...
    jaccard_score = (0.45 * label_sim + 0.25 * description_sim + 0.15 * hint_sim
                      + 0.10 * polarity_match + 0.05 * status_validity)

    # Phase 6.6.5: Stage A's own slot_affinities (Pass 2) is a second,
    # independent relevance signal that can catch semantically-equivalent
    # but lexically-different matches Jaccard misses (e.g. "expanding nuclear
    # production infrastructure" vs "rapid advancement in nuclear capability").
    # A genuinely irrelevant event will score low on BOTH signals.
    affinity = float((ev.get('slot_affinities') or {}).get(slot.slot_id, 0.0) or 0.0)

    return max(jaccard_score, affinity)
...
def normalize_and_aggregate_primary_slots(
    ...
    for pos, member in enumerate(members_raw):
        ...
        if ev and match_conf >= 0.40:
            ...
            _engine_score = _slot_event_match_score(_slot_obj, ev)
            ...
            if _engine_score >= 0.40:
                ...
                _rel_conf   = float(_rel_conf) if _rel_conf is not None else 0.0
            ...
            if not _accept:
                print(f"[v0.12] Stage C mapped {eid} -> '{matched_sid}' "
                      ...
                      f"rescue did not apply - rejecting mapping, slot falls "
                      f"through to Step 2")
            else:
                ...
                if member.get('event_kind'):
                    ev.setdefault('metadata', {})['event_kind'] = member['event_kind']
        ...
        # Additional candidates beyond best_event_id contribute to aggregation
        # if they also clear the weak-match bar.
        if candidate_ids and isinstance(candidate_ids, list):
            for cand_id in candidate_ids[1:]:  # skip best_event_id (already handled)
                cand_ev = _lookup(cand_id)
                if cand_ev and cand_ev.get('id') not in [e.get('id') for e in assignments[matched_sid]]:
                    _cand_score = _slot_event_match_score(slot_by_id[matched_sid], cand_ev)
                    if _cand_score < 0.40:
                        print(f"[v0.12] Stage C secondary candidate {cand_id} -> slot "
                              f"'{matched_sid}' rejected (engine score {_cand_score:.2f} < 0.40)")
                    else:
                        assignments[matched_sid].append(cand_ev)
    ...
    if is_binary and len(slots) == 2:
        ...
        if len(mapped) == 1 and len(unmapped) == 1:
            binary_mapped_raw = aggregate_slot_probability(
    ...
    for slot in slots:
        if assignments[slot.slot_id]:
            raw_probs[slot.slot_id] = aggregate_slot_probability(
    ...
    # Phase 6.7 (GPT guidance, point 7): additional evidence-composition metrics,
    # computed after final slot-assignment resolution alongside the existing counts.
    inverted_count       = 0
    weak_count           = 0
    ...
    for slot in slots:
        ...
        if rep_md.get('event_kind') in ('forecast_signal', 'market_signal'):
            meta_signal_count += 1
    ...
    seed_quality = {
        ...
        # Phase 6.7 (GPT guidance, point 7)
    ...
    print(f"[v0.12] Seed quality: organic_rate={organic_rate:.2f} ({quality_flag}), "
          ...
          f"inverted={inverted_count}, weak={weak_count}, reused_source_events={reused_source_events}, "
          f"meta_signal={meta_signal_count}")
...
For each slot, return:
  ...
  - positive_framing_label: a short rephrasing of the event's proposition in the
    SAME polarity as this slot (i.e., if the event is "X will not occur" and the
    slot is about X occurring, give "X occurs" / "X materializes" — a positive,
    non-negated framing of what the event is actually evidence ABOUT). Used for
    display so users see "X occurs — 22%" rather than a confusing pairing of a
    positive-framed slot name with a negative-framed event's raw probability.
  - event_kind: classify the event itself —
      "substantive_development" = an actual real-world event/development
      "official_action" = a government/institutional action or policy decision
      ...
      "causal_context" = background factor that influences outcomes but isn't itself
                  an outcome
...
def generate_search_queries(topic: str, observer_basis: dict, canonical_slots=None) -> List[dict]:
    """
...
def collect_and_extract_seeded_events(topic: str, observer_basis: dict,
    ...
    queries = generate_search_queries(topic, observer_basis, canonical_slots=_pq.canonical_slots if _pq else None)
    print(f"[v0.12] Stage 0: {len(queries)} queries ({queries[0]['purpose']} + {len(queries)-1} targeted)")
