...
1. BRANCH GROUPS: mutually exclusive alternative outcomes of the same resolution question.
   ...
   - CRITICAL: NEVER place events with temporal_status "resolved" or "active" inside a branch group.
     Resolved/active events describe current or past facts — they exert causal pressure on futures
     but do not compete with them. Only "unresolved" events belong in branch groups.
     BAD: putting "State AI laws already in effect (active)" inside a branch group.
     GOOD: making it a standalone event with edges flowing into the unresolved branch members.
...
def validate_branch_structure(event_dicts: list) -> tuple:
    ...
    for bg_key, members in groups.items():
        ...
        # Active/resolved events in branch groups — remove them, they're not future possibilities
        for ev in members[:]:  # copy to allow mutation during iteration
            status = ev.get('temporal_status', 'unresolved')
            if status in ('resolved', 'active'):
                flags.append(
                    f"ACTIVE_RESOLVED_IN_GROUP: '{bg_key}' contains '{ev.get('label','?')}' "
                    f"with status='{status}' — removed from branch group. "
    ...
    for _bg2, _mems in post_groups2.items():
        ...
        def _is_sq(ev_dict):
            combined = (str(ev_dict.get('branch_label','')+' '+ev_dict.get('label','')).lower())
            return any(sig in combined for sig in _sq_signals)
        ...
        if not has_sq:
            ...
            if _organic_sq:
                ...
                _organic_sq['branch_label'] = 'status_quo_maintained'
    ...
    for _dgk, _dgm in dedup_groups.items():
        def _is_sq2(e): 
            return any(sig in (str(e.get('branch_label',''))+' '+str(e.get('label',''))).lower() for sig in _sq_signals)
