...
1. BRANCH GROUPS: mutually exclusive alternative outcomes of the same resolution question.
   ...
   - Minimum 2 members per branch group. Include at least one "no-change / status-quo" outcome.
    ...
    try:
        ...
        if isinstance(result, dict) and result.get("members"):
            ...
            if assigned >= 2:
                ...
                print("[v0.12] Primary branch fallback: only " + str(assigned) + " member(s) found — skipping single-member group")
...
def infer_branch_groups_and_causal_candidates_specialized(
    ...
    try:
        ...
        if isinstance(result, dict):
            # ── Diagnostic: log what Stage C actually returned ────────────────
            print(f"[STAGE_C_DEBUG] keys={list(result.keys())}")
            print(f"[STAGE_C_DEBUG] branch_groups={len(result.get('branch_groups',[]))}")
            print(f"[STAGE_C_DEBUG] causal_edges={len(result.get('causal_edges',[]))}")
            if result.get('causal_edges'):
                first = result['causal_edges'][0]
                print(f"[STAGE_C_DEBUG] first_edge={first.get('source_id','?')}→{first.get('target_id','?')}")
            return result
