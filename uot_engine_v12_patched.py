...
def find_organic_events_for_slot(slot, event_dicts: list) -> list:
    ...
    Uses Jaccard similarity + opposition guard + polarity check.
    Thresholds from GPT: >= 0.62 strong, 0.48-0.62 if polarity agrees.
    """
    candidates = []
    for ev in event_dicts:
        ...
        if ev.get('outcome_role') in ('causal_context', 'evidence_context'): continue
        label = ev.get('label', '') or ev.get('branch_label', '') or ''
        if not label: continue
        if _has_slot_opposition(slot.label, label): continue
        sim = _jaccard_sim(slot.label, label)
        if sim >= 0.62:
            candidates.append((sim, ev))
        elif sim >= 0.48 and _polarity_matches_label(slot.slot_polarity, label):
            candidates.append((sim, ev))
        elif sim < 0.48 and _polarity_matches_label(slot.slot_polarity, label) and sim >= 0.30:
            candidates.append((sim * 0.8, ev))  # penalized score, accept anyway
    candidates.sort(key=lambda x: -x[0])
    return [ev for _, ev in candidates]
...
def normalize_and_aggregate_primary_slots(
    ...
    slots = locked_slots or (primary_question.canonical_slots if primary_question else [])
    if not slots:
        return event_dicts, {}
    ...
    # ── Step 1: Use explicit Stage C mappings first ───────────────────────────
    ...
    def _lookup(eid):
        ...
        return id_to_event.get(eid) or id_to_ci.get(str(eid).upper())

    for member in (pbg.get("members") or []):
        if not isinstance(member, dict): continue
        slot_id = str(member.get("slot_id", ""))
        # normalize: try exact match first, then fuzzy match to stored slot IDs
        matched_sid = slot_id if slot_id in assignments else None
        if not matched_sid:
            for s in slots:
                if s.slot_id == slot_id or _jaccard_sim(s.slot_id, slot_id) >= 0.80:
                    matched_sid = s.slot_id; break
        if not matched_sid: continue
    ...
    # ── Step 3: Create synthetics only for still-empty slots ─────────────────
    for slot in slots:
        if not assignments[slot.slot_id] and slot.synthetic_allowed:
            sid = "EVT_SYNTH_" + _uuid_agg.uuid4().hex[:6].upper()
            or_role = "status_quo_outcome" if slot.slot_kind == "status_quo" else "primary_outcome"
            synth = {
                "id": sid, "label": slot.label, "probability": 0.5,
                ...
                "metadata": {
                    "synthetic": "true",
                    "synthetic_reason": "no_organic_candidate_for_canonical_slot",
            ...
            print(f"[v0.12] Slot '{slot.slot_id}' — no organic match, created synthetic {sid}")

    # ── Step 4: Aggregate raw probabilities ──────────────────────────────────
    raw_probs = {}
    for slot in slots:
        raw_probs[slot.slot_id] = aggregate_slot_probability(assignments[slot.slot_id])

    # ── Step 5: Normalize across canonical branch group ──────────────────────
    total = sum(max(p, 1e-6) for p in raw_probs.values())
    normalized = {sid: raw_probs[sid] / total for sid in raw_probs}
    ...
    for slot in slots:
        ...
        rep["probability"]        = normalized[sid]
        if len(evs) > 1:
            rep["mapped_event_ids"] = [e["id"] for e in evs]
        print(f"[v0.12] Slot '{sid}': {len(evs)} event(s) → p={normalized[sid]:.3f} "
              f"(rep={rep['id']})")
...
def run_stage_q(topic: str, horizon_label: str) -> 'PrimaryQuestion':
    ...
    for s in result.get("canonical_slots", []):
        ...
        slots.append(OutcomeSlot(
            ...
            slot_polarity = str(s.get("slot_polarity", "yes")),
            slot_kind     = str(s.get("slot_kind",     "outcome")),
        ))
...
For temporal re-observations of tracked scenarios (when slots are marked "locked"):
Map current evidence back onto the locked canonical slots. Do not invent, remove, or rename
primary outcome slots. If evidence suggests a new outcome outside the locked slots, place it
in secondary_branch_groups with outcome_role="branch_outcome" and flag it.
""".strip()
...
def apply_structure_to_event_candidates(
    ...
    # ── Slot completion fallback ──────────────────────────────────────────────
    # If Stage C returned an empty primary_branch_group or missed slots,
    # create synthetic events from PrimaryQuestion's canonical_slots directly.
    if primary_question and primary_question.canonical_slots:
        bg_id_for_fill = bg_id or "primary_outcome"
        existing_bg_events = [e for e in event_dicts
                              if isinstance(e, dict) and e.get("branch_group") == bg_id_for_fill]
        if not existing_bg_events:
            # Stage C produced nothing — create all slots as synthetics
            print(f"[v0.12] Stage C primary_branch_group empty — creating synthetics from Stage Q slots")
            import uuid as _uuid_fill
            for slot in primary_question.canonical_slots:
                synth_id = "EVT_SYNTH_" + _uuid_fill.uuid4().hex[:6].upper()
                or_role   = "status_quo_outcome" if slot.slot_kind == "status_quo" else "primary_outcome"
                synth_ev = {
                    "id":              synth_id,
                    "label":           slot.label,
                    ...
                    "time_uncertainty": 0.4,
                    "temporal_status": "unresolved",
                    "branch_group":    bg_id_for_fill,
                    "branch_label":    slot.label,
                    "outcome_role":    or_role,
                    "disruption_score": 0.2,
                    "record_coherence": 0.5,
                    "categories":      {},
                    "metadata": {
                        "synthetic":        "true",
                        "synthetic_reason": "stage_c_primary_group_empty_fallback",
                        "slot_id":          slot.slot_id,
                        "slot_polarity":    slot.slot_polarity,
                    },
                }
                event_dicts.append(synth_ev)
                id_to_event[synth_id] = synth_ev
                print(f"[v0.12] Slot fallback: created {synth_id} for slot '{slot.slot_id}'")
            _LAST_PRIMARY_QUESTION.update({
                "primary_branch_group_id": bg_id_for_fill,
                "canonical_outcome_slots": [s.slot_id for s in primary_question.canonical_slots],
...
def collect_and_extract_seeded_events(topic: str, observer_basis: dict,
    ...
    if locked_primary_question and locked_canonical_slots:
        print("[v0.12] Stage Q: using locked canonical slots from series (temporal re-observation)")
        if isinstance(locked_primary_question, dict):
            _pq = PrimaryQuestion(
                text=locked_primary_question.get("question_text", topic),
                question_type=locked_primary_question.get("question_type", "binary"),
                horizon_label=locked_primary_question.get("horizon_label", ""),
                canonical_slots=[
                    OutcomeSlot(
                    ...
                    for s in (locked_canonical_slots if isinstance(locked_canonical_slots, list)
                              else [])
                ]
            )
        else:
            _pq = locked_primary_question
    ...
    # Then apply_structure handles secondary branches and causal edges
    _locked_slots = locked_canonical_slots or (kwargs.get("locked_canonical_slots") if kwargs else None)
    event_dicts, _slot_info = normalize_and_aggregate_primary_slots(
        event_dicts, _pq, structure, locked_slots=_locked_slots
    )
    # apply_structure handles secondary branches + causal edges (primary BG already done)
